...
Goal: Represent models as tools with clear contracts and a simple router.
...
Step 2) Implement two tools backed by Foundry Local
```python
# tools/impl.py
import requests, os
...
Goal: Use the OpenAI Python SDK against the Foundry Local endpoint.
...
Goal: Mirror local models with Azure endpoints for overflow capacity.

Step 1) Decide routing strategy
- Local-first for privacy/latency, Azure fallback on errors or large prompts

Step 2) Implement a simple router stub
```python
# hybrid/router.py
import os, requests
...
- [ ] Call Foundry Local via OpenAI SDK and raw REST
- [ ] Enforce JSON outputs for a domain template
- [ ] Sanitize and log calls locally
- [ ] Implement a simple hybrid router with Azure fallback

## Wrap-up

Foundry Local enables robust on-device AI where models become composable tools. With clear interfaces, governance, and hybrid scaling, teams can ship real-time, secure AI apps that respect user privacy while remaining enterprise-ready.
