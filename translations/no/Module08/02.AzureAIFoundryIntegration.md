...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T23:22:01+00:00",
  "source_file": "Module08/02.AzureAIFoundryIntegration.md",
  "language_code": "no"
}
-->
# Sesjon 2: OpenAI SDK-integrasjon med Azure AI Foundry

## Oversikt

Basert på din Foundry Local-grunnlag, fokuserer denne sesjonen på avanserte integrasjonsmønstre for OpenAI SDK som støtter både Microsoft Foundry Local og Azure OpenAI. Du vil lære å bygge fleksible AI-applikasjoner som kan kjøre lokalt for personvern og utvikling, samtidig som de gir skalerbarhet i skyen via Azure OpenAI når det er nødvendig.

## Læringsmål

Ved slutten av denne sesjonen vil du:
- Mestre avansert OpenAI SDK-integrasjon med både Foundry Local og Azure OpenAI
- Implementere strømming av svar for bedre brukeropplevelse
- Lage robuste klientfabrikkmønstre for støtte av flere leverandører
- Bygge samtalehåndteringssystemer med kontekstbevaring
- Etablere ytelsesbenchmarking og helsesjekk
- Distribuere produksjonsklare applikasjoner med riktig feilhåndtering

## Forutsetninger

- Fullført Sesjon 1: Komme i gang med Foundry Local
- Aktiv Foundry Local-installasjon med kjørende modeller
- Python 3.8 eller nyere med støtte for virtuelle miljøer
- OpenAI Python SDK installert (`pip install openai foundry-local-sdk`)
- Azure-konto med OpenAI-tjeneste (valgfritt, for sky-scenarier)
- Grunnleggende forståelse av Python async/await-mønstre

## Del 1: OpenAI SDK-klientfabrikkmønster

### Forstå arkitektur for flere leverandører

Å bygge applikasjoner som fungerer med både Foundry Local og Azure OpenAI krever et fleksibelt mønster for klientopprettelse:
...
## Del 2: Strømming av svar og sanntidsinteraksjon

### Implementering av strømming for chat-kompletteringer

Strømming gir en bedre brukeropplevelse ved å vise svar mens de genereres:
...
### Håndtering av samtaler med flere turer
...
## Del 3: Ytelsesbenchmarking og analyse

### Måling av responstid

Mål og sammenlign ytelse på tvers av ulike konfigurasjoner:
...
### Testing av temperatur og parametere
...
## Del 4: Helsesjekk og diagnostikk for tjenester

### Omfattende helsesjekksystem
...
### Validator for miljøkonfigurasjon
...
## Del 5: Miljøvariabler og konfigurasjonsstyring

### Referanse for miljøvariabler

Fullstendig referanse for konfigurasjon av begge leverandører:
...
**Windows Command Prompt-oppsett:**
...
## Del 6: Praktiske øvelser

### Øvelse 1: SDK-integrasjon med flere leverandører

Bygg en komplett applikasjon som sømløst bytter mellom leverandører:
...
### Øvelse 2: Avansert samtalehåndtering
...
### Øvelse 3: Produksjonsklar helsesjekk
...
## Del 7: Oppsummering og neste steg

### Oppnåelser i sesjonen

I denne sesjonen har du mestret:
- ✅ **OpenAI SDK-integrasjon**: Avanserte mønstre for både Foundry Local og Azure OpenAI
- ✅ **Strømming av svar**: Sanntids chat-kompletteringer for bedre brukeropplevelse
- ✅ **Støtte for flere leverandører**: Sømløs bytting mellom lokale og skybaserte AI-tjenester
- ✅ **Samtalehåndtering**: Kontekstbevisste samtaler med flere turer
- ✅ **Ytelsesovervåking**: Benchmarking og helsesjekk for produksjonsdistribusjoner
- ✅ **Produksjonsmønstre**: Robust feilhåndtering og konfigurasjonsstyring

### Nøkkelmønstre for arkitektur
...
1. **🔄 Implementer alltid fallbacks**: Azure → Foundry Local → Feilhåndtering
2. **🌊 Bruk strømming for lange svar**: Bedre opplevd ytelse
3. **🛡️ Implementer omfattende feilhåndtering**: Brukervennlige feilmeldinger
4. **📈 Overvåk ytelse**: Spor responstider og suksessrater
5. **⚙️ Miljøbasert konfigurasjon**: Enkel bytting mellom utvikling/staging/produksjon
6. **🔒 Sikker håndtering av legitimasjon**: Aldri hardkod API-nøkler

### Retningslinjer for valg av leverandør

| Scenario | Anbefalt leverandør | Begrunnelse |
|----------|---------------------|-------------|
| **Utvikling** | Foundry Local | Rask iterasjon, ingen API-kostnader |
| **Personvernkritisk** | Foundry Local | Data forlater aldri enheten |
| **Produksjon med høyt volum** | Azure OpenAI | Bedre skalerbarhet, enterprise SLA |
| **Nyeste modeller** | Azure OpenAI | Tilgang til de nyeste modellene |
| **Krav til offline bruk** | Foundry Local | Ingen avhengighet av internett |
| **Kostnadssensitiv** | Foundry Local | Ingen kostnader per token |

### Hurtigreferanse for miljøvariabler
...
### Forberedelse til Sesjon 3: Åpen kildekode-modeller

1. **Utforsk modellkatalogen**: Gjennomgå tilgjengelige modeller i Foundry Local
2. **Forstå modellformater**: Lær om ONNX, kvantisering og optimalisering
3. **Vurder tilpassede modeller**: Tenk på krav for domene-spesifikke modeller

### Hurtigveiledning for feilsøking
...
- **[OpenAI Python SDK-dokumentasjon](https://github.com/openai/openai-python)**: Offisiell SDK-referanse
- **[Azure OpenAI-dokumentasjon](https://learn.microsoft.com/azure/ai-services/openai/)**: Veiledning for Azure OpenAI-tjenesten
- **[Foundry Local SDK-referanse](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**: Dokumentasjon for lokal inferens
- **[Veiledning for strømming av svar](https://learn.microsoft.com/azure/ai-foundry/foundry-local/how-to/integrate-with-inference-sdks)**: Avanserte strømmemønstre
- **[Eksempel 01: Rask chat via OpenAI SDK](samples/01/README.md)**: Grunnleggende integrasjonsmønstre
- **[Eksempel 02: Avansert SDK-integrasjon](samples/02/README.md)**: Praktiske eksempler fra denne sesjonen
- **[Eksempel 04: Chainlit-applikasjon](samples/04/README.md)**: Utvikling av webgrensesnitt
- **[Eksempel 05: Multi-agent-systemer](samples/05/README.md)**: Avansert orkestrering

Du er nå klar til å bygge sofistikerte AI-applikasjoner som sømløst integrerer lokale og skybaserte AI-funksjoner, og gir fleksibiliteten til å velge riktig leverandør for hver spesifikke brukssituasjon samtidig som du opprettholder konsistente utviklingsmønstre.
