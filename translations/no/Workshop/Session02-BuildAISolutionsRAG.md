...
## Validering: Sjekk av Foundry Local-miljø

Før du starter demoene, valider ditt lokale miljø:
...
### 2. Integrasjon av strukturerte data

- **CSV-behandling**: Pandas-integrasjon med AI-modeller
- **Statistisk analyse**: Automatisk datasammendrag
- **Kontekstskaping**: Dynamisk generering av kontekst basert på forespørsler
- **Støtte for flere formater**: JSON, CSV og tabulære data
...
3. Kjør `evaluate(dataset, metrics=[...])`
4. Lagre metrikker (CSV/JSON) for trendanalyse.

#### Hurtigstart for vektorlagring (FAISS)
...
For diskpersistens bruk `faiss.write_index(index, "kb.index")`.
