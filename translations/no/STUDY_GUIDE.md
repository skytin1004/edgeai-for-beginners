...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T20:04:48+00:00",
...
| Dag 7 | Modul 6-7: AI-agenter & Utviklingsverktøy | 5 timer |
...
| Modul | Fullføringsdato | Brukt Tid | Viktige Lærdommer |
|--------|----------------|-----------|-------------------|
...
Velkommen til studieveiledningen for EdgeAI for Nybegynnere! Dette dokumentet er laget for å hjelpe deg med å navigere kursmaterialet effektivt og få mest mulig ut av læringsopplevelsen. Det gir strukturerte læringsveier, foreslåtte studieplaner, sammendrag av nøkkelkonsepter og tilleggsmateriale for å utdype forståelsen av EdgeAI-teknologier.
...
5. **SLMOps - Operasjoner for Små Språkmodeller** - Produksjonslivssyklus og implementering
6. **SLM Agentiske Systemer** - AI-agenter, funksjonskall og Model Context Protocol
7. **EdgeAI Implementeringsprøver** - AI-verktøysett, Windows-utvikling og plattformspesifikke implementeringer
8. **Microsoft Foundry Local – Komplett Utviklerverktøysett** - Lokal-først utvikling med hybrid Azure-integrasjon (Modul 08)
...
- **Tilleggsmateriale**: Utforsk ekstra ressurser for emner som interesserer deg mest
...
| Dag 6 | Modul 8: Foundry Local Verktøysett | 3 timer |
...
| Uke 3 | Modul 3: SLM Implementering (3t) + Modul 8: Foundry Local Verktøysett (2-3t) | 5-6 timer |

## Modul 1: EdgeAI Grunnleggende og Transformasjon

### Viktige Læringsmål

- Forstå forskjellene mellom skybasert og edge-basert AI
...
1. Sammenlign og kontraster skybasert AI med edge-basert AI-implementeringer.
...
- **Prioriterte Konsepter**: 
  ...
  - Optimaliseringsrammeverk for inferens
  - Bærekraftshensyn
...
3. **Rask Optimalisering**: Bruk en optimaliseringsteknikk for å forbedre inferenshastigheten (1 time)
...
4. Hva er de viktigste ytelsesmålingene å overvåke for en edge-implementert SLM?
5. Beskriv en komplett implementeringsarbeidsflyt fra modellvalg til produksjonsimplementering.

### Praktiske Øvelser

1. **Grunnleggende Lokal Implementering**: Implementer en enkel SLM ved hjelp av Ollama (1 time)
...
- **Prioriterte Konsepter**: 
  ...
  - Implementering på bedriftsnivå
  - Automatiserte optimaliseringsarbeidsflyter

#### Seksjon 4: OpenVINO Verktøysett
- **Prioriterte Konsepter**: 
  - Intel maskinvareoptimalisering
  - Neural Network Compression Framework (NNCF)
  - Tverrplattform inferensimplementering
  - OpenVINO GenAI for LLM-implementering

#### Seksjon 5: Apple MLX Rammeverk
- **Prioriterte konsepter**: 
...
2. Forklar fordelene med GGUF-formatet for edge-distribusjon.
3. Hvordan forbedrer maskinvarebevisst optimalisering i Microsoft Olive distribusjonseffektiviteten?
4. Hva er de viktigste fordelene med OpenVINOs NNCF for modellkomprimering?
...
- Mestre destillasjons- og finjusteringsteknikker for edge-distribusjon
- Implementere produksjonsdistribusjonsstrategier med overvåking
...
#### Seksjon 2: Modelldestillasjon
- **Prioriterte konsepter**: 
  - Kunnskapsoverføringsteknikker
  - Implementering av to-trinns destillasjonsprosess
  - Destillasjonsarbeidsflyter i Azure ML
...
2. Forklar fordelene med modelldestillasjon for edge-distribusjon.
3. Hva er de viktigste hensynene for finjustering av SLM i ressursbegrensede miljøer?
4. Beskriv en komplett produksjonsdistribusjonspipeline for edge AI-applikasjoner.

### Praktiske øvelser

1. **Grunnleggende destillasjon**: Lag en mindre modell fra en større lærermodell (1 time)
...
- Bygge intelligente AI-agenter for edge-miljøer ved bruk av små språkmodeller
...
- **Prioriterte konsepter**: 
  ...
  - Designmønstre spesifikke for edge-agenter
...
- **Prioriterte konsepter**: 
  ...
  - Virkelige applikasjoner (nettautomatisering, databehandling, API-integrasjon)

### Selvrefleksjonsspørsmål

1. Hva er de viktigste arkitektoniske hensynene for edge AI-agenter?
...
- **Prioriterte konsepter**: 
  ...
  - Modellkatalog og oppdagelse for edge-distribusjon
...
- **Prioriterte konsepter**: 
  ...
  - Datamaskinvisjons-APIer for bildebehandling og OCR
  - Foundry Local CLI for lokal utvikling og testing

#### Seksjon 3: Plattformspesifikke implementeringer
- **Prioriterte konsepter**: 
  - NVIDIA Jetson Orin Nano-distribusjon (67 TOPS AI-ytelse)
  - Mobilapplikasjoner med .NET MAUI og ONNX Runtime GenAI
  - Azure EdgeAI-løsninger med hybrid arkitektur mellom sky og edge
...
3. Hva er fordelene med Windows AI Foundry for edge-utvikling?
4. Forklar rollen til NPU-optimalisering i moderne EdgeAI-applikasjoner.
5. Hvordan utnytter Phi Silica API NPU-maskinvare for ytelsesoptimalisering?
6. Sammenlign fordelene med lokal kontra skybasert distribusjon for personvernfølsomme applikasjoner.
...
## Modul 8: Microsoft Foundry Local – Komplett utviklerverktøysett

### Viktige læringsmål

- Installere og konfigurere Foundry Local på Windows
- Kjøre, oppdage og administrere modeller lokalt via Foundry CLI
- Integrere med OpenAI-kompatible REST- og SDK-klienter
- Bygge praktiske eksempler: Chainlit-chat, agenter og modellrouter
- Forstå hybride mønstre med Azure AI Foundry

### Fokusområder for studier

- Installasjon og CLI-grunnleggende (modell, tjeneste, cache)
- SDK-integrasjon (OpenAI-kompatible klienter og Azure OpenAI)
- Rask validering med Open WebUI
- Agent- og funksjonskallmønstre
- Modeller som verktøy (router- og registerdesign)

### Selvrefleksjonsspørsmål

1. Hvordan oppdager du den lokale endepunktet og lister tilgjengelige modeller?
2. Hva er forskjellene mellom Foundry Local REST og Azure OpenAI-bruk?
3. Hvordan ville du designe en enkel router for å velge modeller som verktøy?
4. Hvilke CLI-kategorier er mest relevante for daglig utvikling?
5. Hvordan validerer du Foundry Local-beredskap før du kjører apper?

### Praktiske øvelser

1. Installer/oppgrader Foundry Local og kjør `phi-4-mini` lokalt (30 minutter)
2. Kall `/v1/models` og kjør en enkel chat via REST (30 minutter)
3. Start Chainlit-app-eksempelet og chat lokalt (30 minutter)
4. Kjør multi-agent-koordinatoren og inspiser utdataene (30 minutter)
5. Prøv routeren for modeller som verktøy med miljøbaserte overstyringer (30 minutter)

## Tidsallokeringsveiledning

For å hjelpe deg med å få mest mulig ut av den 20-timers kurstidslinjen, her er en foreslått fordeling av tiden din:

| Aktivitet | Tidsallokering | Beskrivelse |
|----------|----------------|-------------|
| Lesing av kjerneinnhold | 9 timer | Fokus på de essensielle konseptene i hver modul |
...
| Mini-prosjekt | 3 timer | Anvendelse av kunnskap i en liten praktisk implementering |
...
- [OpenVINO NNCF](https://github.com/openvinotoolkit/nncf) - Rammeverk for nevralt nettverkskomprimering
...
| Modul 8: Foundry Local Verktøysett | | | |
| Praktiske øvelser | | | |
| Mini-prosjekt | | | |

## Mini-prosjektideer

Vurder å fullføre ett av disse prosjektene for å praktisere EdgeAI-konsepter (hver designet for å ta 2-4 timer):

### Nybegynnerprosjekter (2-3 timer hver)
1. **Edge Tekstassistent**: Lag et enkelt offline tekstfullføringsverktøy ved bruk av en liten språkmodell
2. **Modellsammenligningsdashboard**: Bygg en grunnleggende visualisering av ytelsesmetrikker på tvers av ulike SLM-er
3. **Optimaliseringseksperiment**: Mål effekten av ulike kvantiseringsnivåer på den samme basismodellen
...
5. **Windows AI Foundry-applikasjon**: Lag en Windows-app ved bruk av Phi Silica API og NPU-optimalisering
6. **Tverrplattformdistribusjon**: Distribuer den samme optimaliserte modellen på Windows (OpenVINO) og mobil (.NET MAUI)
...
8. **OpenVINO Optimaliseringspipeline**: Implementer fullstendig modelloptimalisering ved bruk av NNCF og GenAI-verktøysett  
9. **SLMOps-pipeline**: Implementer en komplett modelllivssyklus fra trening til distribusjon på edge  
10. **Multi-modell Edge-system**: Distribuer flere spesialiserte modeller som samarbeider på edge-maskinvare  
11. **MCP Integrasjonssystem**: Bygg et agentisk system ved bruk av Model Context Protocol for verktøysinteraksjon  
...
- Microsoft Learn (Foundry Local)  
  ...
  - Integrer med inferens-SDKer: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/how-to/how-to-integrate-with-inference-sdks  
...
EdgeAI representerer frontlinjen for implementering av kunstig intelligens, og bringer kraftige funksjoner direkte til enheter samtidig som det adresserer viktige bekymringer rundt personvern, forsinkelse og tilkobling. Dette 20-timers kurset gir deg den essensielle kunnskapen og praktiske ferdighetene du trenger for å begynne å jobbe med EdgeAI-teknologier umiddelbart.
