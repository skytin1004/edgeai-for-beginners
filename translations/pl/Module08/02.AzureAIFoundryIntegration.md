...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T12:32:30+00:00",
  "source_file": "Module08/02.AzureAIFoundryIntegration.md",
  "language_code": "pl"
}
-->
# Sesja 2: Integracja OpenAI SDK z Azure AI Foundry

## Przegląd

Bazując na podstawach Foundry Local, ta sesja koncentruje się na zaawansowanych wzorcach integracji OpenAI SDK, które płynnie wspierają zarówno Microsoft Foundry Local, jak i Azure OpenAI. Nauczysz się tworzyć elastyczne aplikacje AI, które mogą działać lokalnie dla zachowania prywatności i rozwoju, a jednocześnie oferować skalowalność w chmurze dzięki Azure OpenAI, gdy zajdzie taka potrzeba.

## Cele nauki

Po zakończeniu tej sesji będziesz:
- Opanować zaawansowaną integrację OpenAI SDK z Foundry Local i Azure OpenAI
- Implementować odpowiedzi strumieniowe dla lepszego doświadczenia użytkownika
- Tworzyć solidne wzorce fabryki klientów dla wsparcia wielu dostawców
- Budować systemy zarządzania rozmowami z zachowaniem kontekstu
- Ustanawiać benchmarki wydajności i monitorowanie stanu zdrowia
- Wdrażać aplikacje gotowe do produkcji z odpowiednim obsługiwaniem błędów

## Wymagania wstępne

- Ukończona Sesja 1: Wprowadzenie do Foundry Local
- Aktywna instalacja Foundry Local z działającymi modelami
- Python 3.8 lub nowszy z możliwością tworzenia wirtualnych środowisk
- Zainstalowany OpenAI Python SDK (`pip install openai foundry-local-sdk`)
- Konto Azure z usługą OpenAI (opcjonalnie, dla scenariuszy chmurowych)
- Podstawowa znajomość wzorców Python async/await

## Część 1: Wzorzec fabryki klienta OpenAI SDK

### Zrozumienie architektury wielodostawcy

Tworzenie aplikacji działających zarówno z Foundry Local, jak i Azure OpenAI wymaga elastycznego wzorca tworzenia klienta:
...
## Część 2: Odpowiedzi strumieniowe i interakcja w czasie rzeczywistym

### Implementacja strumieniowych uzupełnień rozmów

Strumieniowanie zapewnia lepsze doświadczenie użytkownika, pokazując odpowiedzi w miarę ich generowania:
...
### Zarządzanie rozmowami wieloetapowymi
...
## Część 3: Benchmarking wydajności i analiza

### Pomiar czasu odpowiedzi

Mierz i porównuj wydajność w różnych konfiguracjach:
...
### Testowanie temperatury i parametrów
...
## Część 4: Monitorowanie stanu zdrowia usługi i diagnostyka

### Kompleksowy system sprawdzania stanu zdrowia
...
### Walidator konfiguracji środowiska
...
## Część 5: Zmienne środowiskowe i zarządzanie konfiguracją

### Referencja zmiennych środowiskowych

Kompletny przewodnik konfiguracji dla obu dostawców:
...
**Konfiguracja w Windows Command Prompt:**
...
### Ćwiczenie 1: Integracja SDK dla wielu dostawców

Zbuduj kompletną aplikację, która płynnie przełącza się między dostawcami:
...
### Ćwiczenie 2: Zaawansowane zarządzanie rozmowami
...
### Ćwiczenie 3: Monitorowanie stanu zdrowia gotowe do produkcji
...
## Część 7: Podsumowanie i kolejne kroki

### Osiągnięcia sesji

W tej sesji opanowałeś:
- ✅ **Integracja OpenAI SDK**: Zaawansowane wzorce dla Foundry Local i Azure OpenAI
- ✅ **Odpowiedzi strumieniowe**: Uzupełnienia rozmów w czasie rzeczywistym dla lepszego doświadczenia użytkownika
- ✅ **Wsparcie dla wielu dostawców**: Płynne przełączanie między lokalnymi i chmurowymi usługami AI
- ✅ **Zarządzanie rozmowami**: Rozmowy wieloetapowe z zachowaniem kontekstu
- ✅ **Monitorowanie wydajności**: Benchmarking i sprawdzanie stanu zdrowia dla wdrożeń produkcyjnych
- ✅ **Wzorce produkcyjne**: Solidne obsługiwanie błędów i zarządzanie konfiguracją
...
**Przepływ odpowiedzi strumieniowych:**
```
...
1. **🔄 Zawsze implementuj mechanizmy awaryjne**: Azure → Foundry Local → Obsługa błędów
2. **🌊 Używaj strumieniowania dla długich odpowiedzi**: Lepsza percepcja wydajności
3. **🛡️ Implementuj kompleksową obsługę błędów**: Przyjazne dla użytkownika komunikaty o błędach
4. **📈 Monitoruj wydajność**: Śledź czasy odpowiedzi i wskaźniki sukcesu
5. **⚙️ Konfiguracja oparta na środowisku**: Łatwe przełączanie między dev/staging/prod
6. **🔒 Bezpieczne zarządzanie poświadczeniami**: Nigdy nie zapisuj kluczy API w kodzie

### Wytyczne dotyczące wyboru dostawcy

| Scenariusz | Zalecany dostawca | Uzasadnienie |
|------------|-------------------|--------------|
| **Rozwój** | Foundry Local | Szybka iteracja, brak kosztów API |
| **Krytyczne dla prywatności** | Foundry Local | Dane nigdy nie opuszczają urządzenia |
| **Produkcja na dużą skalę** | Azure OpenAI | Lepsza skalowalność, SLA dla przedsiębiorstw |
| **Najnowsze modele** | Azure OpenAI | Dostęp do najnowszych wersji modeli |
| **Wymagania offline** | Foundry Local | Brak zależności od internetu |
| **Ograniczenia kosztowe** | Foundry Local | Brak opłat za tokeny |

### Szybka referencja zmiennych środowiskowych
...
### Przygotowanie do Sesji 3: Modele open-source

1. **Przegląd katalogu modeli**: Zapoznaj się z dostępnymi modelami w Foundry Local
2. **Zrozumienie formatów modeli**: Dowiedz się więcej o ONNX, kwantyzacji i optymalizacji
3. **Rozważ modele niestandardowe**: Pomyśl o wymaganiach dla modeli specyficznych dla domeny
...
- **[Dokumentacja OpenAI Python SDK](https://github.com/openai/openai-python)**: Oficjalna dokumentacja SDK
- **[Dokumentacja Azure OpenAI](https://learn.microsoft.com/azure/ai-services/openai/)**: Przewodnik po usłudze Azure OpenAI
- **[Referencja Foundry Local SDK](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**: Dokumentacja lokalnej inferencji
- **[Przewodnik po strumieniowych uzupełnieniach](https://learn.microsoft.com/azure/ai-foundry/foundry-local/how-to/integrate-with-inference-sdks)**: Zaawansowane wzorce strumieniowania
- **[Przykład 01: Szybki czat za pomocą OpenAI SDK](samples/01/README.md)**: Podstawowe wzorce integracji
- **[Przykład 02: Zaawansowana integracja SDK](samples/02/README.md)**: Ćwiczenia praktyczne z tej sesji
- **[Przykład 04: Aplikacja Chainlit](samples/04/README.md)**: Tworzenie interfejsu webowego
- **[Przykład 05: Systemy wieloagentowe](samples/05/README.md)**: Zaawansowane wzorce orkiestracji

Jesteś teraz wyposażony w wiedzę, aby budować zaawansowane aplikacje AI, które płynnie integrują lokalne i chmurowe możliwości AI, zapewniając elastyczność w wyborze odpowiedniego dostawcy dla każdego konkretnego przypadku użycia, przy jednoczesnym zachowaniu spójnych wzorców rozwoju.
