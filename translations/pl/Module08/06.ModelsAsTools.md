...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T13:38:07+00:00",
...
Traktuj modele AI jako modułowe, dostosowywalne narzędzia działające bezpośrednio na urządzeniu dzięki Foundry Local. Ta sesja kładzie nacisk na praktyczne przepływy pracy związane z prywatnością, inferencją o niskim opóźnieniu oraz integracją tych narzędzi za pomocą SDK, API lub CLI. Dowiesz się również, jak skalować do Azure AI Foundry, gdy zajdzie taka potrzeba.
...
## Część 1: Abstrakcje narzędzi (krok po kroku)

Cel: Reprezentowanie modeli jako narzędzi z jasnymi kontraktami i prostym routerem.

Krok 1) Zdefiniuj interfejs narzędzia i rejestr
```python
# tools/registry.py
...
Krok 2) Zaimplementuj dwa narzędzia oparte na Foundry Local
```python
# tools/impl.py
import requests, os
...
## Część 2: Integracja SDK i API (krok po kroku)

Cel: Użycie OpenAI Python SDK z punktem końcowym Foundry Local.
...
Cel: Dopasowanie wyników do dziedziny za pomocą szablonów promptów i schematów JSON.

Krok 1) Utwórz szablon promptu dla dziedziny
```python
# domain/templates.py
...
## Część 5: Skalowanie do Azure AI Foundry (krok po kroku)

Cel: Odzwierciedlenie lokalnych modeli za pomocą punktów końcowych Azure dla dodatkowej pojemności.

Krok 1) Decyzja o strategii routingu
- Najpierw lokalnie dla prywatności/opóźnienia, Azure jako zapas w przypadku błędów lub dużych promptów

Krok 2) Implementacja prostego routera
```python
# hybrid/router.py
import os, requests
...
- [ ] Zarejestruj co najmniej dwa narzędzia i kieruj żądania
- [ ] Wywołaj Foundry Local za pomocą OpenAI SDK i surowego REST
- [ ] Wymuś wyjścia JSON dla szablonu dziedziny
- [ ] Oczyść i loguj wywołania lokalnie
- [ ] Zaimplementuj prosty hybrydowy router z zapasem Azure

## Podsumowanie

Foundry Local umożliwia solidne AI na urządzeniu, gdzie modele stają się narzędziami do komponowania. Dzięki jasnym interfejsom, zarządzaniu i hybrydowemu skalowaniu zespoły mogą tworzyć aplikacje AI w czasie rzeczywistym, które szanują prywatność użytkowników, jednocześnie pozostając gotowe do zastosowań w przedsiębiorstwach.
