...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-25T00:35:38+00:00",
...
Note: Agent capabilities in Foundry Local are constantly evolving—check the latest release notes to confirm support for advanced patterns before implementation.

## Overview

Foundry Local enables rapid prototyping of agent-based applications, including system prompts, grounding, and orchestration patterns. When agent support is available, you can standardize on OpenAI-compatible function calling or integrate Azure AI Agents for hybrid cloud designs.

> **🔄 Updated for Modern SDK**: This module has been updated to align with the latest patterns in the Microsoft Foundry-Local repository and reflects the full implementation in `samples/05/`. The examples now utilize the modern `foundry-local-sdk` and OpenAI client instead of manual API requests.

**🏗️ Architecture Highlights:**
- **Specialist Agents**: Retrieval, Reasoning, and Execution agents with distinct roles
- **Coordinator Pattern**: Manages workflows across multiple agents with feedback loops
- **Modern SDK Integration**: Leverages `FoundryLocalManager` and OpenAI client
- **Production Ready**: Includes error handling, performance monitoring, and health checks
- **Comprehensive Examples**: Interactive Jupyter notebook showcasing advanced features
...
## Learning Objectives
- Design system prompts and grounding strategies for consistent behavior
- Implement function calling (tool use) patterns
- Orchestrate workflows across multiple agents (local and hybrid setups)
- Plan for observability and safety measures

## Part 1: System Prompts and Grounding

- Define clear roles, constraints, and output formats
- Ground responses using local or enterprise data sources
...
Design a coordinator to route tasks to specialist agents (retrieval, reasoning, execution) using Foundry Local’s OpenAI-compatible endpoint.

Step 1) Define specialist agents using the modern SDK (refer to `samples/05/agents/specialists.py`)
```python
...
Step 3) Validate against Foundry Local and run the provided samples
```powershell
...
Guidelines:
- Implement retries and timeouts between agent calls
- Add a lightweight in-memory store (e.g., a dictionary) for conversation or thread state
- Introduce rate-limiting when chaining multiple calls

## Part 4: Observability and Safety

Monitor prompts, responses, and errors locally, while ensuring data hygiene in your agent stack.
...
- Before sending messages to the model, remove or hash sensitive fields (e.g., emails, phone numbers, IDs)
- Keep raw source data on-device and only pass necessary context strings
...
- Wrap each agent call with try/except blocks and implement exponential backoff
- Halt the pipeline on repeated failures
...
- Store JSON logs in `./agent_logs`
- Periodically compress and rotate logs
- Export summaries for review (e.g., counts, average latency, error rates)

Step 6) Cross-check with Microsoft Learn documentation
- Foundry Local provides an OpenAI-compatible API (validated with `curl /v1/models`)
- Use `foundry model run <name>` to confirm model availability
- Follow official guidance for client integration and sample applications (Open WebUI/how-tos)
