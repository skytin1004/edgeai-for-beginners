...
If the last command fails, start (or restart) the service: `foundry service start`.

## Demo Flow (30 minutes)

### 1. System Prompts and Grounding Strategies (10 minutes)

#### Step 1.1: Advanced Prompt Engineering

Create `samples/02-rag-solutions/prompt_engineering.py`:
...
Create `samples/02-rag-solutions/csv_qa_system.py`:
...
#### Step 3.1: Enhanced Document RAG System

Create `samples/02-rag-solutions/document_rag.py`:
...
Create `samples/02-rag-solutions/migration_guide.py`:
...
For disk persistence use `faiss.write_index(index, "kb.index")`.
