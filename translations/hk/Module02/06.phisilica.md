...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-22T05:53:51+00:00",
...
Phi-Silica 是 Microsoft 專門針對裝置端語言模型的解決方案，展現了在 Windows 11 內建部署上的卓越效率。這款專門設計的模型在消費者硬件上提供了出色的性能，同時保持最小的資源佔用，樹立了裝置端 AI 的新標準。

## 開發者資源

### 相關 Phi 模型
構成 Phi-Silica 基礎的 Phi 模型可透過 [Azure AI Foundry Model Catalog](https://ai.azure.com/explore/models?q=phi) 獲取，為開發者提供適用於各種部署場景的高效模型。
...
在本課程中，我們將探討 Microsoft 的 Phi-Silica 模型及其基本概念。我們將涵蓋使其在裝置端表現卓越的專門架構、使 Phi-Silica 高效運行的優化技術，以及專為 Windows Copilot+ PCs 設計的實際應用。
...
- 確認使 Phi-Silica 能在 Windows 11 Copilot+ PCs 上高效運行的關鍵優化。
- 認識 Phi-Silica 的性能特徵和能力。
- 將 Phi-Silica 的知識應用於 Windows 11 AI 開發場景。

## 理解專門的 NPU 硬件

Windows 11 Copilot+ PCs 引入了專門為 AI 工作負載設計的神經處理單元 (NPUs)。這些專門的硬件元件在機器學習任務中相較於傳統的 CPU 和 GPU 提供了顯著的效率優勢，使得裝置端 AI 能以最低的功耗實現強大的功能。

傳統的裝置端 AI 方法通常需要在能力和效率之間妥協。然而，透過專門的 NPU 硬件以及像 Phi-Silica 這樣高度優化的模型，Windows 設備現在能以前所未有的效率提供有意義的 AI 功能。

## Phi-Silica 的性能革命

Phi-Silica 達到了重新定義裝置端語言模型可能性的卓越性能指標：
...
Phi-Silica 建立在 Microsoft 對小型高效語言模型的研究基礎上，並針對 NPU 硬件進行了專門的優化：

- **張量操作優化**：重構操作以最大化 NPU 的吞吐量。
- **記憶體訪問模式**：精心設計以減少數據移動。
- **激活函數選擇**：針對高效 NPU 執行進行優化。
...
Phi-Silica 與 Windows 11 生態系統深度整合：

### Windows Copilot

Phi-Silica 支持 Windows 11 Copilot+ PCs 的部分功能，通過本地處理提升性能和隱私。
...
Phi-Silica 的 1.5 瓦功耗使 AI 功能不會對電池造成顯著影響，讓 AI 功能全天候使用成為可能。
...
Phi-Silica 能生成連貫且符合上下文的文本，用於各種應用：
...
Phi-Silica 基於 Phi 模型家族，並進行了專門的優化：

- **架構**：基於 Transformer，進行了效率優化的修改。
- **大小**：緊湊的參數數量，專為 NPU 執行而設計。
- **功能**：文本生成、理解、摘要以及任務導向的助手功能。
...
作為裝置端模型，Phi-Silica 擁有固定的知識庫，無法即時訪問互聯網以獲取訓練數據之外的事實性查詢。

### 任務複雜性

雖然功能強大，但 Phi-Silica 在處理極其複雜的任務時可能存在限制，這些任務甚至可能挑戰更大的模型。

### 硬件需求

最佳性能需要配備專門為這些工作負載設計的 NPU 硬件的 Windows Copilot+ PC。
...
Phi-Silica 達到了卓越的效率指標：

- **生成速度**：在 NPU 硬件上每秒 650 個 Token。
- **功耗**：運行期間僅需 1.5 瓦。
- **延遲**：啟動回應時間低於 10 毫秒。
- **吞吐量**：同時處理多個查詢時性能降幅極小。
