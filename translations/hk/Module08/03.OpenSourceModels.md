...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T09:52:51+00:00",
...
本節課程專注於使用 Foundry Local 進行模型探索與管理的實踐操作。你將學習如何列出可用模型、測試不同選項，以及了解基本性能特徵。課程重點在於通過 Foundry CLI 的實際操作，幫助你選擇適合自己使用場景的模型。
...
1. **從小型模型開始**：使用 `phi-4-mini` 驗證設置
2. **一次運行一個模型**：在啟動新模型前停止之前的模型
3. **監控資源**：密切關注內存使用情況
4. **一致性測試**：使用相同的提示進行公平比較
5. **記錄結果**：記錄模型性能以便於使用場景的選擇
...
你現在已具備使用 Foundry Local 的簡單 CLI 方法來探索、測試和選擇適合 AI 應用的模型的實用技能。
...
Foundry CLI 提供了簡單的指令來進行模型探索與管理：
...
**中型模型：**
- `qwen2.5-7b-instruct`：卓越的推理能力和更長的上下文
...
**複雜推理：**
- 最佳選擇：`qwen2.5-7b-instruct` 或 `qwen2.5-14b-instruct`
- 經濟選擇：`phi-4`
