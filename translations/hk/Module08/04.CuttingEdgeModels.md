...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T11:47:38+00:00",
  "source_file": "Module08/04.CuttingEdgeModels.md",
  "language_code": "hk"
}
-->
# 第四節：前沿模型 – LLMs、SLMs 和設備端推理

## 概述

比較 LLMs 和 SLMs，評估本地與雲端推理的取捨，並實現展示 EdgeAI 場景的演示，使用 Phi 和 ONNX Runtime。我們還會重點介紹 Chainlit RAG、WebGPU 推理選項以及 Open WebUI 的整合。
...
考慮因素：
- 延遲：SLMs 在設備端通常能提供毫秒級的響應
- 成本：本地推理降低雲端成本
- 隱私：敏感數據保留在設備端
- 能力：LLMs 在處理複雜任務時可能優於 SLMs
- 可靠性：混合策略降低停機風險

## 第二部分：本地 vs 雲端 – 混合模式

- 本地優先，雲端作為備選，用於處理大型或複雜的提示
- 雲端優先，本地用於隱私敏感或離線場景
- 根據任務類型進行路由（例如，代碼生成使用 DeepSeek，普通聊天使用 Phi/Qwen）

## 第三部分：使用 Chainlit 的 RAG 聊天應用（簡化版）
...
使用 WebGPU 在瀏覽器中直接運行小型模型。這非常適合隱私優先的演示和零安裝體驗。以下是使用 ONNX Runtime Web 和 WebGPU 執行提供程序的逐步示例。

1) 檢查 WebGPU 支持
- Chromium 瀏覽器：chrome://gpu → 確認“WebGPU”已啟用
- 程式化檢查（我們也會在代碼中檢查）：`if (!('gpu' in navigator)) { /* no WebGPU */ }`
...
4) 故障排除
- 如果 WebGPU 不可用：更新 Chrome/Edge 並確保 GPU 驅動程序是最新的，然後檢查 chrome://flags 中的“Enable WebGPU”。
- 如果出現 CORS 或 fetch 錯誤：確保文件通過 http://（而不是 file://）提供，並且模型 URL 允許跨域請求。
...
```
注意：
- 在 Windows 上，`host.docker.internal` 讓容器能夠訪問主機的 `localhost`。
- 我們將 `OPENAI_API_BASE_URL` 設置為 Foundry Local 的端點，並使用一個虛擬的 `OPENAI_API_KEY`。

4) 從 Open WebUI 界面進行配置（替代方法）
- 瀏覽到 http://localhost:3000
- 完成初始設置（管理員用戶）
- 前往設置 → 模型/提供者
- 設置 Base URL：`http://host.docker.internal:8000/v1`
- 設置 API Key：`local-key`（佔位符）
- 保存
...
- [ ] 使用至少兩個模型運行 Chainlit 演示
- [ ] 將 Open WebUI 連接到你的本地端點並進行測試
