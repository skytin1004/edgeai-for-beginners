...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T09:54:28+00:00",
...
本節課程專注於使用 Chainlit 和 Microsoft Foundry Local 建立生產級聊天應用程式。你將學習如何為 AI 對話創建現代化的網頁介面、實現流式回應，以及部署具有完善錯誤處理和用戶體驗設計的穩健聊天應用程式。
...
## 第四部分：創建和使用 Jupyter Notebook

### Notebook 支援概覽

範例 04 包含一個全面的 Jupyter Notebook (`chainlit_app.ipynb`)，提供：
...
WebGPU 允許直接在瀏覽器中運行 AI 模型，最大化隱私並提供零安裝體驗。本範例展示了使用 ONNX Runtime Web 和 WebGPU 執行的應用。
...
第四節課程涵蓋了建立生產級 Chainlit 應用程式以支持對話式 AI。你學習了：
...
- **[Chainlit 教學 Notebook](samples/04/chainlit_app.ipynb)**：互動學習材料
