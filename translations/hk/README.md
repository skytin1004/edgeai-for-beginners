...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-17T10:28:33+00:00",
...
歡迎來到 **EdgeAI 初學者指南**——這是一個全面的旅程，帶您進入邊緣人工智能的變革性世界。本課程將強大的 AI 能力與實際的邊緣設備部署相結合，幫助您直接在數據生成和決策需要的地方發揮 AI 的潛力。
...
邊緣 AI 是指在硬件上本地運行 AI 算法和語言模型——靠近數據生成的地方——而不依賴雲資源進行推理。它減少了延遲，增強了隱私，並支持實時決策。
...
SLMs 如 Phi-4、Mistral-7B 和 Gemma 是更大 LLM 的優化版本——通過訓練或蒸餾實現：
...
- [**第3章：微調 - 為特定任務自訂模型**](./Module05/03.SLMOps-Finetuing.md)
  - 參數高效微調（PEFT）技術
  - LoRA 及 QLoRA 高級方法
...
**主題**：SLM Agentic 系統的實施，從基礎到高級函數調用及模型上下文協議整合

#### 章節結構：
- [**第1章：AI 代理及小型語言模型基礎**](./Module06/01.IntroduceAgent.md)
  - 代理分類框架（反射型、基於模型、基於目標、學習型代理）
  - SLM 基礎及優化策略（GGUF、量化、邊緣框架）
  - SLM 與 LLM 的權衡分析（成本減少10-30倍，任務效能達70-80%）
  - 使用 Ollama、VLLM 及 Microsoft 邊緣解決方案進行實際部署
...
- [**使用 .NET MAUI 和 ONNX Runtime GenAI 的移動應用 EdgeAI**](./Module07/README.md#2-edgeai-in-mobile-applications-with-net-maui-and-onnx-runtime-genai)
  - 使用單一 C# 程式碼庫的跨平台移動 AI
  - 硬件加速支援（CPU、GPU、移動 AI 處理器）
...
- [**使用 Windows ML 的 EdgeAI**](./Module07/README.md#4-edgeai-with-windows-ml)
  ...
  - 通用硬件支援（AMD、Intel、NVIDIA、Qualcomm 矽片）
  - 自動硬件抽象及優化
...
- 精通使用 OpenVINO 及 NNCF 進行 Intel 硬件優化
- 在多樣化平台中選擇硬件感知優化的能力
- 跨平台邊緣計算環境的生產部署技能
...
- 精通 NVIDIA、移動、Azure 及 Windows 平台的硬件特定優化技術
- 理解性能、成本及隱私要求之間的部署權衡
...
- 為雲端-邊緣混合架構做好準備
- 成為新興 AI 技術及框架的入門途徑
這門課程將你置於人工智能技術部署的最前沿，智能功能無縫整合到驅動現代生活的設備和系統中。
...
4. 繼續 **Module04**，進行高級模型優化、格式轉換和框架整合
5. 完成 **Module05**，掌握 SLMOps，實現生產級應用
6. 探索 **Module06**，了解 SLM 智能代理系統及功能調用能力
7. 最後完成 **Module07**，獲得 AI 工具包和多樣化 EdgeAI 實施範例的實踐經驗

每個模組都設計為獨立完整，但按順序學習能取得最佳效果。

## 學習指南

一份全面的 [學習指南](STUDY_GUIDE.md) 可幫助你最大化學習體驗。學習指南提供：

- **結構化學習路徑**：優化的課程完成時間表，約 20 小時
- **時間分配指導**：具體建議如何平衡閱讀、練習和項目
- **重點概念聚焦**：每個模組的優先學習目標
- **自我評估工具**：問題和練習測試你的理解
- **迷你項目點子**：實際應用以加強學習

學習指南適合密集學習（一週）和兼職學習（三週），即使你只能投入 10 小時，也有清晰的時間分配指導。

---

**EdgeAI 的未來在於持續改進模型架構、量化技術和部署策略，優先考慮效率和專業化，而非通用能力。採納這種範式轉變的組織將能充分利用人工智能的變革潛力，同時保持對數據和運營的控制。**
