...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T11:39:31+00:00",
...
歡迎來到 Windows Edge AI 開發指南——這是一份全面的指南，幫助您使用 Microsoft 的 Windows AI Foundry 平台構建智能應用程式，充分利用設備上的 AI 技術。本指南專為希望將尖端的 Edge AI 功能整合到應用程式中的 Windows 開發者設計，同時充分利用 Windows 硬件加速的全部潛力。
...
**通用硬件支持**  
Windows ML 提供自動硬件優化，適用於整個 Windows 生態系統，確保您的 AI 應用程式無論底層芯片架構如何都能最佳運行。
...
Windows AI API 提供即用型 AI 功能，由設備上的模型驅動，針對 Copilot+ PC 設備進行了效率和性能優化，設置需求極低。
...
**通用硬件支持**  
- 自動優化適用於 AMD、Intel、NVIDIA 和 Qualcomm 的芯片  
...
**核心集成**  
- 實施 Windows AI API 集成，並進行正確的錯誤處理  
- 設計適應 AI 處理工作流的用戶界面  
- 為模型推理實施緩存和優化策略  
- 添加 AI 操作性能的遙測和監控  
...
Windows Edge AI 開發代表了強大 AI 功能與穩健、安全且可擴展的 Windows 平台的融合。通過掌握 Windows AI Foundry 生態系統，開發者可以創建智能應用程式，提供卓越的用戶體驗，同時保持最高的隱私、安全性和性能標準。  

Windows AI API、Foundry Local 和 Windows ML 的結合，為構建下一代智能 Windows 應用程式提供了無與倫比的基礎。隨著 AI 的不斷發展，Windows 平台確保您的應用程式能夠隨著新興技術的發展而擴展，同時在多樣化的 Windows 硬件生態系統中保持兼容性和性能。  

無論您是在構建消費者應用程式、企業解決方案，還是專業行業工具，Windows Edge AI 開發都能幫助您創建智能、響應迅速且深度整合的體驗，充分發揮現代 Windows 設備的潛力。  

## 附加資源  

如需 Foundry Local 的逐步 Windows 指南（安裝、CLI、動態端點、SDK 使用），請參閱倉庫指南：[foundrylocal.md](./foundrylocal.md)。  
...
- [AI 開發畫廊](https://learn.microsoft.com/windows/ai/ai-dev-gallery/)  
