...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T11:39:00+00:00",
...
Edge AI（邊緣人工智能）結合了人工智能與邊緣計算，實現了在設備上直接進行智能處理，而無需依賴雲端連接。本章將探討五種不同平台和框架上的 EdgeAI 實現，展示在邊緣運行 AI 模型的多樣性與強大功能。

## 1. NVIDIA Jetson Orin Nano 的 EdgeAI

NVIDIA Jetson Orin Nano 是邊緣 AI 計算的一次突破，提供高達 67 TOPS 的 AI 性能，並採用緊湊的信用卡大小設計。這款強大的邊緣 AI 平台使生成式 AI 的開發變得更加普及，適合愛好者、學生和專業開發者。

### 主要特點
- 提供高達 67 TOPS 的 AI 性能，比前代產品提升 1.7 倍
- 配備 1024 個 CUDA 核心和最多 32 個 Tensor 核心，用於 AI 處理
- 6 核 Arm Cortex-A78AE v8.2 64 位 CPU，最高頻率達 1.5 GHz
- 價格僅 $249，為開發者、學生和創客提供最實惠且易於使用的平台

### 應用場景
Jetson Orin Nano 擅長運行現代生成式 AI 模型，包括視覺變壓器、大型語言模型和視覺-語言模型。它專為生成式 AI（GenAI）用例設計，現在可以在掌上設備上運行多種 LLM。常見應用包括 AI 驅動的機器人、智能無人機、智能攝像頭和自主邊緣設備。
...
此解決方案展示了如何使用 .NET MAUI（多平台應用程序 UI）和 ONNX Runtime GenAI，將生成式 AI 和大型語言模型（LLMs）集成到跨平台移動應用中。這種方法使 .NET 開發者能夠構建在 Android 和 iOS 設備上原生運行的高級 AI 驅動移動應用。
...
微軟基於 Azure 的 EdgeAI 解決方案專注於在雲-邊緣混合環境中高效部署小型語言模型（SLMs）。這種方法彌合了雲規模 AI 服務與邊緣部署需求之間的差距。
...
### 使用場景
Azure EdgeAI 實現非常適合需要企業級 AI 部署和雲管理功能的場景，包括智能文檔處理、實時分析和利用雲與邊緣計算資源的混合 AI 工作流。
...
Windows ML 是微軟針對設備上模型推理和簡化部署進行優化的最新運行時，並作為 Windows AI Foundry 的基礎。該平台使開發者能夠創建利用 PC 硬件全能力的 AI 驅動 Windows 應用。

### 平台功能
- 適用於所有運行 Windows 11 版本 24H2（構建 26100）或更高版本的 PC
- 支持所有 x64 和 ARM64 PC 硬件，即使是沒有 NPU 或 GPU 的 PC
- 支持開發者自帶模型，並高效部署到包括 AMD、Intel、NVIDIA 和 Qualcomm 在內的矽合作夥伴生態系統
- 通過基礎設施 API，開發者無需為不同矽目標創建多個應用構建版本
...
Foundry Local 使開發者能夠使用本地資源在 .NET 中構建檢索增強生成（RAG）應用，結合本地語言模型與語義搜索功能。這種方法提供了完全基於本地基礎設施運行的隱私保護 AI 解決方案。

### 技術架構
- 結合 Phi 語言模型、本地嵌入和語義內核，創建 RAG 場景
- 使用嵌入作為浮點值數組（向量），表示內容及其語義含義
- 語義內核作為主要協調器，集成 Phi 和智能組件，創建無縫的 RAG 管道
- 支持包括 SQLite 和 Qdrant 在內的本地向量數據庫

### 實現優勢
RAG（檢索增強生成）本質上是“查找一些內容並將其放入提示中”的高級方式。這種本地實現確保了數據隱私，同時提供基於自定義知識庫的智能響應。該方法對於需要數據主權和離線操作能力的企業場景特別有價值。
...
- 本倉庫中的完整 Windows 指南：[foundrylocal.md](./foundrylocal.md)
...
Windows AI Foundry 平台提供了一套專門為 Windows 設備上的 Edge AI 開發設計的全面工具和 API，包括對 NPU 加速硬件的專門支持、Windows ML 集成以及平台特定的優化技術。
...
### Edge AI 開發的 AI 工具包
對於使用 Visual Studio Code 的開發者，AI 工具包擴展提供了一個專門設計的開發環境，用於構建、測試和部署 Edge AI 應用。該工具包簡化了整個 Edge AI 開發工作流程。
