...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T23:03:10+00:00",
...
Эта сессия посвящена практическому изучению обнаружения и управления моделями с помощью Foundry Local. Вы научитесь перечислять доступные модели, тестировать различные варианты и понимать основные характеристики производительности. Подход акцентирует внимание на практическом изучении с использованием foundry CLI, чтобы помочь вам выбрать подходящие модели для ваших задач.

## Цели обучения

- Освоить команды foundry CLI для обнаружения и управления моделями
- Понять, как работает кэширование моделей и локальное хранение
- Научиться быстро тестировать и сравнивать различные модели
- Установить практические рабочие процессы для выбора и тестирования моделей
- Изучить растущую экосистему моделей, доступных через Foundry Local

## Предварительные требования

- Завершение Сессии 1: Начало работы с Foundry Local
- Установленный и доступный Foundry Local CLI
- Достаточно места для загрузки моделей (размер моделей может варьироваться от 1 ГБ до 20 ГБ+)
- Базовое понимание типов моделей и их применения

## Обзор

Эта сессия исследует, как интегрировать модели с открытым исходным кодом в Foundry Local.
...
1. **Запустите скрипт Примера 03**: `samples\03\list_and_bench.cmd`
2. **Попробуйте разные модели**: Протестируйте как минимум 3 разные модели
3. **Сравните производительность**: Обратите внимание на различия в скорости и качестве ответов
4. **Документируйте результаты**: Создайте простую таблицу сравнения
...
3. **Контролируйте ресурсы**: Следите за использованием памяти
4. **Тестируйте последовательно**: Используйте одинаковые запросы для справедливого сравнения
5. **Документируйте результаты**: Ведите заметки о производительности моделей для ваших задач
...
- **Предварительные требования**: Уверенное владение переключением моделей и базовым тестированием производительности
...
Теперь у вас есть практические навыки для обнаружения, тестирования и выбора подходящих моделей для ваших AI-приложений с использованием простого подхода CLI Foundry Local: выбор моделей сообщества, интеграция контента Hugging Face и использование стратегии "принеси свою модель" (BYOM). Вы также узнаете о серии Model Mondays для непрерывного обучения и обнаружения моделей.
...
- Управлять моделями локально с помощью кэша и версионирования
...
**Средние модели:**
- `qwen2.5-7b`: Отличное рассуждение и более длинный контекст
- `deepseek-r1-7b`: Оптимизирована для генерации кода

**Крупные модели:**
- `llama-3.2`: Последняя открытая модель от Meta
- `qwen2.5-14b`: Модель корпоративного уровня для сложных рассуждений
...
### Альтернатива тестированию в PowerShell
...
- Следите за использованием памяти (модели остаются загруженными в ОЗУ)
- Мониторьте загрузку процессора во время выполнения запросов
- Проверьте ввод-вывод диска при первоначальной загрузке модели
...
### Выбор моделей в зависимости от задачи

**Для общего общения и вопросов-ответов:**
- Начните с: `phi-4-mini` (быстрая, эффективная)
- Перейдите на: `phi-4` (лучшее рассуждение)
- Продвинутый вариант: `qwen2.5-7b` (длинный контекст)

**Для генерации кода:**
- Рекомендуется: `deepseek-r1-7b`
- Альтернатива: `qwen2.5-7b` (также подходит для кода)

**Для сложных рассуждений:**
- Лучший выбор: `qwen2.5-7b` или `qwen2.5-14b`
...
**Критерии оценки:**
- **Точность**: Насколько информация правильна?
- **Ясность**: Насколько легко понять объяснение?
- **Полнота**: Насколько полно ответ охватывает вопрос?
- **Скорость**: Как быстро поступает ответ?
...
- Внесите свои результаты в командный файл `models.json`
