...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-01T01:03:52+00:00",
...
- Na-install at naa-access ang Foundry Local CLI
- Sapat na storage space para sa pag-download ng modelo (ang mga modelo ay maaaring nasa pagitan ng 1GB hanggang 20GB+)
- Pangunahing kaalaman sa mga uri ng modelo at mga layunin nito
...
3. **Ihambing ang performance**: Pansinin ang mga pagkakaiba sa bilis at kalidad ng tugon
4. **Idokumento ang mga natuklasan**: Gumawa ng simpleng chart ng paghahambing
...
- Mag-upgrade ng RAM kung madalas na umaabot sa limitasyon
...
1. **Magsimula sa Maliit**: Magsimula sa `phi-4-mini` upang ma-validate ang setup
2. **Isang Modelo sa Bawat Oras**: Itigil ang nakaraang modelo bago magsimula ng bago
...
5. **Idokumento ang Resulta**: Magtala ng performance ng modelo para sa iyong mga layunin
...
- **Mga Kinakailangan**: Kumportable sa pagpapalit ng modelo at pangunahing pagsusuri ng performance
...
✅ **Pag-monitor ng Performance**: Pangunahing pagsukat ng paggamit ng resources at oras ng tugon  
✅ **Pagpili ng Modelo**: Praktikal na gabay para sa pagpili ng mga modelo batay sa layunin  
✅ **Pamamahala ng Cache**: Pag-unawa sa storage at mga proseso ng paglilinis  

Ngayon ay mayroon ka nang praktikal na kasanayan upang magdiskubre, mag-test, at pumili ng angkop na mga modelo para sa iyong AI application gamit ang simpleng CLI approach ng Foundry Local.
...
Magsimula sa mga sikat at subok na modelo upang maunawaan ang mga katangian ng performance:
...
- `phi-4-mini`: Mabilis, mahusay, mahusay para sa general chat
- `phi-4`: Mas kapable na bersyon na may mas mahusay na reasoning

**Medium Models:**
- `qwen2.5-7b`: Mahusay sa reasoning at mas mahabang context
- `deepseek-r1-7b`: Na-optimize para sa code generation

**Larger Models:**
- `llama-3.2`: Pinakabagong open-source na modelo ng Meta
- `qwen2.5-14b`: Enterprise-grade na reasoning
...
### Alternatibo sa PowerShell Testing
...
- Panatilihin ang 2-3 modelo na naka-cache para sa mabilisang pagpapalit
- Alisin ang mga hindi ginagamit na modelo upang magbakante ng espasyo: `foundry cache clean`
- I-monitor ang disk usage, lalo na sa mas maliit na SSD
...
### Pagpili ng Modelo Batay sa Layunin

**Para sa General Chat at Q&A:**
- Magsimula sa: `phi-4-mini` (mabilis, mahusay)
- Mag-upgrade sa: `phi-4` (mas mahusay na reasoning)
- Advanced: `qwen2.5-7b` (mas mahabang context)

**Para sa Code Generation:**
- Inirerekomenda: `deepseek-r1-7b`
- Alternatibo: `qwen2.5-7b` (mahusay din para sa code)

**Para sa Complex Reasoning:**
- Pinakamahusay: `qwen2.5-7b` o `qwen2.5-14b`
- Budget option: `phi-4`
...
### Workflow sa Pagpapalit ng Modelo
...
Narito ang isang simpleng approach para ikumpara ang performance ng modelo:
...
- I-contribute ang mga natuklasan sa `models.json` ng iyong team
