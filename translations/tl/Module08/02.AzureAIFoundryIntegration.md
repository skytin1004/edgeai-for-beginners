...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-25T00:54:45+00:00",
  "source_file": "Module08/02.AzureAIFoundryIntegration.md",
  "language_code": "tl"
}
-->
# Session 2: OpenAI SDK Integration with Azure AI Foundry

## Overview

Sa pagpapatuloy ng iyong Foundry Local foundation, ang sesyon na ito ay nakatuon sa mga advanced na pattern ng integrasyon ng OpenAI SDK na sumusuporta sa parehong Microsoft Foundry Local at Azure OpenAI. Matututunan mo kung paano bumuo ng mga flexible na AI application na maaaring tumakbo nang lokal para sa privacy at development, habang nagbibigay ng scalability sa cloud gamit ang Azure OpenAI kapag kinakailangan.

## Learning Objectives

Sa pagtatapos ng sesyon na ito, ikaw ay:
- Magiging bihasa sa advanced na integrasyon ng OpenAI SDK sa parehong Foundry Local at Azure OpenAI
- Magpapatupad ng streaming responses para sa mas pinahusay na karanasan ng user
- Lilikha ng matibay na client factory patterns para sa suporta sa multi-provider
- Bubuo ng mga sistema ng pamamahala ng pag-uusap na may context preservation
- Magtatatag ng performance benchmarking at health monitoring
- Magde-deploy ng production-ready applications na may tamang error handling

## Prerequisites

- Natapos ang Session 1: Getting Started with Foundry Local
- Aktibong Foundry Local installation na may mga tumatakbong modelo
- Python 3.8 o mas bago na may kakayahan sa virtual environment
- Nakainstall ang OpenAI Python SDK (`pip install openai foundry-local-sdk`)
- Azure account na may OpenAI service (opsyonal, para sa cloud scenarios)
- Pangunahing kaalaman sa Python async/await patterns

## Part 1: OpenAI SDK Client Factory Pattern

### Pag-unawa sa Multi-Provider Architecture

Ang pagbuo ng mga application na gumagana sa parehong Foundry Local at Azure OpenAI ay nangangailangan ng flexible na pattern sa paglikha ng client:
...
## Part 2: Streaming Responses and Real-time Interaction

### Pagpapatupad ng Streaming Chat Completions

Ang streaming ay nagbibigay ng mas magandang karanasan ng user sa pamamagitan ng pagpapakita ng mga sagot habang ito ay nabubuo:
...
## Part 3: Performance Benchmarking and Analysis

### Pagsukat ng Response Time

Sukatin at ihambing ang performance sa iba't ibang configuration:
...
### Temperature at Parameter Testing
...
## Part 4: Service Health Monitoring and Diagnostics
...
## Part 5: Environment Variables and Configuration Management

### Environment Variable Reference

Kumpletong reference para sa pag-configure ng parehong provider:
...
### Exercise 1: Multi-Provider SDK Integration

Bumuo ng kumpletong application na seamless na lumilipat sa pagitan ng mga provider:
...
### Exercise 3: Production-Ready Health Monitoring
...
Sa sesyon na ito, natutunan mo:
- ✅ **OpenAI SDK Integration**: Mga advanced na pattern para sa parehong Foundry Local at Azure OpenAI
- ✅ **Streaming Responses**: Real-time chat completions para sa mas pinahusay na karanasan ng user
- ✅ **Multi-Provider Support**: Seamless na paglipat sa pagitan ng lokal at cloud AI services
- ✅ **Conversation Management**: Context-aware multi-turn conversations
- ✅ **Performance Monitoring**: Benchmarking at health checking para sa production deployments
- ✅ **Production Patterns**: Matibay na error handling at configuration management
...
1. **🔄 Laging Magpatupad ng Fallbacks**: Azure → Foundry Local → Error handling
2. **🌊 Gumamit ng Streaming para sa Mahahabang Sagot**: Mas magandang perceived performance
3. **🛡️ Magpatupad ng Comprehensive Error Handling**: User-friendly na mga error message
4. **📈 I-monitor ang Performance**: Subaybayan ang response times at success rates
5. **⚙️ Environment-Based Configuration**: Madaling paglipat sa pagitan ng dev/staging/prod
6. **🔒 Secure Credential Management**: Huwag kailanman i-hardcode ang mga API key

### Provider Selection Guidelines

| Scenario | Recommended Provider | Reasoning |
|----------|---------------------|----------|
| **Development** | Foundry Local | Mabilis na iteration, walang API costs |
| **Privacy-Critical** | Foundry Local | Hindi umaalis ang data sa device |
| **High-Volume Production** | Azure OpenAI | Mas magandang scaling, enterprise SLA |
| **Latest Models** | Azure OpenAI | Access sa pinakabagong model releases |
| **Offline Requirements** | Foundry Local | Walang internet dependency |
| **Cost-Sensitive** | Foundry Local | Walang per-token charges |
...
### Preparing for Session 3: Open-Source Models

1. **Explore Model Catalog**: Suriin ang mga available na modelo sa Foundry Local
2. **Understand Model Formats**: Alamin ang tungkol sa ONNX, quantization, at optimization
3. **Consider Custom Models**: Isaalang-alang ang mga domain-specific na pangangailangan sa modelo
...
- **[OpenAI Python SDK Documentation](https://github.com/openai/openai-python)**: Opisyal na SDK reference
- **[Azure OpenAI Documentation](https://learn.microsoft.com/azure/ai-services/openai/)**: Gabay sa Azure OpenAI service
- **[Foundry Local SDK Reference](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**: Dokumentasyon sa lokal na inference
- **[Streaming Completions Guide](https://learn.microsoft.com/azure/ai-foundry/foundry-local/how-to/integrate-with-inference-sdks)**: Mga advanced na streaming pattern
- **[Sample 01: Quick Chat via OpenAI SDK](samples/01/README.md)**: Pangunahing mga pattern ng integrasyon
- **[Sample 02: Advanced SDK Integration](samples/02/README.md)**: Mga hands-on na halimbawa para sa sesyon na ito
- **[Sample 04: Chainlit Application](samples/04/README.md)**: Pagbuo ng Web UI
- **[Sample 05: Multi-Agent Systems](samples/05/README.md)**: Mga advanced na orchestration pattern

Ngayon ay handa ka nang bumuo ng mga sopistikadong AI application na seamless na nag-iintegrate ng lokal at cloud AI capabilities, na nagbibigay ng flexibility na pumili ng tamang provider para sa bawat partikular na use case habang pinapanatili ang consistent na development patterns.
