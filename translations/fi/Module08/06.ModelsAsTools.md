...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T20:24:11+00:00",
...
- Suunnittele hybridiskaalaus Azure AI Foundryyn

## Osa 1: Työkaluabstraktiot (askel askeleelta)

Tavoite: Edusta malleja työkaluina, joilla on selkeät sopimukset ja yksinkertainen reititin.

Vaihe 1) Määrittele työkaluliittymä ja rekisteri  
```python
# tools/registry.py
...
Vaihe 2) Toteuta kaksi työkalua Foundry Localin avulla  
```python
# tools/impl.py
import requests, os
...
Vaihe 3) Reititin tehtävän mukaan  
```python
# tools/router.py
...
## Osa 2: SDK- ja API-integraatio (askel askeleelta)

Tavoite: Käytä OpenAI Python SDK:ta Foundry Local -päätepisteen kanssa.
...
Vaihe 3) Paikallinen vain -lippu ja lokitus  
```python
...
## Osa 5: Skaalaus Azure AI Foundryyn (askel askeleelta)

Tavoite: Peilaa paikalliset mallit Azure-päätepisteisiin ylivuotokapasiteettia varten.

Vaihe 1) Päätä reititysstrategia  
- Paikallinen ensin yksityisyyden/viiveen vuoksi, Azure varajärjestelmänä virheiden tai suurten kehotteiden kohdalla  

Vaihe 2) Toteuta yksinkertainen reitittimen stubi  
```python
# hybrid/router.py
import os, requests
...
- [ ] Rekisteröi vähintään kaksi työkalua ja reititä pyynnöt
- [ ] Kutsu Foundry Localia OpenAI SDK:n ja raakamuotoisen RESTin kautta
- [ ] Varmista JSON-tulostus toimialamallille
- [ ] Puhdista ja kirjaa kutsut paikallisesti
- [ ] Toteuta yksinkertainen hybridireititin Azure-varajärjestelmällä

## Yhteenveto

Foundry Local mahdollistaa vankan laitekohtaisen tekoälyn, jossa mallit toimivat koottavina työkaluina. Selkeiden rajapintojen, hallinnan ja hybridiskaalauksen avulla tiimit voivat julkaista reaaliaikaisia, turvallisia tekoälysovelluksia, jotka kunnioittavat käyttäjän yksityisyyttä ja ovat samalla yritysvalmiita.
