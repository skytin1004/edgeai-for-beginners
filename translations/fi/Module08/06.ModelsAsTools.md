...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T23:32:02+00:00",
...
Käsittele AI-malleja modulaarisina, räätälöitävinä työkaluina, jotka toimivat suoraan laitteella Foundry Localin avulla. Tässä osiossa keskitytään käytännön työnkulkuihin, jotka säilyttävät yksityisyyden, mahdollistavat matalan viiveen ennustamisen ja integroinnin SDK:iden, API:iden tai CLI:n kautta. Opit myös, kuinka skaalata Azure AI Foundryyn tarvittaessa.

> **🔄 Päivitetty modernille SDK:lle**: Tämä moduuli on sovitettu uusimpiin Microsoft Foundry-Local -repositoryn malleihin ja vastaa älykkään reitityksen toteutusta `samples/06/`-kansiossa. Esimerkit käyttävät nyt modernia `foundry-local-sdk`:tä ja edistyneitä mallin valintastrategioita.
...
- **Moderni SDK-integraatio**: Käyttää `FoundryLocalManager`-luokkaa automaattisella palvelun tunnistuksella
- **Ympäristön konfigurointi**: Joustava mallin määrittely ympäristömuuttujien avulla
...
- Integrointi ennustamisen SDK:iden kanssa: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/how-to/how-to-integrate-with-inference-sdks
...
## Oppimistavoitteet
- Suunnittele mallin työkalumalleja laitteella
- Integroi OpenAI-yhteensopivan REST API:n tai SDK:iden kautta
- Räätälöi mallit toimialakohtaisiin käyttötapauksiin
- Suunnittele hybridi-skaalaus Azure AI Foundryyn

## Osa 1: Älykäs mallin reititin (moderni toteutus)

Tavoite: Toteuta älykäs mallin valinta automaattisella reitityksellä kyselyn sisällön perusteella.

> **📋 Huomio**: Tämä toteutus vastaa `samples/06/router.py`-kansiossa käytettyjä malleja, joissa on edistynyt avainsanoihin perustuva mallin valinta.

Vaihe 1) Määritä moderni mallin reititin FoundryLocalManagerin avulla  
```python
...
Tavoite: Käytä Foundry Local SDK:ta yhdessä OpenAI Python SDK:n kanssa saumattomaan integrointiin.
...
## Osa 3: Toimialakohtainen räätälöinti (askel askeleelta)
...
Vaihe 2) Varmista JSON-tulostus  
```python
# domain/analyst.py
...
Tavoite: Varmista yksityisyys ja kestävyys, kun mallit toimivat työkaluina paikallisesti.
...
Vaihe 3) Paikallinen lippu ja lokitus  
```python
...
## Osa 5: Tuotantokäyttöönotto ja skaalaus

Tavoite: Ota käyttöön älykäs reititin seurannalla ja Azure AI Foundry -integraatiolla.

> **📋 Huomio**: Paikallinen toteutus `samples/06/model_router.ipynb`-kansiossa sisältää kattavat esimerkit tuotantokäyttöönottokuvioista.
...
- **Integraatiodokumentaatio**: [Integrointi ennustamisen SDK:iden kanssa](https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/how-to/how-to-integrate-with-inference-sdks)
- **Edistyneet kuviot**: Tutustu funktiokutsuihin ja monen agentin orkestrointiin moduulissa 5

## Yhteenveto

Foundry Local mahdollistaa vankan laitteella toimivan AI:n, jossa mallit muuttuvat älykkäiksi, erikoistuneiksi työkaluiksi. Automaattisen mallin valinnan, kattavan seurannan ja tuotantovalmiiden kuvioiden avulla tiimit voivat julkaista kehittyneitä AI-sovelluksia, jotka mukautuvat eri tehtävätyyppeihin säilyttäen yksityisyyden ja suorituskyvyn. Tässä esitetty älykäs reititinkuviomalli tarjoaa perustan monimutkaisten AI-järjestelmien rakentamiselle, jotka voivat skaalautua paikallisesta kehityksestä tuotantokäyttöön.
