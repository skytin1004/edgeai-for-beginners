...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-01T01:18:00+00:00",
...
Toto sezení se zaměřuje na praktické objevování a správu modelů pomocí Foundry Local. Naučíte se, jak zobrazit dostupné modely, otestovat různé možnosti a pochopit základní charakteristiky výkonu. Přístup klade důraz na praktické zkoumání pomocí příkazového řádku Foundry CLI, které vám pomůže vybrat správné modely pro vaše použití.

## Cíle učení

- Ovládnout příkazy Foundry CLI pro objevování a správu modelů
- Pochopit vzory ukládání do mezipaměti a místního úložiště modelů
...
- Základní znalost typů modelů a jejich použití

## Přehled

Toto sezení zkoumá, jak přinést open-source modely do Foundry Local.
...
3. **Sledujte zdroje**: Sledujte využití paměti
...
- **Požadavky**: Pohodlná práce s přepínáním modelů a základním testováním výkonu
- **Doporučení**: Identifikujte 2-3 oblíbené modely z tohoto sezení

### Další zdroje

- **[Dokumentace Foundry Local](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**: Oficiální dokumentace
- **[Referenční příručka CLI](https://learn.microsoft.com/azure/ai-foundry/foundry-local/reference/reference-cli)**: Kompletní přehled příkazů
...
- **[Sample 03: Objevování modelů](samples/03/README.md)**: Praktický ukázkový skript
...
✅ **Monitorování výkonu**: Základní sledování využití zdrojů a doby odezvy  
✅ **Výběr modelů**: Praktické pokyny pro výběr modelů podle použití  
✅ **Správa mezipaměti**: Pochopení ukládání a postupů čištění  

Nyní máte praktické dovednosti pro objevování, testování a výběr vhodných modelů pro vaše AI aplikace pomocí jednoduchého přístupu Foundry Local CLI: výběr komunitních modelů, integrace obsahu Hugging Face a přijetí strategie „přineste si vlastní model“ (BYOM). Také objevíte sérii Model Mondays pro kontinuální učení a objevování modelů.
...
## Cíle učení
- Objevovat a hodnotit open-source modely pro lokální inference
...
Začněte s populárními, dobře otestovanými modely, abyste pochopili charakteristiky výkonu:
...
**Střední modely:**
- `qwen2.5-7b`: Vynikající uvažování a delší kontext
- `deepseek-r1-7b`: Optimalizováno pro generování kódu

**Velké modely:**
- `llama-3.2`: Nejnovější open-source model od Meta
- `qwen2.5-14b`: Podnikové uvažování na vysoké úrovni

## Část 2: Rychlé testování a porovnávání modelů

### Přístup Sample 03: Jednoduchý seznam a hodnocení
...
Jakmile je model spuštěn, testujte jej s konzistentními výzvami:
...
- Sledujte využití CPU během inference
...
- Pokročilé: `qwen2.5-7b` (delší kontext)

**Pro generování kódu:**
- Doporučeno: `deepseek-r1-7b`
- Alternativa: `qwen2.5-7b` (také dobré pro kód)

**Pro složité uvažování:**
- Nejlepší: `qwen2.5-7b` nebo `qwen2.5-14b`
...
## Část 5: Jednoduché hodnocení modelů
...
- Připravte se na sezení 4: porovnání LLM vs SLM, lokální vs cloudová inference a praktické ukázky
