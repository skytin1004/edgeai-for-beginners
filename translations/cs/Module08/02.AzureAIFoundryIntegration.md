...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-25T01:20:09+00:00",
  "source_file": "Module08/02.AzureAIFoundryIntegration.md",
  "language_code": "cs"
}
-->
# Session 2: Integrace OpenAI SDK s Azure AI Foundry

## Přehled

Na základě vašich znalostí Foundry Local se tato lekce zaměřuje na pokročilé integrační vzory OpenAI SDK, které bezproblémově podporují jak Microsoft Foundry Local, tak Azure OpenAI. Naučíte se vytvářet flexibilní AI aplikace, které mohou běžet lokálně pro zajištění soukromí a vývoj, a zároveň poskytovat škálovatelnost v cloudu prostřednictvím Azure OpenAI, když je to potřeba.

## Cíle učení

Na konci této lekce budete schopni:
- Ovládnout pokročilou integraci OpenAI SDK s Foundry Local i Azure OpenAI
- Implementovat streamované odpovědi pro lepší uživatelský zážitek
- Vytvořit robustní vzory klientských továren pro podporu více poskytovatelů
- Budovat systémy správy konverzací s uchováním kontextu
- Zavést benchmarking výkonu a monitorování stavu
- Nasadit aplikace připravené pro produkci s odpovídajícím zpracováním chyb

## Předpoklady

- Dokončená lekce 1: Začínáme s Foundry Local
- Aktivní instalace Foundry Local s běžícími modely
- Python 3.8 nebo novější s podporou virtuálního prostředí
- Nainstalovaný OpenAI Python SDK (`pip install openai foundry-local-sdk`)
- Azure účet s OpenAI službou (volitelné, pro cloudové scénáře)
- Základní znalost asynchronních vzorů Pythonu (async/await)

## Část 1: Vzor klientské továrny OpenAI SDK

### Porozumění architektuře více poskytovatelů

Vytváření aplikací, které fungují s Foundry Local i Azure OpenAI, vyžaduje flexibilní vzor pro vytváření klientů:
...
## Část 2: Streamované odpovědi a interakce v reálném čase

### Implementace streamovaných dokončení chatu

Streamování poskytuje lepší uživatelský zážitek tím, že zobrazuje odpovědi, jak jsou generovány:
...
### Správa konverzací s více kroky
...
## Část 3: Benchmarking výkonu a analýza

### Měření doby odezvy

Měřte a porovnávejte výkon napříč různými konfiguracemi:
...
## Část 4: Monitorování stavu služby a diagnostika

### Komplexní systém kontroly stavu
...
### Validátor konfigurace prostředí
...
## Část 5: Proměnné prostředí a správa konfigurace

### Referenční příručka proměnných prostředí

Kompletní referenční příručka pro konfiguraci obou poskytovatelů:
...
**Nastavení v příkazovém řádku Windows:**
...
### Cvičení 1: Integrace SDK pro více poskytovatelů

Vytvořte kompletní aplikaci, která bezproblémově přepíná mezi poskytovateli:
...
### Cvičení 2: Pokročilá správa konverzací
...
### Cvičení 3: Monitorování stavu připravené pro produkci
...
## Část 7: Shrnutí a další kroky

### Úspěchy lekce

V této lekci jste zvládli:
- ✅ **Integraci OpenAI SDK**: Pokročilé vzory pro Foundry Local i Azure OpenAI
- ✅ **Streamované odpovědi**: Dokončení chatu v reálném čase pro lepší uživatelský zážitek
- ✅ **Podpora více poskytovatelů**: Bezproblémové přepínání mezi lokálními a cloudovými AI službami
- ✅ **Správa konverzací**: Konverzace s uchováním kontextu
- ✅ **Monitorování výkonu**: Benchmarking a kontrola stavu pro produkční nasazení
- ✅ **Produkční vzory**: Robustní zpracování chyb a správa konfigurace

### Klíčové architektonické vzory

**Vzor klientské továrny:**
```
...
1. **🔄 Vždy implementujte záložní řešení**: Azure → Foundry Local → Zpracování chyb
2. **🌊 Používejte streamování pro dlouhé odpovědi**: Lepší vnímaný výkon
3. **🛡️ Implementujte komplexní zpracování chyb**: Uživatelsky přívětivé chybové zprávy
4. **📈 Monitorujte výkon**: Sledujte dobu odezvy a úspěšnost
5. **⚙️ Konfigurace na základě prostředí**: Snadné přepínání mezi vývojem/testováním/produkčním prostředím
6. **🔒 Zabezpečte správu přihlašovacích údajů**: Nikdy neukládejte API klíče přímo v kódu

### Doporučení pro výběr poskytovatele

| Scénář | Doporučený poskytovatel | Důvod |
|--------|-------------------------|-------|
| **Vývoj** | Foundry Local | Rychlá iterace, žádné náklady na API |
| **Důležité soukromí** | Foundry Local | Data nikdy neopouští zařízení |
| **Produkce s vysokým objemem** | Azure OpenAI | Lepší škálování, podniková SLA |
| **Nejnovější modely** | Azure OpenAI | Přístup k nejnovějším verzím modelů |
| **Požadavky na offline režim** | Foundry Local | Nezávislost na internetu |
| **Citlivost na náklady** | Foundry Local | Žádné poplatky za tokeny |

### Rychlá referenční příručka proměnných prostředí
...
### Příprava na lekci 3: Open-source modely

1. **Prozkoumejte katalog modelů**: Prohlédněte si dostupné modely ve Foundry Local
2. **Porozumějte formátům modelů**: Naučte se o ONNX, kvantizaci a optimalizaci
3. **Zvažte vlastní modely**: Přemýšlejte o požadavcích na modely specifické pro danou oblast
...
- **[OpenAI Python SDK Dokumentace](https://github.com/openai/openai-python)**: Oficiální referenční příručka SDK
- **[Azure OpenAI Dokumentace](https://learn.microsoft.com/azure/ai-services/openai/)**: Průvodce službou Azure OpenAI
- **[Foundry Local SDK Reference](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**: Dokumentace pro lokální inferenci
- **[Průvodce streamovanými dokončeními](https://learn.microsoft.com/azure/ai-foundry/foundry-local/how-to/integrate-with-inference-sdks)**: Pokročilé vzory streamování
- **[Ukázka 01: Rychlý chat přes OpenAI SDK](samples/01/README.md)**: Základní integrační vzory
- **[Ukázka 02: Pokročilá integrace SDK](samples/02/README.md)**: Praktické příklady z této lekce
- **[Ukázka 04: Chainlit aplikace](samples/04/README.md)**: Vývoj webového uživatelského rozhraní
- **[Ukázka 05: Systémy s více agenty](samples/05/README.md)**: Pokročilé vzory orchestrace

Nyní jste připraveni vytvářet sofistikované AI aplikace, které bezproblémově integrují lokální a cloudové AI schopnosti, poskytují flexibilitu při výběru správného poskytovatele pro každý konkrétní případ použití a zároveň zachovávají konzistentní vzory vývoje.
