...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-23T01:07:35+00:00",
...
Považujte AI modely za modulární, přizpůsobitelné nástroje, které běží přímo na zařízení díky Foundry Local. Toto sezení zdůrazňuje praktické pracovní postupy pro ochranu soukromí, nízkou latenci při inferenci a integraci těchto nástrojů prostřednictvím SDK, API nebo CLI. Naučíte se také, jak škálovat na Azure AI Foundry, když je to potřeba.

Reference:
- Dokumentace Foundry Local: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/
- Integrace s inference SDKs: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/how-to/how-to-integrate-with-inference-sdks
...
- Integrovat prostřednictvím OpenAI-kompatibilního REST API nebo SDK
- Přizpůsobit modely specifickým potřebám daného oboru
- Plánovat hybridní škálování na Azure AI Foundry

## Část 1: Abstrakce nástrojů (krok za krokem)

Cíl: Reprezentovat modely jako nástroje s jasnými kontrakty a jednoduchým routerem.

Krok 1) Definujte rozhraní nástroje a registr
```python
# tools/registry.py
...
## Část 2: Integrace SDK a API (krok za krokem)

Cíl: Použít OpenAI Python SDK proti endpointu Foundry Local.
...
## Část 4: Offline režim a bezpečnostní postoj (krok za krokem)

Cíl: Zajistit ochranu soukromí a odolnost při běhu modelů jako nástrojů lokálně.

Krok 1) Předehřátí a validace lokálního endpointu
```cmd
foundry model run phi-4-mini
curl http://localhost:8000/v1/models
```

Krok 2) Sanitizace vstupů
```python
...
## Část 5: Škálování na Azure AI Foundry (krok za krokem)

Cíl: Zrcadlit lokální modely s Azure endpointy pro kapacitu při přetížení.

Krok 1) Rozhodnutí o strategii routování
- Lokální priorita pro ochranu soukromí/latenci, Azure jako záloha při chybách nebo velkých promtech

Krok 2) Implementace jednoduchého routeru
```python
# hybrid/router.py
import os, requests
...
- [ ] Zaregistrujte alespoň dva nástroje a směrujte požadavky
- [ ] Volání Foundry Local prostřednictvím OpenAI SDK a surového REST
- [ ] Vynucení JSON výstupů pro šablonu oboru
- [ ] Sanitizace a logování volání lokálně
- [ ] Implementace jednoduchého hybridního routeru s Azure zálohou

## Závěr

Foundry Local umožňuje robustní AI na zařízení, kde se modely stávají kompozitními nástroji. Díky jasným rozhraním, správě a hybridnímu škálování mohou týmy vytvářet aplikace AI v reálném čase, které respektují soukromí uživatelů a zároveň splňují požadavky podniků.
