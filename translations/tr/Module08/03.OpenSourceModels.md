...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-01T00:19:14+00:00",
...
- Model keşfi ve yönetimi için foundry CLI komutlarını ustalıkla kullanın
- Model önbelleği ve yerel depolama düzenlerini anlayın
- Farklı modelleri hızlı bir şekilde test edin ve karşılaştırın
- Model seçimi ve kıyaslama için pratik iş akışları oluşturun
- Foundry Local aracılığıyla sunulan büyüyen model ekosistemini keşfedin

## Ön Koşullar

- Oturum 1: Foundry Local ile Başlangıç tamamlandı
- Foundry Local CLI kurulu ve erişilebilir durumda
...
## Bölüm 6: Uygulamalı Alıştırma

### Alıştırma: Model Keşfi ve Karşılaştırma

Örnek 03'e dayalı kendi model değerlendirme scriptinizi oluşturun:
...
1. **Örnek 03 scriptini çalıştırın**: `samples\03\list_and_bench.cmd`
...
- Bellek sınırlarına sık sık ulaşıyorsanız RAM yükseltin
...
2. **Bir Seferde Bir Model**: Yeni bir model başlatmadan önce önceki modelleri durdurun
...
- **Ön Koşullar**: Model değiştirme ve temel performans testlerinde rahatlık
- **Önerilen**: Bu oturumdan 2-3 favori model belirlemiş olun
...
- **[Örnek 03: Model Keşfi](samples/03/README.md)**: Uygulamalı örnek script
...
✅ **Önbellek Yönetimi**: Depolama ve temizleme prosedürlerini anlama  

Artık Foundry Local'ın basit CLI yaklaşımını kullanarak AI uygulamalarınız için uygun modelleri keşfetme, test etme ve seçme konusunda pratik becerilere sahipsiniz. Ayrıca topluluk modellerini seçme, Hugging Face içeriklerini entegre etme ve “kendi modelinizi getirme” (BYOM) stratejilerini benimseme konularını keşfedeceksiniz. Sürekli öğrenme ve model keşfi için Model Mondays serisini de inceleyebilirsiniz.
...
## Öğrenme Hedefleri
- Yerel çıkarım için açık kaynak modelleri keşfetme ve değerlendirme
- Foundry Local içinde seçili Hugging Face modellerini derleme ve çalıştırma
- Doğruluk, gecikme ve kaynak ihtiyaçları için model seçimi stratejilerini uygulama
- Önbellek ve sürüm yönetimi ile modelleri yerel olarak yönetme

## Bölüm 1: Foundry CLI ile Model Keşfi

### Temel Model Yönetim Komutları

Foundry CLI, model keşfi ve yönetimi için basit komutlar sağlar:
...
- `phi-4`: Daha iyi akıl yürütme yeteneği olan daha yetenekli versiyon

**Orta Modeller:**
- `qwen2.5-7b`: Mükemmel akıl yürütme ve daha uzun bağlam
- `deepseek-r1-7b`: Kod üretimi için optimize edilmiş

**Büyük Modeller:**
- `llama-3.2`: Meta'nın en son açık kaynak modeli
- `qwen2.5-14b`: Kurumsal düzeyde akıl yürütme

## Bölüm 2: Hızlı Model Testi ve Karşılaştırma

### Örnek 03 Yaklaşımı: Basit Liste ve Kıyaslama

Örnek 03 desenimize dayanarak, işte minimal iş akışı:
...
- İlk model yükleme sırasında disk G/Ç'yi kontrol edin
...
- İleri düzey: `qwen2.5-7b` (daha uzun bağlam)

**Kod Üretimi için:**
- Önerilen: `deepseek-r1-7b`
- Alternatif: `qwen2.5-7b` (kod için de iyi)

**Karmaşık Akıl Yürütme için:**
- En İyisi: `qwen2.5-7b` veya `qwen2.5-14b`
