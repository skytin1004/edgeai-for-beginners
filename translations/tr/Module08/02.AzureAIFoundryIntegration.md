...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T21:33:46+00:00",
  "source_file": "Module08/02.AzureAIFoundryIntegration.md",
  "language_code": "tr"
}
-->
# Oturum 2: OpenAI SDK'nin Azure AI Foundry ile Entegrasyonu

## Genel Bakış

Foundry Local temelinizi geliştirerek, bu oturumda hem Microsoft Foundry Local hem de Azure OpenAI'yi destekleyen gelişmiş OpenAI SDK entegrasyon modellerine odaklanıyoruz. Gizlilik ve geliştirme için yerel olarak çalışabilen, ihtiyaç duyulduğunda Azure OpenAI üzerinden bulut ölçeklenebilirliği sağlayan esnek AI uygulamaları oluşturmayı öğreneceksiniz.

## Öğrenme Hedefleri

Bu oturumun sonunda:
- Foundry Local ve Azure OpenAI ile gelişmiş OpenAI SDK entegrasyonunu ustalıkla kullanabileceksiniz
- Kullanıcı deneyimini geliştirmek için akış yanıtlarını uygulayabileceksiniz
- Çoklu sağlayıcı desteği için sağlam bir istemci fabrikası modeli oluşturabileceksiniz
- Bağlam koruma ile konuşma yönetim sistemleri geliştirebileceksiniz
- Performans kıyaslama ve sağlık izleme sistemleri kurabileceksiniz
- Hataları doğru bir şekilde ele alan üretime hazır uygulamalar dağıtabileceksiniz

## Ön Koşullar

- Oturum 1: Foundry Local ile Başlangıç tamamlanmış olmalı
- Çalışan modellerle aktif bir Foundry Local kurulumu
- Sanal ortam desteği olan Python 3.8 veya daha yeni bir sürüm
- OpenAI Python SDK kurulu (`pip install openai foundry-local-sdk`)
- Azure hesabı ve OpenAI hizmeti (isteğe bağlı, bulut senaryoları için)
- Python async/await modelleri hakkında temel bilgi

## Bölüm 1: OpenAI SDK İstemci Fabrikası Modeli

### Çoklu Sağlayıcı Mimarisini Anlamak

Hem Foundry Local hem de Azure OpenAI ile çalışan uygulamalar oluşturmak için esnek bir istemci oluşturma modeli gereklidir:
...
## Bölüm 2: Akış Yanıtları ve Gerçek Zamanlı Etkileşim

### Akışlı Sohbet Tamamlama Uygulaması

Akış, yanıtlar oluşturuldukça gösterilerek daha iyi bir kullanıcı deneyimi sağlar:
...
### Çoklu Dönüşlü Konuşma Yönetimi
...
## Bölüm 3: Performans Kıyaslama ve Analiz

### Yanıt Süresi Ölçümü

Farklı yapılandırmalar arasında performansı ölçün ve karşılaştırın:
...
## Bölüm 4: Hizmet Sağlığı İzleme ve Tanılama

### Kapsamlı Sağlık Kontrol Sistemi
...
### Ortam Yapılandırma Doğrulayıcı
...
## Bölüm 5: Ortam Değişkenleri ve Yapılandırma Yönetimi

### Ortam Değişkeni Referansı

Her iki sağlayıcıyı yapılandırmak için tam referans:
...
## Bölüm 6: Uygulamalı Alıştırmalar

### Alıştırma 1: Çoklu Sağlayıcı SDK Entegrasyonu

Sağlayıcılar arasında sorunsuz geçiş yapabilen eksiksiz bir uygulama oluşturun:
...
### Alıştırma 2: Gelişmiş Konuşma Yönetimi
...
### Alıştırma 3: Üretime Hazır Sağlık İzleme
...
## Bölüm 7: Özet ve Sonraki Adımlar

### Oturumda Kazanılanlar

Bu oturumda şunları öğrendiniz:
- ✅ **OpenAI SDK Entegrasyonu**: Hem Foundry Local hem de Azure OpenAI için gelişmiş modeller
- ✅ **Akış Yanıtları**: Kullanıcı deneyimini geliştiren gerçek zamanlı sohbet tamamlama
- ✅ **Çoklu Sağlayıcı Desteği**: Yerel ve bulut AI hizmetleri arasında sorunsuz geçiş
- ✅ **Konuşma Yönetimi**: Bağlam farkındalığı olan çoklu dönüşlü konuşmalar
- ✅ **Performans İzleme**: Üretim dağıtımları için kıyaslama ve sağlık kontrolü
- ✅ **Üretim Modelleri**: Sağlam hata yönetimi ve yapılandırma yönetimi

### Temel Mimari Modeller

**İstemci Fabrikası Modeli:**
```
...
1. **🔄 Her Zaman Yedekleme Uygulayın**: Azure → Foundry Local → Hata yönetimi
2. **🌊 Uzun Yanıtlar İçin Akış Kullanın**: Algılanan performansı artırır
3. **🛡️ Kapsamlı Hata Yönetimi Uygulayın**: Kullanıcı dostu hata mesajları
4. **📈 Performansı İzleyin**: Yanıt sürelerini ve başarı oranlarını takip edin
5. **⚙️ Ortam Tabanlı Yapılandırma**: Geliştirme/ön üretim/üretim arasında kolay geçiş
6. **🔒 Kimlik Bilgilerini Güvenli Yönetin**: API anahtarlarını asla kodda sabitlemeyin

### Sağlayıcı Seçim Kılavuzu

| Senaryo | Önerilen Sağlayıcı | Gerekçe |
|----------|---------------------|----------|
| **Geliştirme** | Foundry Local | Hızlı yineleme, API maliyeti yok |
| **Gizlilik Hassasiyeti** | Foundry Local | Veriler cihazdan çıkmaz |
| **Yüksek Hacimli Üretim** | Azure OpenAI | Daha iyi ölçeklenebilirlik, kurumsal SLA |
| **En Yeni Modeller** | Azure OpenAI | En yeni model sürümlerine erişim |
| **Çevrimdışı Gereksinimler** | Foundry Local | İnternet bağımlılığı yok |
| **Maliyet Hassasiyeti** | Foundry Local | Token başına ücret yok |

### Ortam Değişkenleri Hızlı Referansı
...
### Oturum 3'e Hazırlık: Açık Kaynak Modeller

1. **Model Kataloğunu İnceleyin**: Foundry Local'deki mevcut modelleri gözden geçirin
2. **Model Formatlarını Anlayın**: ONNX, kuantizasyon ve optimizasyon hakkında bilgi edinin
3. **Özel Modelleri Düşünün**: Alanınıza özel model gereksinimlerini değerlendirin

### Sorun Giderme Hızlı Kılavuzu
...
- **[OpenAI Python SDK Belgeleri](https://github.com/openai/openai-python)**: Resmi SDK referansı
- **[Azure OpenAI Belgeleri](https://learn.microsoft.com/azure/ai-services/openai/)**: Azure OpenAI hizmet rehberi
- **[Foundry Local SDK Referansı](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**: Yerel çıkarım belgeleri
- **[Akışlı Tamamlama Rehberi](https://learn.microsoft.com/azure/ai-foundry/foundry-local/how-to/integrate-with-inference-sdks)**: Gelişmiş akış modelleri
- **[Örnek 01: OpenAI SDK ile Hızlı Sohbet](samples/01/README.md)**: Temel entegrasyon modelleri
- **[Örnek 02: Gelişmiş SDK Entegrasyonu](samples/02/README.md)**: Bu oturumun uygulamalı örnekleri
- **[Örnek 04: Chainlit Uygulaması](samples/04/README.md)**: Web UI geliştirme
- **[Örnek 05: Çoklu Aracı Sistemler](samples/05/README.md)**: Gelişmiş orkestrasyon modelleri

Artık yerel ve bulut AI yeteneklerini sorunsuz bir şekilde entegre eden, her bir kullanım senaryosu için doğru sağlayıcıyı seçme esnekliği sunan ve tutarlı geliştirme modellerini koruyan sofistike AI uygulamaları oluşturmak için donanımlısınız.
