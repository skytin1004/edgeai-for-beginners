...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T10:19:50+00:00",
...
Traitez les modèles d'IA comme des outils modulaires et personnalisables fonctionnant directement sur l'appareil avec Foundry Local. Cette session met l'accent sur des workflows pratiques pour une inférence respectueuse de la vie privée et à faible latence, ainsi que sur l'intégration de ces outils via des SDK, des API ou une CLI. Vous apprendrez également à évoluer vers Azure AI Foundry si nécessaire.
...
> **📋 Remarque** : Cette implémentation correspond aux modèles utilisés dans `samples/06/router.py` avec une sélection avancée basée sur des mots-clés.
...
Étape 2) Initialiser le client avec le SDK moderne et la découverte des services  
```python
...
Étape 2) Appliquer un format de sortie JSON  
```python
# domain/analyst.py
...
Étape 1) Préparer et valider le point de terminaison local  
```cmd
...
> **📋 Remarque** : L'implémentation locale dans `samples/06/model_router.ipynb` inclut des exemples complets de modèles de déploiement en production.
...
Foundry Local permet de développer des IA robustes sur l'appareil, où les modèles deviennent des outils intelligents et spécialisés. Avec une sélection automatique des modèles, une surveillance complète et des modèles prêts pour la production, les équipes peuvent déployer des applications d'IA sophistiquées qui s'adaptent à différents types de tâches tout en maintenant la confidentialité et les performances. Le modèle de routeur intelligent présenté ici constitue une base pour construire des systèmes d'IA complexes pouvant évoluer du développement local au déploiement en production.
