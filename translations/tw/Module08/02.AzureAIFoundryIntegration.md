...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T09:45:52+00:00",
...
在您已掌握 Foundry Local 基礎的基礎上，本節課程將重點介紹進階的 OpenAI SDK 整合模式，這些模式能無縫支援 Microsoft Foundry Local 和 Azure OpenAI。您將學習如何構建靈活的 AI 應用程式，既能在本地運行以保護隱私和進行開發，又能在需要時通過 Azure OpenAI 提供雲端擴展性。
...
- 支援虛擬環境的 Python 3.8 或更高版本
- 已安裝 OpenAI Python SDK (`pip install openai foundry-local-sdk`)
- 擁有 Azure 帳戶並啟用 OpenAI 服務（可選，用於雲端場景）
...
1. **🔄 始終實現備援機制**：Azure → Foundry Local → 錯誤處理
2. **🌊 對長回應使用流式模式**：提升感知效能
3. **🛡️ 實施全面錯誤處理**：提供使用者友好的錯誤訊息
4. **📈 監控效能**：追蹤回應時間和成功率
5. **⚙️ 基於環境的配置**：輕鬆切換開發/測試/生產環境
6. **🔒 安全憑證管理**：切勿硬編碼 API 金鑰
...
| **隱私敏感** | Foundry Local | 資料不離開設備 |
| **高流量生產** | Azure OpenAI | 更好的擴展性，企業級 SLA |
| **最新模型** | Azure OpenAI | 獲取最新模型版本 |
| **離線需求** | Foundry Local | 無需網路依賴 |
| **成本敏感** | Foundry Local | 無每字元費用 |
...
- **[OpenAI Python SDK 文件](https://github.com/openai/openai-python)**：官方 SDK 參考
- **[Azure OpenAI 文件](https://learn.microsoft.com/azure/ai-services/openai/)**：Azure OpenAI 服務指南
- **[Foundry Local SDK 參考](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**：本地推理文件
- **[流式補全指南](https://learn.microsoft.com/azure/ai-foundry/foundry-local/how-to/integrate-with-inference-sdks)**：進階流式模式
- **[範例 01：通過 OpenAI SDK 快速聊天](samples/01/README.md)**：基本整合模式
- **[範例 02：進階 SDK 整合](samples/02/README.md)**：本節課程的實作範例
- **[範例 04：Chainlit 應用程式](samples/04/README.md)**：Web UI 開發
- **[範例 05：多代理系統](samples/05/README.md)**：進階編排模式

您現在已具備構建複雜 AI 應用程式的能力，這些應用程式能無縫整合本地和雲端 AI 功能，提供靈活性以根據具體使用場景選擇合適的供應商，同時保持一致的開發模式。
