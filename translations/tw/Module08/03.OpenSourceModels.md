...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T23:28:41+00:00",
...
本節課程專注於使用 Foundry Local 進行模型探索與管理的實踐操作。您將學習如何列出可用模型、測試不同選項，以及了解基本性能特徵。課程重點在於使用 Foundry CLI 進行動手探索，幫助您選擇適合自己使用案例的模型。
...
本節課程探討如何將開源模型引入 Foundry Local，並採用“自帶模型”（BYOM）策略。您還將了解 Model Mondays 系列，持續學習與模型探索。
...
- **[Foundry Local 文件](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**：官方文檔
...
- **[範例 03：模型探索](samples/03/README.md)**：動手示例腳本
...
您現在擁有使用 Foundry Local 的簡單 CLI 方法探索、測試與選擇適合 AI 應用的模型的實用技能。
...
- `phi-4`：更強大的版本，具有更好的推理能力

**中型模型：**
- `qwen2.5-7b`：出色的推理能力與更長的上下文
...
- SSD 存儲，加快模型加載速度
- 現代 CPU，具備良好的單線程性能
- 支援 NPU（Windows 11 Copilot+ PC）以加速運行
...
- 為第四節課程做準備：比較 LLMs 與 SLMs、本地與雲端推理，以及動手演示
