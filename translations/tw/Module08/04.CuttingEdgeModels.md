...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T11:47:18+00:00",
...
比較 LLMs 和 SLMs，評估本地與雲端推理的取捨，並實現展示使用 Phi 和 ONNX Runtime 的 EdgeAI 情境的演示。我們還將重點介紹 Chainlit RAG、WebGPU 推理選項以及 Open WebUI 的整合。
...
使用 WebGPU 在瀏覽器中直接運行小型模型。這非常適合隱私優先的演示和零安裝體驗。以下是使用 ONNX Runtime Web 和 WebGPU 執行提供者的簡化步驟示例。
...
4) 故障排除
- 如果 WebGPU 不可用：更新 Chrome/Edge 並確保 GPU 驅動程式是最新的，然後檢查 chrome://flags 中的“Enable WebGPU”。
- 如果出現 CORS 或 fetch 錯誤：確保通過 http://（而不是 file://）提供文件，並且模型 URL 允許跨域請求。
