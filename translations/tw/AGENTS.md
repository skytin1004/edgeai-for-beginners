...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-03T08:26:12+00:00",
...
EdgeAI for Beginners 是一個全面的教育資源庫，旨在教授使用小型語言模型 (SLMs) 進行邊緣 AI 開發。課程涵蓋邊緣 AI 基礎知識、模型部署、優化技術，以及使用 Microsoft Foundry Local 和各種 AI 框架進行生產級實現。
...
Python 範例沒有自動化測試，但可以通過運行它們進行驗證：
```bash
...
**模組08/範例/08（Electron 應用程式）有全面的測試：**
```bash
cd Module08/samples/08
...
**重要事項：** 此資源庫使用 GitHub Actions 進行自動翻譯。

- 翻譯存放於 `/translations/` 目錄（50+ 種語言）
...
- **模組06：** AI 代理和函數調用
- **模組07：** 平台特定實現
- **模組08：** Foundry Local 工具包及 10 個全面範例
...
**這是一個教育資源庫，專注於教授邊緣 AI 開發。主要貢獻模式是改進教育內容以及添加/增強展示邊緣 AI 概念的範例應用程式。**
