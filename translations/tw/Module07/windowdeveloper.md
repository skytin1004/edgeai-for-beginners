...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-15T17:17:00+00:00",
...
歡迎來到 Windows Edge AI 開發指南——這是您全面了解如何使用 Microsoft 的 Windows AI Foundry 平台構建智能應用程式的指南。此指南專為希望將尖端的 Edge AI 功能整合到其應用程式中的 Windows 開發者設計，同時充分利用 Windows 硬體加速的全部潛力。
...
**開發者生態系統**  
豐富的工具，包括 Visual Studio 整合、全面的文檔和範例應用程式，加速開發週期。
...
- 實現 Phi Silica 語言模型整合，用於文本生成和推理  
...
Windows AI API 提供即用型 AI 功能，通過設備端模型支持，針對 Copilot+ PC 設備進行效率和性能優化，且設置需求極少。
...
- 針對實時推理進行優化，功耗極低  
- 支持使用 LoRA 技術進行自定義微調  
- 與 Windows 語義搜索和知識檢索整合  

**計算機視覺 API**  
- **文本識別 (OCR)**：高準確度地從圖像中提取文本  
- **圖像超分辨率**：使用設備端 AI 模型提升圖像質量  
- **圖像分割**：識別並隔離圖像中的特定物體  
- **圖像描述**：為視覺內容生成詳細的文本描述  
...
Windows ML 是 Windows 的核心 AI 平台和整合推理運行時，允許開發者高效地在廣泛的 Windows 硬體生態系統中部署自定義模型。
...
Windows Edge AI 開發代表了強大 AI 功能與穩健、安全且可擴展的 Windows 平台的融合。通過掌握 Windows AI Foundry 生態系統，開發者可以創建智能應用程式，提供卓越的使用者體驗，同時保持最高的隱私、安全性和性能標準。  

Windows AI API、Foundry Local 和 Windows ML 的結合提供了無與倫比的基礎，用於構建下一代智能 Windows 應用程式。隨著 AI 的不斷演進，Windows 平台確保您的應用程式能夠隨著新興技術擴展，同時保持與多樣化 Windows 硬體生態系統的兼容性和性能。  

無論您是在開發消費者應用程式、企業解決方案，還是專業行業工具，Windows Edge AI 開發都能讓您創建智能、響應迅速且深度整合的體驗，充分利用現代 Windows 設備的全部潛力。  
