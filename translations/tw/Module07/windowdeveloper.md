...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T11:33:47+00:00",
...
- **硬體抽象**：可在 AMD、Intel、NVIDIA 和 Qualcomm 晶片上無縫部署
...
- 將應用程式定位為利用 Windows 生態系統的優勢  

## Windows AI Foundry 平台組件

### 1. Windows AI API

Windows AI API 提供即用型 AI 功能，由設備端模型驅動，針對 Copilot+ PC 設備進行了效率和性能優化，且設置需求極低。
...
- **圖像分割**：識別並隔離圖像中的特定物體  
- **圖像描述**：為視覺內容生成詳細的文本描述  
- **物體擦除**：使用 AI 驅動的修補技術移除圖像中的不需要物體  

**多模態功能**  
- **視覺-語言集成**：結合文本和圖像理解  
- **語義搜索**：啟用跨多媒體內容的自然語言查詢  
- **知識檢索**：使用本地數據構建智能搜索體驗  
...
Windows ML 是 Windows 的核心 AI 平台和集成推理運行時，允許開發者在廣泛的 Windows 硬體生態系統中高效部署自定義模型。
...
Windows AI API、Foundry Local 和 Windows ML 的結合提供了無與倫比的基礎，用於構建下一代智能 Windows 應用程式。隨著 AI 的不斷演進，Windows 平台確保您的應用程式能夠隨著新興技術擴展，同時保持在多樣化的 Windows 硬體生態系統中的兼容性和性能。  
