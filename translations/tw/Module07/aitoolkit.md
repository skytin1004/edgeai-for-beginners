...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-15T17:23:20+00:00",
...
本指南將帶您了解如何在邊緣 AI 專案中使用 AI Toolkit 的核心概念、工具和最佳實踐，從初始模型選擇到生產部署。

## 概述

AI Toolkit 提供了一個整合的開發環境，涵蓋完整的邊緣 AI 應用生命周期，並與 VS Code 無縫整合。它支持來自 OpenAI、Anthropic、Google 和 GitHub 等提供商的流行 AI 模型，同時支持通過 ONNX 和 Ollama 進行本地模型部署——這對於需要設備端推理的邊緣 AI 應用至關重要。
...
- **將所學概念應用於真實世界的邊緣 AI 用例**，包括 IoT、移動和嵌入式應用
...
- **Ollama 支持**：利用 Ollama 提供的本地運行模型，確保隱私和離線操作
- **模型比較**：並排比較模型，找到性能和資源消耗的最佳平衡
...
- **跨平台支援**: 部署於 Android、iOS 和嵌入式 Linux 系統
...
Visual Studio Code 的 AI 工具包提供了一個全面的平台，用於邊緣 AI 的開發，涵蓋模型發現、優化到部署和監控。透過其整合的工具和工作流程，開發者能高效地創建、測試並部署能在資源受限的邊緣設備上有效運行的 AI 應用程式。
...
隨著邊緣 AI 的不斷發展，Visual Studio Code 的 AI 工具包始終走在前沿，為開發者提供尖端工具和能力，助力打造下一代智能邊緣應用程式。

---

**免責聲明**：  
本文件已使用 AI 翻譯服務 [Co-op Translator](https://github.com/Azure/co-op-translator) 進行翻譯。儘管我們努力確保翻譯的準確性，但請注意，自動翻譯可能包含錯誤或不準確之處。原始文件的母語版本應被視為權威來源。對於重要信息，建議使用專業人工翻譯。我們對因使用此翻譯而產生的任何誤解或錯誤解釋不承擔責任。