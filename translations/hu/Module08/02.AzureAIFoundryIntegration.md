...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-25T01:12:56+00:00",
  "source_file": "Module08/02.AzureAIFoundryIntegration.md",
  "language_code": "hu"
}
-->
# 2. szekció: OpenAI SDK integráció az Azure AI Foundry-val

## Áttekintés

Az előző Foundry Local alapokra építve, ez a szekció az OpenAI SDK fejlett integrációs mintáira összpontosít, amelyek zökkenőmentesen támogatják mind a Microsoft Foundry Local-t, mind az Azure OpenAI-t. Megtanulod, hogyan építs rugalmas AI alkalmazásokat, amelyek helyben futnak a magánélet és fejlesztés érdekében, miközben felhőalapú skálázhatóságot biztosítanak az Azure OpenAI segítségével, amikor szükséges.

## Tanulási célok

A szekció végére képes leszel:
- Elsajátítani az OpenAI SDK fejlett integrációját a Foundry Local-lal és az Azure OpenAI-val
- Streaming válaszokat implementálni a jobb felhasználói élmény érdekében
- Robusztus kliensgyár mintákat létrehozni több szolgáltató támogatására
- Beszélgetéskezelő rendszereket építeni kontextusmegőrzéssel
- Teljesítmény-benchmarkingot és állapotfigyelést létrehozni
- Gyártásra kész alkalmazásokat telepíteni megfelelő hibakezeléssel

## Előfeltételek

- Az 1. szekció elvégzése: Bevezetés a Foundry Local-ba
- Aktív Foundry Local telepítés futó modellekkel
- Python 3.8 vagy újabb verzió virtuális környezet támogatással
- Telepített OpenAI Python SDK (`pip install openai foundry-local-sdk`)
- Azure fiók OpenAI szolgáltatással (opcionális, felhőalapú forgatókönyvekhez)
- Alapvető ismeretek a Python async/await mintákról

## 1. rész: OpenAI SDK kliensgyár minta

### Több szolgáltató architektúrájának megértése

Olyan alkalmazások építése, amelyek működnek mind a Foundry Local-lal, mind az Azure OpenAI-val, rugalmas kliens létrehozási mintát igényel:
...
## 2. rész: Streaming válaszok és valós idejű interakció

### Streaming chat completions implementálása

A streaming jobb felhasználói élményt nyújt azáltal, hogy a válaszokat azok generálása közben jeleníti meg:
...
### Többfordulós beszélgetéskezelés
...
## 3. rész: Teljesítmény-benchmarking és elemzés

### Válaszidő mérése

Mérd és hasonlítsd össze a teljesítményt különböző konfigurációk között:
...
### Hőmérséklet és paraméterek tesztelése
...
## 4. rész: Szolgáltatás állapotfigyelés és diagnosztika

### Átfogó állapotellenőrző rendszer
...
### Környezetkonfiguráció ellenőrző
...
## 5. rész: Környezeti változók és konfigurációkezelés

### Környezeti változók referencia

Teljes referencia mindkét szolgáltató konfigurálásához:
...
**Windows Command Prompt beállítás:**
...
## 6. rész: Gyakorlati feladatok

### 1. gyakorlat: Több szolgáltatós SDK integráció

Építs egy teljes alkalmazást, amely zökkenőmentesen vált a szolgáltatók között:
...
### 2. gyakorlat: Fejlett beszélgetéskezelés
...
### 3. gyakorlat: Gyártásra kész állapotfigyelés
...
## 7. rész: Összefoglalás és következő lépések

### Szekció eredményei

Ebben a szekcióban elsajátítottad:
- ✅ **OpenAI SDK integráció**: Fejlett minták a Foundry Local és az Azure OpenAI számára
- ✅ **Streaming válaszok**: Valós idejű chat completions a jobb felhasználói élmény érdekében
- ✅ **Több szolgáltató támogatása**: Zökkenőmentes váltás helyi és felhőalapú AI szolgáltatások között
- ✅ **Beszélgetéskezelés**: Kontextusérzékeny többfordulós beszélgetések
- ✅ **Teljesítményfigyelés**: Benchmarking és állapotellenőrzés gyártási telepítésekhez
- ✅ **Gyártási minták**: Robusztus hibakezelés és konfigurációkezelés

### Kulcsfontosságú architektúra minták
...
1. **🔄 Mindig implementálj visszaesési lehetőségeket**: Azure → Foundry Local → Hibakezelés
2. **🌊 Használj streaminget hosszú válaszokhoz**: Jobb érzékelt teljesítmény
3. **🛡️ Implementálj átfogó hibakezelést**: Felhasználóbarát hibaüzenetek
4. **📈 Figyeld a teljesítményt**: Kövesd a válaszidőket és sikerességi arányokat
5. **⚙️ Környezetalapú konfiguráció**: Könnyű váltás fejlesztés/staging/gyártás között
6. **🔒 Biztonságos hitelesítő adatok kezelése**: Soha ne kódolj API kulcsokat

### Szolgáltató kiválasztási irányelvek

| Forgatókönyv | Ajánlott szolgáltató | Indoklás |
|--------------|---------------------|----------|
| **Fejlesztés** | Foundry Local | Gyors iteráció, nincs API költség |
| **Adatvédelem szempontjából kritikus** | Foundry Local | Az adatok nem hagyják el az eszközt |
| **Nagy volumenű gyártás** | Azure OpenAI | Jobb skálázás, vállalati SLA |
| **Legújabb modellek** | Azure OpenAI | Hozzáférés a legújabb modellekhez |
| **Offline követelmények** | Foundry Local | Nincs internetfüggőség |
| **Költségérzékeny** | Foundry Local | Nincs tokenenkénti díj |
...
### Felkészülés a 3. szekcióra: Nyílt forráskódú modellek

1. **Modellek katalógusának felfedezése**: Tekintsd át a Foundry Local-ban elérhető modelleket
2. **Modellek formátumának megértése**: Ismerd meg az ONNX, kvantálás és optimalizálás fogalmait
3. **Egyedi modellek fontolóra vétele**: Gondolj a domain-specifikus modellkövetelményekre
...
- **[OpenAI Python SDK Dokumentáció](https://github.com/openai/openai-python)**: Hivatalos SDK referencia
- **[Azure OpenAI Dokumentáció](https://learn.microsoft.com/azure/ai-services/openai/)**: Azure OpenAI szolgáltatás útmutató
- **[Foundry Local SDK Referencia](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**: Helyi inferencia dokumentáció
- **[Streaming Completions Útmutató](https://learn.microsoft.com/azure/ai-foundry/foundry-local/how-to/integrate-with-inference-sdks)**: Fejlett streaming minták
- **[Minta 01: Gyors chat az OpenAI SDK-val](samples/01/README.md)**: Alapvető integrációs minták
- **[Minta 02: Fejlett SDK integráció](samples/02/README.md)**: Ennek a szekciónak a gyakorlati példái
- **[Minta 04: Chainlit alkalmazás](samples/04/README.md)**: Webes UI fejlesztés
- **[Minta 05: Többügynökös rendszerek](samples/05/README.md)**: Fejlett orkestrációs minták

Most már készen állsz arra, hogy kifinomult AI alkalmazásokat építs, amelyek zökkenőmentesen integrálják a helyi és felhőalapú AI képességeket, lehetővé téve a megfelelő szolgáltató kiválasztását minden egyes konkrét felhasználási esethez, miközben következetes fejlesztési mintákat tartasz fenn.
