...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-25T01:11:34+00:00",
  "source_file": "Module08/03.OpenSourceModels.md",
  "language_code": "hu"
}
-->
# 3. ülés: Nyílt forráskódú modellek felfedezése és kezelése

## Áttekintés

Ez az ülés a Foundry Local gyakorlati modellfelfedezésére és kezelésére összpontosít. Megtanulhatod, hogyan listázd a rendelkezésre álló modelleket, teszteld a különböző lehetőségeket, és megértsd az alapvető teljesítményjellemzőket. Az oktatás hangsúlyt fektet a Foundry CLI gyakorlati használatára, hogy segítsen kiválasztani a megfelelő modelleket az adott felhasználási esetekhez.

## Tanulási célok

- A Foundry CLI parancsok elsajátítása a modellek felfedezéséhez és kezeléséhez
...
- Az 1. ülés elvégzése: Bevezetés a Foundry Local használatába
...
Ez az ülés bemutatja, hogyan lehet nyílt forráskódú modelleket integrálni a Foundry Local-ba.

## 6. rész: Gyakorlati feladat

### Feladat: Modellek felfedezése és összehasonlítása

Hozz létre saját modellértékelő szkriptet a 03-as minta alapján:
...
- Frissítsd a RAM-ot, ha gyakran elérsz a korlátokhoz
...
1. **Kezdd kicsiben**: Kezdd a `phi-4-mini` modellel a beállítás ellenőrzéséhez
2. **Egyszerre egy modell**: Állítsd le az előző modelleket, mielőtt újakat indítasz
...
### Felkészülés a 4. ülésre

- **4. ülés fókusza**: Optimalizációs eszközök és technikák
- **Előfeltételek**: Magabiztos modellváltás és alapvető teljesítménytesztelés
- **Ajánlott**: Azonosíts 2-3 kedvenc modellt ebből az ülésből
...
- **[03-as minta: Modellek felfedezése](samples/03/README.md)**: Gyakorlati példa szkript

### Főbb tanulságok

✅ **Modellek felfedezése**: Használd a `foundry model list` parancsot a modellek felfedezéséhez  
✅ **Gyors tesztelés**: A `list_and_bench.cmd` minta gyors értékeléshez  
✅ **Teljesítményfigyelés**: Alapvető erőforrás-használat és válaszidő mérése  
✅ **Modellválasztás**: Gyakorlati irányelvek a modellek kiválasztásához felhasználási esetek alapján  
✅ **Gyorsítótár kezelése**: Tárolási és tisztítási eljárások megértése  

Most már rendelkezel a gyakorlati készségekkel, hogy felfedezd, teszteld és kiválaszd a megfelelő modelleket AI alkalmazásaidhoz a Foundry Local egyszerű CLI megközelítésével: közösségi modellek kiválasztása, Hugging Face tartalom integrálása és „hozd a saját modelledet” (BYOM) stratégiák alkalmazása. Fedezd fel a Model Mondays sorozatot a folyamatos tanulás és modellfelfedezés érdekében.
...
## 1. rész: Modellek felfedezése a Foundry CLI segítségével
...
Kezdd népszerű, jól tesztelt modellekkel a teljesítményjellemzők megértéséhez:
...
- Modell letöltési folyamatát (első futtatáskor)
- Memóriaallokáció részleteit
- Szolgáltatás kötési információkat
- Teljesítmény inicializálási metrikákat

### Modellkategóriák megértése

**Kis nyelvi modellek (SLM-ek):**
- `phi-4-mini`: Gyors, hatékony, általános beszélgetésekhez kiváló
- `phi-4`: Fejlettebb verzió, jobb érvelési képességekkel

**Közepes modellek:**
- `qwen2.5-7b-instruct`: Kiváló érvelés és hosszabb kontextus
- `deepseek-r1-distill-qwen-7b`: Optimalizált kódgeneráláshoz

**Nagyobb modellek:**
- `llama-3.2`: Meta legújabb nyílt forráskódú modellje
- `qwen2.5-14b-instruct`: Vállalati szintű érvelés
...
A Foundry Local automatikusan kezeli a modellek letöltését és gyorsítótárazását:
...
- Fontold meg a modell méret és képesség közötti kompromisszumokat
...
- Kövesd a CPU kihasználtságot az inferencia során
- Ellenőrizd a lemez I/O-t a modell kezdeti betöltésekor
...
**Általános beszélgetésekhez és kérdésekhez:**
- Kezdd: `phi-4-mini` (gyors, hatékony)
- Frissíts: `phi-4` (jobb érvelés)
- Haladó: `qwen2.5-7b-instruct` (hosszabb kontextus)

**Kódgeneráláshoz:**
- Ajánlott: `deepseek-r1-distill-qwen-7b`
- Alternatíva: `qwen2.5-7b-instruct` (kódhoz is jó)

**Komplex érveléshez:**
- Legjobb: `qwen2.5-7b-instruct` vagy `qwen2.5-14b-instruct`
...
- Modern CPU jó egy szálas teljesítménnyel
...
Íme egy egyszerű megközelítés a modell teljesítményének összehasonlításához:
...
2. "Írj egy Python függvényt egy lista rendezéséhez."
...
- Készülj a 4. ülésre: LLM-ek és SLM-ek összehasonlítása, helyi és felhőalapú inferencia, valamint gyakorlati bemutatók
