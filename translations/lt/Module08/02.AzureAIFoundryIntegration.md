...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-25T02:42:23+00:00",
  "source_file": "Module08/02.AzureAIFoundryIntegration.md",
  "language_code": "lt"
}
-->
# Sesija 2: OpenAI SDK integracija su Azure AI Foundry

## Apžvalga

Remdamiesi Foundry Local pagrindais, šioje sesijoje gilinsitės į pažangius OpenAI SDK integracijos modelius, kurie sklandžiai palaiko tiek Microsoft Foundry Local, tiek Azure OpenAI. Išmoksite kurti lanksčias AI programas, kurios gali veikti lokaliai, užtikrinant privatumą ir vystymą, o prireikus suteikti debesų mastelio galimybes per Azure OpenAI.

## Mokymosi tikslai

Po šios sesijos jūs:
- Įvaldysite pažangią OpenAI SDK integraciją su Foundry Local ir Azure OpenAI
- Įgyvendinsite srautinius atsakymus, kad pagerintumėte naudotojo patirtį
- Sukursite patikimus klientų kūrimo modelius, palaikančius kelis tiekėjus
- Kursite pokalbių valdymo sistemas su konteksto išsaugojimu
- Nustatysite našumo vertinimo ir sveikatos stebėjimo sistemas
- Diegsite gamybai paruoštas programas su tinkamu klaidų tvarkymu

## Reikalavimai

- Baigta 1 sesija: Darbo pradžia su Foundry Local
- Aktyvi Foundry Local instaliacija su veikiančiais modeliais
- Python 3.8 ar naujesnė versija su virtualios aplinkos galimybėmis
- Įdiegtas OpenAI Python SDK (`pip install openai foundry-local-sdk`)
- Azure paskyra su OpenAI paslauga (neprivaloma, debesų scenarijams)
- Pagrindinės Python async/await modelio žinios

## 1 dalis: OpenAI SDK klientų kūrimo modelis

### Daugiafunkcinės architektūros supratimas

Programų kūrimas, kurios veikia tiek su Foundry Local, tiek su Azure OpenAI, reikalauja lankstaus klientų kūrimo modelio:
...
## 2 dalis: Srautinių atsakymų ir realaus laiko sąveika

### Srautinių pokalbių užbaigimų įgyvendinimas

Srautinis atsakymų pateikimas suteikia geresnę naudotojo patirtį, rodant atsakymus jų generavimo metu:
...
### Daugkartinių pokalbių valdymas
...
## 3 dalis: Našumo vertinimas ir analizė

### Atsakymo laiko matavimas

Matavimas ir našumo palyginimas skirtingose konfigūracijose:
...
### Temperatūros ir parametrų testavimas
...
## 4 dalis: Paslaugos sveikatos stebėjimas ir diagnostika

### Išsami sveikatos patikros sistema
...
### Aplinkos konfigūracijos tikrintuvas
...
## 5 dalis: Aplinkos kintamieji ir konfigūracijos valdymas

### Aplinkos kintamųjų nuoroda

Pilna nuoroda, kaip konfigūruoti abu tiekėjus:
...
**Windows Command Prompt nustatymas:**
...
## 6 dalis: Praktinės užduotys

### Užduotis 1: Daugiafunkcinė SDK integracija

Sukurkite pilną programą, kuri sklandžiai perjungia tiekėjus:
...
### Užduotis 2: Pažangus pokalbių valdymas
...
### Užduotis 3: Gamybai paruoštas sveikatos stebėjimas
...
## 7 dalis: Apibendrinimas ir tolesni žingsniai

### Sesijos pasiekimai

Šioje sesijoje jūs įvaldėte:
- ✅ **OpenAI SDK integraciją**: Pažangūs modeliai tiek Foundry Local, tiek Azure OpenAI
- ✅ **Srautinius atsakymus**: Realaus laiko pokalbių užbaigimai, gerinantys naudotojo patirtį
- ✅ **Daugiafunkcinį palaikymą**: Sklandus perjungimas tarp lokalių ir debesų AI paslaugų
- ✅ **Pokalbių valdymą**: Konteksto išsaugojimu pagrįsti daugkartiniai pokalbiai
- ✅ **Našumo stebėjimą**: Vertinimas ir sveikatos patikra gamybos diegimams
- ✅ **Gamybos modelius**: Patikimas klaidų tvarkymas ir konfigūracijos valdymas

### Pagrindiniai architektūros modeliai

**Klientų kūrimo modelis:**
```
...
1. **🔄 Visada įgyvendinkite atsarginius sprendimus**: Azure → Foundry Local → Klaidų tvarkymas
2. **🌊 Naudokite srautinius atsakymus ilgiems atsakymams**: Geresnis suvokiamas našumas
3. **🛡️ Įgyvendinkite išsamų klaidų tvarkymą**: Draugiški klaidų pranešimai
4. **📈 Stebėkite našumą**: Sekite atsakymo laikus ir sėkmės rodiklius
5. **⚙️ Konfigūracija pagal aplinką**: Lengvas perjungimas tarp dev/staging/prod
6. **🔒 Saugus kredencialų valdymas**: Niekada nekoduokite API raktų

### Tiekėjo pasirinkimo gairės

| Scenarijus | Rekomenduojamas tiekėjas | Priežastis |
|------------|--------------------------|------------|
| **Vystymas** | Foundry Local | Greita iteracija, nėra API kaštų |
| **Privatumo svarba** | Foundry Local | Duomenys niekada nepalieka įrenginio |
| **Didelės apimties gamyba** | Azure OpenAI | Geresnis mastelis, įmonės SLA |
| **Naujausi modeliai** | Azure OpenAI | Prieiga prie naujausių modelių |
| **Darbas be interneto** | Foundry Local | Nėra priklausomybės nuo interneto |
| **Kaštų jautrumas** | Foundry Local | Nėra mokesčių už žodžius |

### Aplinkos kintamųjų greita nuoroda
...
### Pasiruošimas 3 sesijai: Atvirojo kodo modeliai

1. **Tyrinėkite modelių katalogą**: Peržiūrėkite Foundry Local galimus modelius
2. **Supraskite modelių formatus**: Sužinokite apie ONNX, kvantizaciją ir optimizaciją
3. **Apsvarstykite pritaikytus modelius**: Pagalvokite apie specifinius modelius jūsų sričiai
...
- **[OpenAI Python SDK dokumentacija](https://github.com/openai/openai-python)**: Oficialus SDK vadovas
- **[Azure OpenAI dokumentacija](https://learn.microsoft.com/azure/ai-services/openai/)**: Azure OpenAI paslaugos vadovas
- **[Foundry Local SDK nuoroda](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**: Lokalaus inferencijos dokumentacija
- **[Srautinių užbaigimų vadovas](https://learn.microsoft.com/azure/ai-foundry/foundry-local/how-to/integrate-with-inference-sdks)**: Pažangūs srautinių modelių pavyzdžiai
- **[Pavyzdys 01: Greitas pokalbis per OpenAI SDK](samples/01/README.md)**: Pagrindiniai integracijos modeliai
- **[Pavyzdys 02: Pažangi SDK integracija](samples/02/README.md)**: Šios sesijos praktiniai pavyzdžiai
- **[Pavyzdys 04: Chainlit programa](samples/04/README.md)**: Web UI kūrimas
- **[Pavyzdys 05: Daugiaagentės sistemos](samples/05/README.md)**: Pažangūs orkestracijos modeliai

Dabar esate pasiruošę kurti sudėtingas AI programas, kurios sklandžiai integruoja lokalias ir debesų AI galimybes, suteikdamos lankstumą pasirinkti tinkamą tiekėją kiekvienam konkrečiam naudojimo atvejui, išlaikant nuoseklius kūrimo modelius.
