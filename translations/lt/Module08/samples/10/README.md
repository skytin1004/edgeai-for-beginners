...
Šis pavyzdys parodo, kaip Foundry Local modelius pateikti kaip daugkartinio naudojimo įrankius, kuriuos galima integruoti į esamas programas, darbo eigas ir kūrimo aplinkas. Jame pristatomi Microsoft rekomenduojami įrankių integracijos ir funkcijų kvietimo modeliai.

## Pagrindinės sąvokos

### 🔧 **Įrankių pirmumo architektūra**
- Foundry Local modeliai kaip kviečiamos funkcijos
- Standartizuotos įrankių sąsajos ir schemos
- Sklandi integracija su esamais kodų pagrindais
- Tipų saugūs įrankių apibrėžimai ir validacija

### ⚡ **Funkcijų kvietimo modeliai**
- Microsoft Foundry Local funkcijų kvietimo įgyvendinimas
- OpenAI suderinami įrankių apibrėžimai
- Automatinė parametrų validacija ir konvertavimas
- Klaidų tvarkymas ir atsakymų formatavimas

### 🔌 **Integracijos pagrindai**
- **LangChain integracija**: Natūrali LangChain įrankių palaikymas
- **Semantic Kernel**: Microsoft Semantic Kernel funkcijos
- **REST API**: HTTP pagrindu veikiantys įrankių galiniai taškai
- **CLI įrankiai**: Komandinės eilutės sąsajos integracija
- **Jupyter Notebooks**: Interaktyvūs kūrimo įrankiai

### 🎯 **Naudojimo scenarijų modeliai**
- Kodo analizės ir generavimo įrankiai
- Turinio apdorojimas ir santrauka
- Duomenų analizė ir vizualizacija
- Tyrimai ir informacijos paieška
...
- **Node.js**: v18+ (JavaScript integracijoms)
- **Atmintis**: Rekomenduojama 12GB+
- **Saugykla**: 10GB+ modeliams ir įrankiams
...
### Pagrindų specifinės priklausomybės
```bash
...
### 1. Funkcijomis pagrįsti įrankiai
```python
...
### Komandinės eilutės integracija
```python
import typer
...
1. **Įrankių integracijos modelius**
   - Funkcijomis ir klasėmis pagrįstą įrankių dizainą
   - Microsoft Foundry Local integracijos modelius
   - Schemos validaciją ir tipų saugumą
   - Klaidų tvarkymą ir atkūrimą

2. **Pagrindų integraciją**
   - LangChain įrankių kūrimą
   - Semantic Kernel funkcijų integraciją
   - REST API pagrindų integraciją
   - CLI programų kūrimą

3. **Produkcijos aspektus**
   - Našumo optimizavimo strategijas
   - Talpyklos ir išteklių valdymą
   - Stebėjimą ir stebimumą
   - Saugumą ir validaciją

4. **Pažangius įrankių modelius**
   - Įrankių sudėtį ir grandines
   - Kontekstui jautrų apdorojimą
   - Grupinius ir srautinius veiksmus
   - Individualių integracijų kūrimą

## Tolimesni žingsniai

- **Integracijos projektai**: Kurkite individualias integracijas su jūsų pasirinktais pagrindais
- **Įrankių kūrimas**: Kurkite specializuotus įrankius savo sričiai
- **Našumo derinimas**: Optimizuokite pagal savo specifinius naudojimo scenarijus
- **Produkcijos diegimas**: Skalizuokite įrankius verslo naudojimui

## Prisidėjimas

Žr. pagrindinio saugyklos gaires dėl prisidėjimo instrukcijų.
