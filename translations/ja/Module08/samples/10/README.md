...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T10:42:51+00:00",
  "source_file": "Module08/samples/10/README.md",
  "language_code": "ja"
}
-->
# Foundry Localをツールとして統合

Microsoft Foundry Localを呼び出し可能なツールとして、より大規模なアプリケーションに統合するための包括的なフレームワーク。Microsoftの公式なツールベースAI統合パターンに従っています。

## 概要

このサンプルは、Foundry Localモデルを再利用可能なツールとして公開し、既存のアプリケーション、ワークフロー、開発環境に統合する方法を示しています。Microsoftが推奨するツール統合と関数呼び出しのパターンを紹介します。

## 主要なコンセプト

### 🔧 **ツール優先のアーキテクチャ**
- Foundry Localモデルを呼び出し可能な関数として利用
- 標準化されたツールインターフェースとスキーマ
- 既存のコードベースとのシームレスな統合
- 型安全なツール定義と検証

### ⚡ **関数呼び出しパターン**
- Microsoft Foundry Localの関数呼び出し実装
- OpenAI互換のツール定義
- 自動パラメータ検証と変換
- エラーハンドリングとレスポンスフォーマット

### 🔌 **統合フレームワーク**
- **LangChain統合**: ネイティブなLangChainツールサポート
- **Semantic Kernel**: Microsoft Semantic Kernel関数
- **REST API**: HTTPベースのツールエンドポイント
- **CLIツール**: コマンドラインインターフェース統合
- **Jupyter Notebooks**: インタラクティブな開発ツール
...
### システム要件
- **Python**: 3.9以上、asyncioサポート付き
- **Node.js**: v18以上（JavaScript統合用）
- **メモリ**: 推奨12GB以上
- **ストレージ**: モデルとツール用に10GB以上
...
このサンプルを完了すると、以下の内容を理解できます：

1. **ツール統合パターン**
   - 関数ベースおよびクラスベースのツール設計
   - Microsoft Foundry Local統合パターン
   - スキーマ検証と型安全性
   - エラーハンドリングと回復
...
- **統合プロジェクト**: 好みのフレームワークでカスタム統合を構築
- **ツール開発**: 専門分野向けのツールを作成
- **パフォーマンス調整**: 特定のユースケースに最適化
- **本番展開**: エンタープライズ用途にツールをスケール

## コントリビューション

コントリビューションの手順については、メインリポジトリのガイドラインを参照してください。

## ライセンス

このサンプルは、Microsoft Foundry Localプロジェクトと同じライセンスに従います。
