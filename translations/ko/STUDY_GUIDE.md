...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-26T08:02:00+00:00",
...
EdgeAI 초보자 학습 가이드에 오신 것을 환영합니다! 이 문서는 학습 자료를 효과적으로 활용하고 학습 경험을 극대화할 수 있도록 설계되었습니다. 구조화된 학습 경로, 추천 학습 일정, 주요 개념 요약, 그리고 Edge AI 기술에 대한 이해를 심화할 수 있는 추가 자료를 제공합니다.

이 과정은 20시간의 간결한 학습으로 EdgeAI에 대한 필수 지식을 효율적으로 전달하며, 바쁜 직장인과 학생들이 이 떠오르는 분야에서 실용적인 기술을 빠르게 습득할 수 있도록 돕습니다.

## 과정 개요

이 과정은 다음의 8개 모듈로 구성되어 있습니다:
...
- Edge AI가 무엇인지, 그리고 오늘날 기술 환경에서 왜 중요한지 이해하기
...
- **우선 개념**: 
  - Edge AI와 기존 클라우드 AI 처리의 차이점
  - 하드웨어, 모델 최적화, 비즈니스 요구의 융합
  - 실시간, 개인정보 보호, 비용 효율적인 AI 배포

#### 섹션 2: 산업 응용
- **우선 개념**: 
  - 제조 및 산업 4.0: 예측 유지보수 및 품질 관리
...
5. 이 과정에서 개발할 주요 기술적 역량은 무엇인가요?
6. 이 과정에서 사용되는 네 단계 학습 접근법을 설명하세요.

### 실습 과제

1. **산업 연구**: 한 가지 산업 응용을 선택하고 실제 Edge AI 구현 사례를 조사하세요 (30분)
2. **모델 탐색**: Hugging Face에서 제공되는 소형 언어 모델을 찾아보고 파라미터 수와 기능을 비교하세요 (30분)
...
- [엣지 컴퓨팅 기초](https://www.edgecomputing.org/)
...
1. **빠른 환경 설정**: 필수 패키지를 포함한 최소 개발 환경을 설정하세요 (30분)
...
5. 연결이 제한된 모바일 애플리케이션의 경우, 어떤 모델 계열이 가장 적합하며 그 이유는 무엇인가요?

### 실습 과제

1. **모델 비교**: 두 가지 SLM 모델의 간단한 벤치마크 수행 (1시간)  
2. **간단한 텍스트 생성**: 작은 모델을 사용하여 텍스트 생성 기본 구현 (1시간)  
...
2. **성능 확인**: 배포된 모델의 간단한 벤치마크 실행 (30분)  
...
- 여섯 가지 프레임워크에서 최적화 구현 (Llama.cpp, Olive, OpenVINO, MLX, 워크플로우 합성)  
...
2. 엣지 배포를 위한 GGUF 형식의 장점은 무엇인가요?  
...
6. 워크플로 합성이 최적의 최적화 프레임워크 선택에 어떻게 도움이 되나요?  
...
- 엔터프라이즈급 SLM 운영 및 유지 관리 워크플로 구축  
...
- Visual Studio Code용 AI Toolkit을 활용하여 포괄적인 EdgeAI 개발 워크플로 숙달  
...
1. AI Toolkit이 EdgeAI 개발 워크플로를 어떻게 간소화하나요?  
...
6. 프라이버시 민감한 애플리케이션에 대해 로컬 배포와 클라우드 배포의 이점을 비교하세요.  

### 실습 과제

1. **AI Toolkit 설정**: AI Toolkit을 구성하고 모델 최적화 (1시간)  
2. **Windows AI Foundry**: Phi Silica API를 사용하여 간단한 Windows AI 애플리케이션 구축 (1시간)  
3. **크로스 플랫폼 배포**: 동일한 모델을 두 가지 다른 플랫폼에 배포 (1시간)  
4. **NPU 최적화**: Windows AI Foundry 도구를 사용하여 NPU 성능 테스트 (30분)  
...
2. 코디네이터 패턴이 전문 에이전트를 어떻게 조율하나요?  
...
6. 현대화된 접근 방식과 기존 구현 패턴의 이점을 비교하세요.  

### 실습 과제

1. **현대적 SDK 설정**: 자동 서비스 검색을 포함한 FoundryLocalManager 구성 (30분)  
2. **다중 에이전트 시스템**: 전문 에이전트를 활용한 고급 코디네이터 실행 (30분)  
3. **지능형 라우팅**: 다양한 쿼리 유형으로 모델 라우터 테스트 (30분)  
4. **대화형 탐색**: Jupyter 노트북을 사용하여 고급 기능 탐색 (45분)  
5. **프로덕션 배포**: 모니터링 및 오류 처리 패턴 구현 (30분)  
6. **하이브리드 통합**: Azure AI Foundry 폴백 시나리오 구성 (30분)  

## 시간 할당 가이드

20시간의 과정 타임라인을 최대한 활용할 수 있도록, 다음은 시간 할당에 대한 제안입니다:
...
| 미니 프로젝트 | 3시간 | 소규모 실질적 구현을 통해 지식 적용 |

### 시간 제약에 따른 주요 집중 영역

**시간이 10시간밖에 없다면:**
- 모듈 0(소개)와 모듈 1, 2, 3(핵심 EdgeAI 개념)을 완료하세요.
- 각 모듈에서 최소 하나의 실습 과제를 수행하세요.
- 구현 세부사항보다는 핵심 개념 이해에 집중하세요.

**20시간을 전부 할애할 수 있다면:**
- 여덟 개의 모든 모듈(소개 포함)을 완료하세요.
- 각 모듈의 주요 실습 과제를 수행하세요.
- 모듈 7에서 하나의 미니 프로젝트를 완료하세요.
- 최소 2-3개의 추가 자료를 탐구하세요.

**20시간 이상이 있다면:**
- 모든 모듈(소개 포함)을 상세히 완료하고 실습 과제를 수행하세요.
- 여러 개의 미니 프로젝트를 구축하세요.
- 모듈 4에서 고급 최적화 기술을 탐구하세요.
- 모듈 5에서 프로덕션 배포를 구현하세요.

## 필수 자료

이 자료들은 제한된 학습 시간 동안 가장 큰 가치를 제공합니다:

### 필독 문서
- [ONNX Runtime Getting Started](https://onnxruntime.ai/docs/get-started/with-python.html) - 가장 효율적인 모델 최적화 도구
- [Ollama Quick Start](https://github.com/ollama/ollama#get-started) - SLM을 로컬에서 배포하는 가장 빠른 방법
- [Microsoft Phi Model Card](https://huggingface.co/microsoft/phi-2) - 엣지 최적화 모델의 참고 자료
- [OpenVINO Documentation](https://docs.openvino.ai/2025/index.html) - Intel의 종합 최적화 툴킷
- [AI Toolkit for VS Code](https://code.visualstudio.com/docs/intelligentapps/overview) - 통합 EdgeAI 개발 환경
- [Windows AI Foundry](https://docs.microsoft.com/en-us/windows/ai/) - Windows 전용 EdgeAI 개발 플랫폼

### 시간 절약 도구
- [Hugging Face Transformers](https://huggingface.co/docs/transformers/index) - 빠른 모델 접근 및 배포
...
20시간 과정 동안 학습 진행을 추적하기 위해 이 간단한 템플릿을 사용하세요:

| 모듈 | 완료 날짜 | 소요 시간 | 주요 배운 점 |
|------|-----------|-----------|--------------|
| 모듈 0: EdgeAI 소개 | | | |
| 모듈 1: EdgeAI 기본 | | | |
| 모듈 2: SLM 기초 | | | |
...
### 중급 프로젝트 (각 3-4시간)
4. **AI 툴킷 워크플로우**: VS Code AI 툴킷을 사용하여 모델을 처음부터 끝까지 최적화하고 배포하세요.
5. **Windows AI Foundry 애플리케이션**: Phi Silica API와 NPU 최적화를 사용하여 Windows 앱을 만드세요.
6. **크로스 플랫폼 배포**: 동일한 최적화 모델을 Windows(OpenVINO)와 모바일(.NET MAUI)에 배포하세요.
7. **기능 호출 에이전트**: 엣지 시나리오를 위한 기능 호출 기능을 갖춘 AI 에이전트를 구축하세요.

### 고급 통합 프로젝트 (각 4-5시간)
8. **OpenVINO 최적화 파이프라인**: NNCF와 GenAI 툴킷을 사용하여 완전한 모델 최적화를 구현하세요.
9. **SLMOps 파이프라인**: 학습부터 엣지 배포까지의 전체 모델 라이프사이클을 구현하세요.
10. **다중 모델 엣지 시스템**: 엣지 하드웨어에서 협력하는 여러 전문 모델을 배포하세요.
11. **MCP 통합 시스템**: 도구 상호작용을 위한 모델 컨텍스트 프로토콜을 사용하여 에이전트 시스템을 구축하세요.
...
- [EdgeAI for Beginners 저장소](https://github.com/microsoft/edgeai-for-beginners/discussions)의 GitHub 토론
- [Microsoft Tech Community](https://techcommunity.microsoft.com/)
- [Stack Overflow](https://stackoverflow.com/questions/tagged/edge-ai)

## 결론

EdgeAI는 인공지능 구현의 최전선에 있으며, 강력한 기능을 장치에 직접 제공하면서 프라이버시, 지연 시간, 연결성에 대한 중요한 문제를 해결합니다. 이 20시간 과정은 EdgeAI 기술을 즉시 활용할 수 있는 필수 지식과 실질적 기술을 제공합니다.

이 과정은 가장 중요한 개념에 집중하여 간결하게 설계되었으며, 과도한 시간 투자 없이도 빠르게 귀중한 전문성을 얻을 수 있도록 합니다. 간단한 예제를 활용한 실습이 학습 내용을 강화하는 핵심이라는 점을 기억하세요.
