...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-26T19:27:23+00:00",
...
NVIDIA Jetson Orin Nano는 접근 가능한 엣지 AI 컴퓨팅의 혁신을 대표하며, 신용카드 크기의 컴팩트한 폼팩터에서 최대 67 TOPS의 AI 성능을 제공합니다. 이 강력한 엣지 AI 플랫폼은 취미 개발자, 학생, 전문 개발자 모두를 위한 생성형 AI 개발을 민주화합니다.
...
### 응용 분야
Jetson Orin Nano는 비전 트랜스포머, 대형 언어 모델(LLM), 비전-언어 모델을 포함한 최신 생성형 AI 모델 실행에 뛰어납니다. 특히 GenAI 사용 사례에 최적화되어 있으며, 이제 손바닥 크기의 디바이스에서 여러 LLM을 실행할 수 있습니다. 주요 사용 사례로는 AI 기반 로봇, 스마트 드론, 지능형 카메라, 자율 엣지 디바이스 등이 있습니다.
...
### 주요 특징
- Android와 iOS 애플리케이션을 위한 단일 코드베이스를 제공하는 .NET MAUI 프레임워크 기반
...
### 개발 혜택
.NET MAUI 접근법은 개발자가 기존의 C# 및 .NET 기술을 활용하여 크로스 플랫폼 AI 애플리케이션을 구축할 수 있도록 합니다. ONNX Runtime GenAI 프레임워크는 Llama, Mistral, Phi, Gemma 등 다양한 모델 아키텍처를 지원합니다. ARM64 커널 최적화는 INT4 양자화된 행렬 곱셈을 가속화하여 모바일 하드웨어에서 효율적인 성능을 제공하며, 익숙한 .NET 개발 경험을 유지합니다.

### 사용 사례
이 솔루션은 .NET 기술을 사용하여 AI 기반 모바일 애플리케이션을 구축하려는 개발자에게 적합합니다. 예를 들어, 지능형 챗봇, 이미지 인식 앱, 언어 번역 도구, 개인화된 추천 시스템 등 디바이스에서 완전히 실행되어 프라이버시를 강화하고 오프라인 기능을 제공하는 애플리케이션을 포함합니다.
...
Microsoft Foundry Local은 ONNX Runtime으로 구동되는 OpenAI 호환 REST 서버를 제공하여 Windows에서 로컬로 모델을 실행할 수 있도록 합니다. 아래는 간단한 요약이며, 자세한 내용은 공식 문서를 참조하세요.
...
REST를 통해 모델 목록 확인(상태에서 PORT 교체):
```cmd
...
Windows 플랫폼을 대상으로 하는 개발자를 위해 Windows EdgeAI 생태계를 완전히 다루는 종합 가이드를 제작했습니다. 이 리소스는 Windows AI Foundry에 대한 자세한 정보, API, 도구, Windows에서의 EdgeAI 개발을 위한 모범 사례를 제공합니다.
...
이 모든 플랫폼의 공통점은 AI 기능의 민주화로, 다양한 기술 수준과 사용 사례에 걸쳐 개발자에게 정교한 머신러닝을 접근 가능하게 만든다는 점입니다. 모바일 애플리케이션, 데스크톱 소프트웨어, 임베디드 시스템을 구축하든, 이러한 EdgeAI 솔루션은 엣지에서 효율적이고 프라이버시를 유지하며 작동하는 차세대 지능형 애플리케이션을 위한 기반을 제공합니다.
