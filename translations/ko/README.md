...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-15T16:59:53+00:00",
...
**초보자를 위한 EdgeAI**에 오신 것을 환영합니다. 이 과정은 Edge 인공지능의 혁신적인 세계로 여러분을 안내하며, 강력한 AI 기능과 실질적인 엣지 디바이스 배포 간의 격차를 연결합니다. 데이터를 생성하고 결정을 내려야 하는 바로 그곳에서 AI의 잠재력을 활용할 수 있도록 돕습니다.

### 학습 목표

이 과정은 기본 개념부터 실제 구현까지 다루며 다음을 포함합니다:
- **엣지 배포에 최적화된 소형 언어 모델(SLMs)**
- **다양한 플랫폼에서 하드웨어 중심 최적화**
- **실시간 추론** 및 개인정보 보호 기능
...
- **개인정보 및 보안**: 민감한 데이터를 클라우드에 노출하지 않고 로컬에서 처리
...
Phi-4, Mistral-7B, Gemma와 같은 SLMs는 더 큰 LLMs의 최적화된 버전으로, 다음을 위해 훈련되거나 증류되었습니다:
...
**주제**: SLM의 전체 라이프사이클 배포, 이론부터 생산 환경까지
...
- [**섹션 4: OpenVINO Toolkit 최적화 스위트**](./Module04/04.openvino.md)  
  - Intel의 크로스 플랫폼 AI 배포를 위한 오픈 소스 툴킷  
  - 고급 최적화를 위한 Neural Network Compression Framework (NNCF)  
  - 대형 언어 모델 배포를 위한 OpenVINO GenAI  
  - CPU, GPU, VPU 및 AI 가속기 전반의 하드웨어 가속  
...
- [**섹션 6: 엣지 AI 개발 워크플로우 통합**](./Module04/06.workflow-synthesis.md)  
  - 여러 최적화 프레임워크를 통합한 통합 워크플로우 아키텍처  
...
**주제**: 증류부터 생산 배포까지 SLM의 전체 라이프사이클 운영
...
- [**섹션 1: SLMOps 소개**](./Module05/01.IntroduceSLMOps.md)  
  ...
  - 전략적 비즈니스 영향 및 경쟁 우위  
  - 실제 구현 과제 및 해결책  
- [**Section 2: 모델 증류 - 이론에서 실무까지**](./Module05/02.SLMOps-Distillation.md)
  - 교사 모델에서 학생 모델로의 지식 전이
  - 두 단계로 이루어진 증류 프로세스 구현
  - Azure ML 증류 워크플로우와 실용적인 예제
  - 85% 추론 시간 단축과 92% 정확도 유지

- [**Section 3: 파인튜닝 - 특정 작업에 맞춘 모델 커스터마이징**](./Module05/03.SLMOps-Finetuing.md)
  - 파라미터 효율적인 파인튜닝(PEFT) 기술
  - LoRA 및 QLoRA 고급 방법
  - Microsoft Olive를 활용한 파인튜닝 구현
  - 멀티 어댑터 학습 및 하이퍼파라미터 최적화

- [**Section 4: 배포 - 프로덕션 준비 완료 구현**](./Module05/04.SLMOps.Deployment.md)
...
### [Module 06: SLM 에이전트 시스템 - AI 에이전트와 함수 호출](./Module06/README.md)
...
- [**Section 3: 모델 컨텍스트 프로토콜(MCP) 통합**](./Module06/03.IntroduceMCP.md)
  - 프로토콜 아키텍처 및 계층 시스템 설계
  - 멀티 백엔드 지원 (개발용 Ollama, 프로덕션용 vLLM)
...
- [**Windows ML을 활용한 EdgeAI**](./Module07/README.md#4-edgeai-with-windows-ml)
  - 온디바이스 추론을 위한 Windows AI Foundry 기반
...
이 포괄적인 EdgeAI 과정을 완료하면, 프로덕션 준비가 된 EdgeAI 솔루션을 설계, 구현 및 배포할 수 있는 전문성을 갖추게 됩니다. 체계적인 접근 방식은 이론적 기초와 실무 구현 기술을 모두 마스터할 수 있도록 보장합니다.

### 기술 역량

**기초 지식**
- 클라우드 기반 AI 아키텍처와 엣지 기반 AI 아키텍처의 근본적인 차이를 이해
- 자원 제약 환경을 위한 모델 양자화, 압축 및 최적화 원칙 숙지
- 하드웨어 가속 옵션(NPU, GPU, CPU)과 배포 시 고려 사항 이해

**구현 기술**
- 다양한 엣지 플랫폼(모바일, 임베디드, IoT, 엣지 서버)에서 소형 언어 모델 배포
- Llama.cpp, Microsoft Olive, ONNX Runtime, Apple MLX를 포함한 최적화 프레임워크 적용
- 실시간 추론 시스템 구현 (응답 시간 1초 미만)
...
### Module 07 학습 결과:
- Visual Studio Code용 AI 툴킷을 활용한 포괄적인 Edge AI 개발 워크플로우 숙달
...
**포트폴리오 개발**
- 실용적 역량을 입증하는 종단 간 EdgeAI 프로젝트 완료
...
- **이론과 실무 통합**: 각 모듈은 이론적 기초와 실무 운영을 모두 포함
- **실제 사례 연구**: Microsoft, Alibaba, Google 등의 실제 사례 기반
- **실습**: 구성 파일, API 테스트 절차 및 배포 스크립트 완비
- **성능 벤치마크**: 추론 속도, 메모리 사용량 및 자원 요구사항에 대한 상세 비교
- **엔터프라이즈급 고려사항**: 보안 관행, 준수 프레임워크 및 데이터 보호 전략

## 시작하기

추천 학습 경로:
1. **Module01**부터 시작하여 EdgeAI의 기본 이해를 구축
2. **Module02**로 진행하여 다양한 SLM 모델 패밀리를 깊이 이해
3. **Module03**을 학습하여 실무 배포 기술을 마스터
4. **Module04**로 계속하여 고급 모델 최적화, 형식 변환 및 프레임워크 통합
5. **Module05**를 완료하여 프로덕션 준비 완료된 SLMOps 구현을 마스터
6. **Module06**을 탐구하여 SLM 에이전트 시스템 및 함수 호출 기능 이해
7. **Module07**을 마무리하여 AI 툴킷과 다양한 EdgeAI 구현 샘플에 대한 실무 경험 획득
...
- **시간 할당 지침**: 읽기, 연습 및 프로젝트 균형을 위한 구체적인 추천
- **핵심 개념 집중**: 각 모듈의 우선 학습 목표
- **자기 평가 도구**: 이해도를 테스트하기 위한 질문 및 연습
- **미니 프로젝트 아이디어**: 학습을 강화하기 위한 실용적 응용

학습 가이드는 집중 학습(1주) 및 파트타임 학습(3주)을 모두 수용할 수 있도록 설계되었으며, 과정에 10시간만 할애할 수 있는 경우에도 효과적으로 시간을 배분하는 방법을 명확히 안내합니다.

---
**EdgeAI의 미래는 모델 아키텍처, 양자화 기술, 그리고 효율성과 전문성을 우선시하는 배포 전략의 지속적인 개선에 달려 있습니다. 이 패러다임 전환을 수용하는 조직은 데이터와 운영을 통제하면서 AI의 변혁적 잠재력을 활용할 수 있는 유리한 위치를 차지하게 될 것입니다.**
...
- [GitHub Copilot을 활용한 C#/.NET 개발자용 마스터하기](https://github.com/microsoft/mastering-github-copilot-for-dotnet-csharp-developers?WT.mc_id=academic-105485-koreyst)
