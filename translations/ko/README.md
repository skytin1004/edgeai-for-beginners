...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-22T07:52:41+00:00",
...
Edge AI 기술에 대한 심층적인 교육 여정을 제공합니다. 이 과정은 EdgeAI 기본 개념, 소형 언어 모델(Small Language Model, SLM) 기초, 실질적인 배포 전략이라는 세 가지 포괄적인 모듈로 구성되어 있습니다. 학습자는 기본 개념에서 고급 구현까지 안내받으며, 실제 사례 연구, 실습 과제, 배포 예제를 통해 AI 모델을 엣지 디바이스에서 효과적으로 실행하여 프라이버시를 강화하고 지연 시간을 줄이며 효율성을 높이는 방법을 배웁니다.
...
1. **저장소 포크하기**: [![GitHub forks](https://img.shields.io/github/forks/microsoft/edgeai-for-beginners.svg?style=social&label=Fork)](https://GitHub.com/microsoft/mcp-for-beginners/fork) 버튼을 클릭하세요.  
2. **저장소 클론하기**: `git clone https://github.com/microsoft/edgeai-for-beginners.git`  
3. [**Azure AI Foundry Discord에 참여하여 전문가 및 다른 개발자들과 교류하세요**](https://discord.com/invite/ByRwuEEgH4)  

### 🌐 다국어 지원

#### GitHub Action을 통한 지원 (자동화 및 항상 최신 상태 유지)

[프랑스어](../fr/README.md) | [스페인어](../es/README.md) | [중국어 (간체)](../zh/README.md) | [중국어 (번체, 마카오)](../mo/README.md) | [중국어 (번체, 홍콩)](../hk/README.md) | [중국어 (번체, 대만)](../tw/README.md) | [일본어](../ja/README.md) | [한국어](./README.md) |  

초보자를 위한 EdgeAI에 오신 것을 환영합니다. 이 과정은 언어 모델의 강력함과 로컬 디바이스의 효율성을 결합합니다. 이 과정에서는 소형 최적화 언어 모델(SLM)이 스마트폰, IoT 보드, 소형 서버와 같은 엣지 하드웨어에서 클라우드 접근 없이 직접 실행될 수 있는 방법을 소개합니다. 실시간, 프라이버시 중심의 AI 추론이 스마트 홈, 산업 모니터링, 오프라인 애플리케이션을 어떻게 혁신하고 있는지 탐구하며, 속도, 보안, 모듈성을 위해 설계된 경량 배포 방식을 다룹니다.

**Edge AI**

Edge AI는 AI 알고리즘과 언어 모델을 로컬 하드웨어에서 실행하여 데이터가 생성되는 가까운 곳에서 추론을 수행하는 것을 의미합니다. 이는 지연 시간을 줄이고 프라이버시를 강화하며 실시간 의사 결정을 가능하게 합니다.

핵심 원칙:
- 디바이스 내 추론: AI 모델이 엣지 디바이스(스마트폰, 라우터, 마이크로컨트롤러, 산업용 PC)에서 실행됩니다.
- 오프라인 기능: 지속적인 인터넷 연결 없이 작동합니다.
- 낮은 지연 시간: 실시간 시스템에 적합한 즉각적인 응답.
- 데이터 주권: 민감한 데이터를 로컬에 유지하여 보안과 규정을 준수합니다.

**소형 언어 모델(SLM)**  
Phi-4, Mistral-7B, Gemma와 같은 SLM은 더 큰 LLM을 최적화하거나 디스틸링하여 다음을 달성합니다:
- 메모리 사용량 감소
- 낮은 연산 요구량
- 빠른 시작 시간
...
### [모듈 01: EdgeAI 기본 개념과 변혁](./Module01/README.md)  
**주제**: 엣지 AI 배포의 변혁적 변화  
...
**주제**: 플랫폼 전반에 걸친 엣지 배포를 위한 완전한 모델 최적화 도구 세트  
...
**주제**: 디스틸링부터 생산 배포까지 SLM 전체 수명 주기 운영  
...
- [**섹션 2: 모델 디스틸링 - 이론에서 실습까지**](./Module05/02.SLMOps-Distillation.md)  
  - 교사 모델에서 학생 모델로의 지식 전이  
  - 2단계 디스틸링 프로세스 구현  
  - Azure ML 디스틸링 워크플로와 실용적인 예제  
  - 85% 추론 시간 단축과 92% 정확도 유지  

- [**섹션 3: 미세 조정 - 특정 작업을 위한 모델 커스터마이징**](./Module05/03.SLMOps-Finetuing.md)  
...
- SLM vs LLM 트레이드오프 분석 (10-30배 비용 절감, 70-80% 작업 효율성)
...
- 모니터링 및 유지 관리 전략을 갖춘 프로덕션 준비 SLM 솔루션 배포

### 모듈 06 학습 결과:
- AI 에이전트 및 소형 언어 모델 아키텍처의 기본 개념 이해
- 다양한 플랫폼과 프레임워크에서의 함수 호출 구현 숙달
- 표준화된 외부 도구 상호작용을 위한 모델 컨텍스트 프로토콜 (MCP) 통합
...
- **실습**: 완전한 구성 파일, API 테스트 절차, 배포 스크립트 포함
...
3. **Module03**을 학습하여 실질적인 배포 기술 숙달
4. **Module04**를 통해 고급 모델 최적화 및 형식 변환 학습
5. **Module05**를 완료하여 프로덕션 준비 구현을 위한 SLMOps 숙달
...
- **구조화된 학습 경로**: 20시간 내에 코스를 완료하기 위한 최적화된 일정
- **시간 할당 지침**: 읽기, 연습, 프로젝트 균형을 위한 구체적인 추천
- **핵심 개념 집중**: 각 모듈의 우선 학습 목표
- **자기 평가 도구**: 이해도를 테스트하기 위한 질문 및 연습
- **미니 프로젝트 아이디어**: 학습을 강화하기 위한 실질적 응용

학습 가이드는 집중 학습(1주) 및 파트타임 학습(3주)을 모두 수용할 수 있도록 설계되었으며, 코스에 10시간만 할애할 수 있는 경우에도 효과적으로 시간을 배분하는 방법을 명확히 안내합니다.

---

**EdgeAI의 미래는 모델 아키텍처, 양자화 기술, 배포 전략의 지속적인 개선에 달려 있습니다. 효율성과 전문화를 일반 목적 능력보다 우선시하는 이 패러다임 전환을 수용하는 조직은 데이터와 운영을 제어하면서 AI의 변혁적 잠재력을 활용할 수 있는 좋은 위치에 있게 될 것입니다.**
