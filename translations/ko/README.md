...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-17T12:14:11+00:00",
...
**EdgeAI for Beginners**에 오신 것을 환영합니다. 이 과정은 Edge 인공지능의 혁신적인 세계로 여러분을 안내하며, 강력한 AI 기능과 실제적인 엣지 디바이스 배포 간의 격차를 연결합니다. 데이터를 생성하고 결정을 내려야 하는 바로 그곳에서 AI의 잠재력을 활용할 수 있도록 돕습니다.

### 학습 목표

이 과정은 기본 개념부터 실무 준비가 된 구현까지 다루며 다음을 포함합니다:
...
Edge AI는 AI 알고리즘과 언어 모델을 하드웨어에서 로컬로 실행하여 데이터가 생성되는 가까운 곳에서 클라우드 리소스를 사용하지 않고 추론을 수행하는 것을 의미합니다. 이는 지연 시간을 줄이고 개인정보를 강화하며 실시간 의사 결정을 가능하게 합니다.
...
- [**섹션 1: SLM 고급 학습**](./Module03/01.SLMAdvancedLearning.md)
  ...
  - 모델 획득 전략 (Phi 모델을 위한 Azure AI Foundry, Hugging Face에서 선택된 모델)
...
### [Module 06: SLM 에이전트 시스템 - AI 에이전트 및 함수 호출](./Module06/README.md)
**주제**: SLM 에이전트 시스템 구현, 기초부터 고급 함수 호출 및 Model Context Protocol 통합

#### 챕터 구조:
- [**Section 1: AI 에이전트와 소형 언어 모델 기초**](./Module06/01.IntroduceAgent.md)
  - 에이전트 분류 프레임워크 (반사형, 모델 기반, 목표 기반, 학습 에이전트)
  - SLM 기본 원리 및 최적화 전략 (GGUF, 양자화, 엣지 프레임워크)
  - SLM vs LLM 트레이드오프 분석 (10-30× 비용 절감, 70-80% 작업 효율성)
  - Ollama, VLLM, Microsoft 엣지 솔루션을 활용한 실용적 배포

- [**Section 2: 소형 언어 모델에서의 함수 호출**](./Module06/02.FunctionCalling.md)
  - 체계적인 워크플로 구현 (의도 감지, JSON 출력, 외부 실행)
  - 플랫폼별 구현 (Phi-4-mini, 선택된 Qwen 모델, Microsoft Foundry Local)
  - 고급 예제 (다중 에이전트 협업, 동적 도구 선택)
  - 생산 고려 사항 (속도 제한, 감사 로그, 보안 조치)

- [**Section 3: Model Context Protocol (MCP) 통합**](./Module06/03.IntroduceMCP.md)
...
### [Module 07: EdgeAI 구현 샘플](./Module07/README.md)
**주제**: 다양한 플랫폼과 프레임워크를 활용한 포괄적인 EdgeAI 구현

#### 챕터 구조:
- [**Visual Studio Code를 위한 AI Toolkit**](./Module07/aitoolkit.md)
  - VS Code 내에서의 포괄적인 Edge AI 개발 환경
  - 엣지 배포를 위한 모델 카탈로그 및 검색
  - 로컬 테스트, 최적화 및 에이전트 개발 워크플로
  - 엣지 시나리오를 위한 성능 모니터링 및 평가
...
- [**Windows ML을 활용한 EdgeAI**](./Module07/README.md#4-edgeai-with-windows-ml)
  ...
  - AMD, Intel, NVIDIA, Qualcomm 실리콘을 포함한 범용 하드웨어 지원
  - 자동 하드웨어 추상화 및 최적화
...
이 포괄적인 EdgeAI 과정을 완료하면, 생산 준비된 EdgeAI 솔루션을 설계, 구현 및 배포할 수 있는 전문성을 갖추게 됩니다. 체계적인 접근 방식은 이론적 기초와 실용적 구현 기술을 모두 마스터하도록 보장합니다.
...
- 하드웨어 가속 옵션 (NPUs, GPUs, CPUs) 및 배포 영향 이해
...
- 프라이버시를 보존하는 EdgeAI 구현을 위한 보안 모범 사례 적용
...
- 특정 배포 제약 조건에 따라 적합한 SLM 패밀리 및 구성 선택

**시스템 아키텍처**
- 기존 인프라와 통합된 종합적인 EdgeAI 솔루션 설계
...
- **헬스케어**: 프라이버시를 보존하는 진단 도구 및 환자 모니터링 시스템
- **운송**: 자율 차량 의사 결정 및 교통 관리
...
- 실제 응용 사례 및 성공 사례 인식
- EdgeAI 솔루션 구현을 위한 실용적 기술 습득

### Module 02 학습 결과:
- 다양한 SLM 설계 철학과 배포 영향에 대한 깊은 이해
- 계산 제약 및 성능 요구 사항에 기반한 전략적 의사 결정 능력 숙달
- 배포 유연성 트레이드오프 이해
...
### Module 04 학습 결과:
- 양자화 경계 및 실용적 응용에 대한 깊은 이해
- 여러 최적화 프레임워크 (Llama.cpp, Olive, OpenVINO, MLX)에 대한 실습 경험
- OpenVINO 및 NNCF를 활용한 Intel 하드웨어 최적화 숙달
- 다양한 플랫폼에서 하드웨어 인식 최적화 선택 능력
- 크로스 플랫폼 엣지 컴퓨팅 환경을 위한 생산 배포 기술
- 최적의 Edge AI 솔루션을 위한 전략적 프레임워크 선택 및 워크플로 종합

### Module 05 학습 결과:
- SLMOps 패러다임 및 운영 원칙 숙달
- 지식 전이 및 효율성 최적화를 위한 모델 증류 구현
- 도메인별 모델 맞춤화를 위한 미세 조정 기술 적용
- 모니터링 및 유지보수 전략을 포함한 생산 준비 SLM 솔루션 배포

### Module 06 학습 결과:
- AI 에이전트 및 소형 언어 모델 아키텍처의 기초 개념 이해
- 여러 플랫폼 및 프레임워크에서 함수 호출 구현 숙달
- 외부 도구 상호작용을 표준화하기 위한 Model Context Protocol (MCP) 통합
- 최소한의 인간 개입 요구 사항으로 정교한 에이전트 시스템 구축

### Module 07 학습 결과:
- 포괄적인 Edge AI 개발 워크플로를 위한 Visual Studio Code용 AI Toolkit 숙달
...
- 다양한 생태계에서 실제 EdgeAI 애플리케이션을 구축하기 위한 실용적 기술 개발
...
- **생산 운영**: 엔터프라이즈급 신뢰성을 갖춘 EdgeAI 시스템 유지 및 확장

**산업 전문성**
- **기술 평가**: 특정 비즈니스 과제를 위한 EdgeAI 솔루션 평가 및 추천
- **구현 계획**: EdgeAI 프로젝트를 위한 현실적인 일정 및 자원 요구 사항 개발
- **위험 관리**: EdgeAI 배포에서 기술 및 운영 위험 식별 및 완화
...
**포트폴리오 개발**
- 실용적 역량을 입증하는 엔드 투 엔드 EdgeAI 프로젝트 완료
- 여러 하드웨어 플랫폼에서 생산 준비 솔루션 배포
- 달성한 최적화 전략 및 성능 개선 문서화
...
- **점진적 학습**: 기본 개념에서 고급 배포까지 점진적으로 학습
- **이론과 실습 통합**: 각 모듈은 이론적 기초와 실습을 모두 포함
- **실제 사례 연구**: Microsoft, Alibaba, Google 등 실제 사례 기반
- **실습 중심 학습**: 구성 파일 작성, API 테스트 절차, 배포 스크립트 완성
- **성능 벤치마크**: 추론 속도, 메모리 사용량, 자원 요구사항에 대한 상세 비교
- **기업 수준 고려사항**: 보안 관행, 준수 프레임워크, 데이터 보호 전략

## 시작하기

추천 학습 경로:
1. **Module01**로 시작하여 EdgeAI의 기본 이해를 구축하세요.
2. **Module02**로 진행하여 다양한 SLM 모델 계열을 깊이 이해하세요.
3. **Module03**을 학습하여 실질적인 배포 기술을 마스터하세요.
4. **Module04**를 통해 고급 모델 최적화, 형식 변환, 프레임워크 통합을 배우세요.
5. **Module05**를 완료하여 SLMOps를 활용한 실무 준비 구현을 마스터하세요.
6. **Module06**을 탐구하여 SLM 에이전트 시스템과 함수 호출 기능을 이해하세요.
7. **Module07**을 마무리하여 AI Toolkit과 다양한 EdgeAI 구현 샘플을 통해 실질적인 경험을 쌓으세요.

각 모듈은 독립적으로 완결되도록 설계되었지만, 순차적으로 학습하면 최상의 결과를 얻을 수 있습니다.
...
- **시간 배분 가이드**: 읽기, 연습, 프로젝트 간의 균형을 맞추기 위한 구체적인 추천
- **핵심 개념 집중**: 각 모듈의 우선 학습 목표
- **자기 평가 도구**: 이해도를 테스트하기 위한 질문과 연습
- **미니 프로젝트 아이디어**: 학습을 강화하기 위한 실질적인 응용

학습 가이드는 집중 학습(1주)과 파트타임 학습(3주) 모두를 수용하도록 설계되었으며, 하루 10시간만 투자할 수 있는 경우에도 효과적으로 시간을 배분할 수 있는 명확한 지침을 제공합니다.

---

**EdgeAI의 미래는 모델 아키텍처, 양자화 기술, 배포 전략의 지속적인 개선에 달려 있으며, 이는 범용 기능보다 효율성과 전문화를 우선시합니다. 이러한 패러다임 전환을 수용하는 조직은 AI의 변혁적 잠재력을 활용하면서 데이터와 운영에 대한 통제력을 유지할 수 있는 유리한 위치에 설 것입니다.**

## 기타 과정

저희 팀은 다른 과정도 제공합니다! 확인해 보세요:
