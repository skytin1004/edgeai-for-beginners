...
이 샘플은 Foundry Local을 사용하여 정교한 AI 에이전트 시스템을 구축하는 방법을 보여주며, 함수 호출, 에이전트 오케스트레이션, 협업 AI 워크플로우에 대한 Microsoft의 공식 패턴을 구현합니다.
...
### 🔧 **전문화된 에이전트 유형**
- **Code Expert**: 프로그래밍, 디버깅, 코드 리뷰
- **Data Analyst**: 데이터 처리, 시각화, 인사이트 도출
- **Research Assistant**: 정보 수집, 요약
- **Writing Specialist**: 콘텐츠 작성, 편집, 문서화
- **Problem Solver**: 복잡한 추론, 의사 결정

### ⚡ **고급 함수 호출**
- Microsoft Foundry Local 함수 호출 패턴
- 타입 안전한 도구 정의
- 자동 매개변수 검증
- 오류 처리 및 복구
- 도구 체이닝 및 구성
...
### 시스템 요구사항
- **Python**: asyncio 지원이 포함된 3.9 이상
- **메모리**: 다중 에이전트를 위해 16GB 이상 권장
- **저장소**: 다중 모델을 위해 15GB 이상
- **CPU/GPU**: 멀티코어 프로세서, GPU 권장
