...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T10:40:06+00:00",
...
Foundry Local을 사용하여 AI 모델을 모듈화되고 맞춤화된 도구로 활용하며, 디바이스에서 직접 실행할 수 있습니다. 이번 세션에서는 개인정보 보호와 저지연 추론을 위한 실용적인 워크플로우와 SDK, API, CLI를 통해 이러한 도구를 통합하는 방법을 강조합니다. 필요 시 Azure AI Foundry로 확장하는 방법도 배울 수 있습니다.

> **🔄 최신 SDK 업데이트**: 이 모듈은 최신 Microsoft Foundry-Local 저장소 패턴에 맞게 조정되었으며, `samples/06/`의 지능형 라우팅 구현과 일치합니다. 예제는 이제 최신 `foundry-local-sdk`와 고급 모델 선택 전략을 사용합니다.
...
목표: 프롬프트 템플릿과 JSON 스키마를 사용하여 도메인에 맞춘 출력 구현.
...
목표: 로컬에서 모델을 도구로 실행할 때 개인정보 보호와 복원력을 보장.
...
> **📋 참고**: `samples/06/model_router.ipynb`의 로컬 구현에는 프로덕션 배포 패턴의 포괄적인 예제가 포함되어 있습니다.

Step 1) 모니터링을 포함한 프로덕션 라우터 (`samples/06/router.py` 참조)  
```python
...
- **고급 패턴**: Module 5에서 함수 호출 및 다중 에이전트 오케스트레이션 탐색

## 마무리

Foundry Local은 모델을 지능적이고 전문화된 도구로 변환하여 디바이스에서 강력한 AI를 구현할 수 있도록 합니다. 자동 모델 선택, 포괄적인 모니터링, 프로덕션 준비 패턴을 통해 팀은 다양한 작업 유형에 적응하면서 개인정보 보호와 성능을 유지하는 정교한 AI 애플리케이션을 배포할 수 있습니다. 여기서 소개된 지능형 라우터 패턴은 로컬 개발에서 프로덕션 배포로 확장 가능한 복잡한 AI 시스템을 구축하기 위한 기반을 제공합니다.
