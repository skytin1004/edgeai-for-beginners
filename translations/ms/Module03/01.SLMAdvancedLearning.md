...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-18T14:57:37+00:00",
...
Small Language Models (SLMs) merupakan kemajuan penting dalam EdgeAI, membolehkan keupayaan pemprosesan bahasa semula jadi yang canggih pada peranti dengan sumber terhad. Memahami cara untuk melaksanakan, mengoptimumkan, dan menggunakan SLM dengan berkesan adalah penting untuk membina penyelesaian AI berasaskan edge yang praktikal.

## Pengenalan

Dalam pelajaran ini, kita akan meneroka Small Language Models (SLMs) dan strategi pelaksanaan lanjutan mereka. Kita akan membincangkan konsep asas SLM, sempadan parameter dan klasifikasi mereka, teknik pengoptimuman, serta strategi pelaksanaan praktikal untuk persekitaran pengkomputeran edge.
...
Small Language Models (SLMs) ialah model AI yang direka untuk memproses, memahami, dan menghasilkan kandungan bahasa semula jadi dengan parameter yang jauh lebih sedikit berbanding model besar mereka. Walaupun Large Language Models (LLMs) mengandungi beratus bilion hingga trilion parameter, SLM direka khusus untuk kecekapan dan pelaksanaan edge.

Rangka kerja klasifikasi parameter membantu kita memahami kategori berbeza SLM dan kesesuaian penggunaannya. Klasifikasi ini penting untuk memilih model yang sesuai bagi senario pengkomputeran edge tertentu.
...
**Keberkesanan Kos**: SLM menawarkan kos latihan dan pelaksanaan yang lebih rendah berbanding LLM, dengan kos operasi yang dikurangkan dan keperluan jalur lebar yang lebih rendah untuk aplikasi edge.
...
**Ciri Penemuan Model**: Platform ini menawarkan penapisan lanjutan berdasarkan jumlah parameter, jenis lesen, dan metrik prestasi. Pengguna boleh mengakses alat perbandingan model secara bersebelahan, penanda aras prestasi masa nyata dan hasil penilaian, serta demo WebGPU untuk ujian segera.

**Koleksi SLM Terkurasi**: Model popular termasuk Phi-4-mini-3.8B untuk tugas penaakulan lanjutan, siri Qwen3 (0.6B/1.7B/4B) untuk aplikasi pelbagai bahasa, Google Gemma3 untuk tugas umum yang cekap, dan model eksperimen seperti BitNET untuk pelaksanaan ultra rendah presisi. Platform ini juga menampilkan koleksi yang didorong oleh komuniti dengan model khusus untuk domain tertentu serta varian yang telah dilatih dan disesuaikan untuk pelbagai kegunaan.

### Katalog Model Azure AI Foundry

Katalog Model Azure AI Foundry menyediakan akses kepada SLM dengan integrasi yang dipertingkatkan untuk perusahaan:

**Integrasi Perusahaan**: Katalog ini termasuk model yang dijual secara langsung oleh Azure dengan sokongan perusahaan dan SLA, menampilkan Phi-4-mini-3.8B untuk keupayaan penaakulan lanjutan dan Llama 3-8B untuk pelaksanaan pengeluaran. Ia juga menampilkan model seperti Qwen3 8B daripada model sumber terbuka pihak ketiga yang dipercayai.
...
**Teknik Pengoptimuman**: Suite ini termasuk kuantisasi dinamik untuk pemilihan presisi automatik (terutamanya berkesan dengan model siri Qwen3), pengoptimuman graf dan penggabungan operator (dioptimumkan untuk seni bina Google Gemma3), pengoptimuman khusus perkakasan untuk CPU, GPU, dan NPU (dengan sokongan khas untuk Phi-4-mini-3.8B pada peranti ARM), dan aliran pengoptimuman berbilang peringkat. Model BitNET memerlukan aliran kerja kuantisasi 1-bit khusus dalam rangka kerja Olive.

**Automasi Aliran Kerja**: Penanda aras automatik merentasi varian pengoptimuman memastikan pemeliharaan metrik kualiti semasa pengoptimuman. Integrasi dengan rangka kerja ML popular seperti PyTorch dan ONNX menyediakan keupayaan pengoptimuman pelaksanaan cloud dan edge.
...
### Rangka Kerja Apple MLX

Apple MLX menyediakan pengoptimuman asli yang direka khusus untuk peranti Apple Silicon:

**Pengoptimuman Apple Silicon**: Rangka kerja ini menggunakan seni bina memori bersatu dengan integrasi Metal Performance Shaders, inferens presisi campuran automatik (terutamanya berkesan dengan Google Gemma3), dan penggunaan lebar jalur memori yang dioptimumkan. Phi-4-mini-3.8B menunjukkan prestasi yang luar biasa pada cip siri M, manakala Qwen3-1.7B menyediakan keseimbangan optimum untuk pelaksanaan MacBook Air.
...
Ollama mempermudahkan pelaksanaan SLM dengan ciri sedia perusahaan untuk persekitaran tempatan dan edge:

**Keupayaan Pelaksanaan**: Pemasangan dan pelaksanaan model dengan satu arahan dengan penarikan dan caching model automatik. Sokongan untuk Phi-4-mini-3.8B, keseluruhan siri Qwen3 (0.6B/1.7B/4B), dan Google Gemma3 dengan REST API untuk integrasi aplikasi serta keupayaan pengurusan dan penukaran model berbilang. Model BitNET memerlukan konfigurasi binaan eksperimen untuk sokongan kuantisasi 1-bit.

**Ciri Lanjutan**: Sokongan penyesuaian model, penjanaan Dockerfile untuk pelaksanaan berbekas, percepatan GPU dengan pengesanan automatik, serta pilihan kuantisasi dan pengoptimuman model menyediakan fleksibiliti pelaksanaan yang komprehensif.
...
**Ciri Pengkomputeran Edge**: Reka bentuk seni bina offline-first dengan pengoptimuman kekangan sumber, pengurusan daftar model tempatan, dan keupayaan penyegerakan edge-to-cloud memastikan pelaksanaan edge yang boleh dipercayai.
...
**Pemilihan Rangka Kerja**: Pilih rangka kerja pengoptimuman berdasarkan perkakasan sasaran dan keperluan pelaksanaan. Gunakan Llama.cpp untuk pelaksanaan yang dioptimumkan CPU, Microsoft Olive untuk aliran kerja pengoptimuman yang komprehensif, dan Apple MLX untuk peranti Apple Silicon.
...
**Penyelidikan dan Eksperimen**: Model BitNET membolehkan penerokaan inferens presisi ultra rendah untuk penyelidikan akademik dan aplikasi bukti konsep yang memerlukan kekangan sumber yang ekstrem.
...
Peranti edge yang berbeza mempunyai keupayaan dan kekangan yang berbeza. Qwen3-0.6B berjalan dengan cekap pada pemproses ARM asas, Google Gemma3 memerlukan sumber pengkomputeran sederhana, dan Phi-4-mini-3.8B mendapat manfaat daripada perkakasan edge yang lebih tinggi. Model BitNET memerlukan perkakasan atau pelaksanaan perisian khusus untuk operasi 1-bit yang optimum.

### Keselamatan dan Privasi

Walaupun SLM membolehkan pemprosesan tempatan untuk peningkatan privasi, langkah keselamatan yang sesuai mesti dilaksanakan untuk melindungi model dan data dalam persekitaran edge. Ini amat penting apabila melaksanakan model seperti Phi-4-mini-3.8B dalam persekitaran perusahaan atau siri Qwen3 dalam aplikasi pelbagai bahasa yang mengendalikan data sensitif.
...
Memahami trend ini dan mengekalkan kesedaran terhadap teknologi yang muncul akan menjadi penting untuk kekal terkini dengan amalan terbaik pembangunan dan pelaksanaan SLM.

## ➡️ Apa yang seterusnya

- [02: Pelaksanaan Praktikal SLM](02.SLMPracticalImplementation.md)
