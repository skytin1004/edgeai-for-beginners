...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-02T11:22:18+00:00",
...
**通用硬件支持**  
Windows ML 提供自动硬件优化，覆盖整个 Windows 生态系统，确保您的 AI 应用程序无论底层芯片架构如何都能实现最佳性能。

**集成的 AI 运行时**  
内置的 Windows ML 推理引擎消除了复杂的设置需求，让开发者专注于应用逻辑，而无需担心基础设施问题。
...
**Windows ML 部署**  
- 将自定义 ONNX 模型引入 Windows 应用程序，使用 Windows ML  
- 利用 CPU、GPU 和 NPU 架构的自动硬件加速  
- 实现资源利用率最优的实时推理  
- 为多种 Windows 设备类别设计可扩展的 AI 应用程序  
...
- 规划数据流架构，既能保持隐私又能实现智能化  
- 实现随用户需求扩展的成本效益 AI 解决方案  

**市场定位**  
- 了解 Windows 原生 AI 应用的竞争优势  
- 确定设备端 AI 提供卓越用户体验的用例  
- 为 AI 增强的 Windows 应用开发市场策略  
- 利用 Windows 生态系统优势定位应用程序  

## Windows App SDK AI 示例

Windows App SDK 提供了全面的示例，展示了 AI 在多种框架和部署场景中的集成。这些示例是理解 Windows AI 开发模式的重要参考。

### Windows AI Foundry 示例

| 示例 | 框架 | 重点领域 | 关键功能 |
|------|------|----------|----------|
| [cs-winui](https://github.com/microsoft/WindowsAppSDK-Samples/tree/main/Samples/WindowsAIFoundry/cs-winui) | C# WinUI 3 | Windows AI API 集成 | 完整的 WinUI 应用，展示 Windows AI API、ARM64 优化、打包部署 |
...
所有示例会自动检测并利用可用硬件：  
- **CPU** - 所有 Windows 设备通用支持  
- **GPU** - 自动检测并优化可用的图形硬件  
- **NPU** - 在支持的设备（如 Copilot+ PC）上利用神经处理单元  

## Windows AI Foundry 平台组件

### 1. Windows AI API

Windows AI API 提供了即用型 AI 功能，由设备端模型驱动，针对 Copilot+ PC 设备进行了效率和性能优化，设置需求极少。
...
Foundry Local 为开发者提供了快速访问 Windows Silicon 上即用型开源语言模型的能力，支持浏览、测试、交互和在本地应用中部署模型。

#### Foundry Local 示例应用

[Foundry Local 仓库](https://github.com/microsoft/Foundry-Local/tree/main/samples) 提供了多种编程语言和框架的全面示例，展示了各种集成模式和用例。
...
| [rust/hello-foundry-local](https://github.com/microsoft/Foundry-Local/tree/main/samples/rust/hello-foundry-local) | Rust | 系统集成 | 低级 SDK 使用、异步操作、reqwest HTTP 客户端 |

#### 按用例分类的示例

**RAG（检索增强生成）**  
- **dotNET/rag**：使用语义内核、Qdrant 向量数据库和 JINA 嵌入实现完整的 RAG  
...
- **Python**：兼容 OpenAI 的 API 异步流式使用  
- **Rust**：使用 reqwest 和 tokio 进行低级异步操作集成  

#### Foundry Local 示例的前置条件  
...
Windows ML 是 Windows 上核心的 AI 平台和集成推理运行时，允许开发者高效地在广泛的 Windows 硬件生态系统中部署自定义模型。  

#### 架构优势  

**通用硬件支持**  
- 自动优化 AMD、Intel、NVIDIA 和 Qualcomm 芯片  
- 支持 CPU、GPU 和 NPU 执行，并可透明切换  
- 硬件抽象，消除平台特定的优化工作  

**模型灵活性**  
- 支持 ONNX 模型格式，并可从流行框架自动转换  
- 使用生产级性能部署自定义模型  
- 与现有 Windows 应用架构集成  
...
**开发环境准备**  
1. 安装 Visual Studio 2022，包含 C++ 和 .NET 工作负载  
...
**示例库设置**  
1. 克隆 [Windows App SDK 示例库](https://github.com/microsoft/WindowsAppSDK-Samples)  
2. 进入 `Samples/WindowsAIFoundry/cs-winui` 以获取 Windows AI API 示例  
3. 进入 `Samples/WindowsML` 以获取全面的 Windows ML 示例  
...
Windows Edge AI 开发代表了强大 AI 功能与稳健、安全、可扩展的 Windows 平台的融合。通过掌握 Windows AI Foundry 生态系统，开发者可以创建智能应用，提供卓越的用户体验，同时保持最高的隐私、安全和性能标准。  

Windows AI API、Foundry Local 和 Windows ML 的结合，为构建下一代智能 Windows 应用提供了无与伦比的基础。随着 AI 的不断发展，Windows 平台确保您的应用能够随着新兴技术扩展，同时在多样化的 Windows 硬件生态系统中保持兼容性和性能。  

无论您是在构建消费类应用、企业解决方案还是专用行业工具，Windows Edge AI 开发都能让您创建智能、响应迅速且深度集成的体验，充分利用现代 Windows 设备的潜力。  
...
- [Windows App SDK 示例库](https://github.com/microsoft/WindowsAppSDK-Samples)
