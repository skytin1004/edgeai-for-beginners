...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-22T05:11:26+00:00",
...
Microsoft Olive 是一个强大且易于使用的硬件感知模型优化工具包，它简化了为不同硬件平台部署机器学习模型的优化过程。无论您是针对 CPU、GPU 还是专用 AI 加速器，Olive 都能帮助您在保持模型精度的同时实现最佳性能。

## 什么是 Microsoft Olive？

Olive 是一个易于使用的硬件感知模型优化工具，它整合了业界领先的模型压缩、优化和编译技术。作为一个端到端的推理优化解决方案，它与 ONNX Runtime 协同工作。

### 主要功能

- **硬件感知优化**：自动为目标硬件选择最佳优化技术
- **40+ 内置优化组件**：涵盖模型压缩、量化、图优化等
- **简单的 CLI 界面**：通过简单命令完成常见优化任务
- **多框架支持**：兼容 PyTorch、Hugging Face 模型和 ONNX
- **流行模型支持**：Olive 可直接优化如 Llama、Phi、Qwen、Gemma 等流行模型架构
...
### 2. 硬件考虑
- 将优化目标与部署硬件匹配
- 如果有 CUDA 兼容硬件，使用 GPU 优化
- 对于 Windows 机器的集成显卡，考虑使用 DirectML
...
### 社区示例
- **Jupyter Notebooks**：可在 Olive 的 GitHub 仓库中找到
- **VS Code 扩展**：AI Toolkit 扩展使用 Olive 进行模型优化
- **博客文章**：Microsoft 开源博客中有详细的 Olive 教程
...
- [04: Apple MLX 框架深入解析](./04.AppleMLX.md)
