...
-->
# Foundry Local 作为工具集成

一个全面的框架，用于将 Microsoft Foundry Local 集成为可调用工具，遵循微软官方的基于工具的 AI 集成模式。

## 概述

此示例展示了如何将 Foundry Local 模型作为可重用工具进行公开，以便集成到现有应用程序、工作流和开发环境中。它展示了微软推荐的工具集成和函数调用模式。

## 核心概念

### 🔧 **工具优先架构**
- 将 Foundry Local 模型作为可调用函数
- 标准化的工具接口和架构
- 与现有代码库的无缝集成
- 类型安全的工具定义和验证

### ⚡ **函数调用模式**
- Microsoft Foundry Local 函数调用实现
- 兼容 OpenAI 的工具定义
- 自动参数验证和转换
- 错误处理和响应格式化

### 🔌 **集成框架**
- **LangChain 集成**：原生 LangChain 工具支持
- **Semantic Kernel**：微软 Semantic Kernel 函数
- **REST API**：基于 HTTP 的工具端点
- **CLI 工具**：命令行界面集成
- **Jupyter Notebooks**：交互式开发工具
...
1. **工具集成模式**
   - 基于函数和基于类的工具设计
   - Microsoft Foundry Local 集成模式
   - 架构验证和类型安全
   - 错误处理和恢复

2. **框架集成**
   - LangChain 工具开发
   - Semantic Kernel 函数集成
   - REST API 框架集成
   - CLI 应用程序开发
