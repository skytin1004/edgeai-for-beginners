...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T09:35:31+00:00",
...
本节课重点介绍如何使用 Foundry Local 进行模型发现与管理的实践操作。您将学习如何列出可用模型、测试不同选项，并了解基本性能特性。课程强调通过 Foundry CLI 的动手探索，帮助您为自己的使用场景选择合适的模型。
...
2. **一次运行一个模型**：启动新模型前先停止之前的模型
3. **监控资源**：关注内存使用情况
4. **一致测试**：使用相同的提示进行公平比较
5. **记录结果**：记录模型性能以便针对您的使用场景进行选择
...
您现在掌握了使用 Foundry Local 的简单 CLI 方法发现、测试和选择适合 AI 应用的模型的实用技能。
...
**中型模型：**
- `qwen2.5-7b-instruct`：优秀的推理能力和更长的上下文
- `deepseek-r1-distill-qwen-7b`：针对代码生成优化

**大型模型：**
- `llama-3.2`：Meta 最新的开源模型
- `qwen2.5-14b-instruct`：企业级推理能力
...
**典型模型大小：**
- `phi-4-mini`：约 2.5 GB
- `qwen2.5-7b-instruct`：约 4.1 GB  
- `deepseek-r1-distill-qwen-7b`：约 4.3 GB
- `llama-3.2`：约 4.9 GB
- `qwen2.5-14b-instruct`：约 8.2 GB

**存储最佳实践：**
- 保留 2-3 个模型缓存以便快速切换
- 删除未使用的模型释放空间：`foundry cache clean`
- 监控磁盘使用情况，尤其是较小的 SSD
- 考虑模型大小与能力的权衡
...
- 起步：`phi-4-mini`（快速、高效）
- 升级：`phi-4`（推理能力更强）
- 高级：`qwen2.5-7b-instruct`（更长的上下文）

**用于代码生成：**
- 推荐：`deepseek-r1-distill-qwen-7b`
- 替代：`qwen2.5-7b-instruct`（代码生成也不错）

**用于复杂推理：**
- 最佳选择：`qwen2.5-7b-instruct` 或 `qwen2.5-14b-instruct`
- 经济选项：`phi-4`
...
- SSD 存储，提升模型加载速度
- 现代 CPU，单线程性能良好
- 支持 NPU（Windows 11 Copilot+ PC）加速
...
- 为第四节做好准备：比较 LLM 与 SLM、本地与云推理，以及动手演示
