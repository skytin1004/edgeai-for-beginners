...
-->
# Session 2: Bygg AI-lösningar med Azure AI Foundry

## Sammanfattning

Utforska hur man bygger användbara GenAI-arbetsflöden med Foundry Local och Azure AI Foundry. Lär dig avancerad promptteknik, integrera strukturerad data och orkestrera uppgifter med reproducerbara pipelines. Fokus ligger på Retrieval-Augmented Generation (RAG) för dokument- och datafrågor, men mönstren kan tillämpas på bredare GenAI-lösningsdesign.
...
- **Överbrygga lokalt till moln**: Förstå migrationsvägar från Foundry Local till Azure AI Foundry
...
### Snabbstart för plattformsoberoende miljö (Windows & macOS)
...
## Validering: Kontroll av Foundry Local-miljö

Innan du börjar med demonstrationerna, validera din lokala miljö:
...
Om det sista kommandot misslyckas, starta (eller starta om) tjänsten: `foundry service start`.

## Demo-flöde (30 minuter)

### 1. Systemprompter och grundningsstrategier (10 minuter)

#### Steg 1.1: Avancerad promptteknik

Skapa `samples/02-rag-solutions/prompt_engineering.py`:
...
#### Steg 1.2: Testa grundningsstrategier
...
### 2. Integrera tabulär data med prompter (CSV Q&A) (10 minuter)

#### Steg 2.1: CSV-dataintegration

Skapa `samples/02-rag-solutions/csv_qa_system.py`:
...
#### Steg 2.2: Testa CSV Q&A-systemet
...
### 3. Startprojekt: Anpassa 02-grounding-data (5 minuter)

#### Steg 3.1: Förbättrat dokument-RAG-system

Skapa `samples/02-rag-solutions/document_rag.py`:
...
### 4. Visa CLI-till-Azure migrationsväg (5 minuter)

#### Steg 4.1: Översikt över migrationsstrategi

Skapa `samples/02-rag-solutions/migration_guide.py`:
...
#### Steg 4.2: Testa migrationsmönster
...
### 1. Avancerad promptteknik

- **Systemprompter**: Domänspecifika expertpersonas
- **Grundningsstrategier**: Tekniker för kontextintegration
- **Temperaturkontroll**: Balans mellan kreativitet och konsekvens
- **Tokenhantering**: Effektiv användning av kontext

### 2. Integration av strukturerad data

- **CSV-bearbetning**: Pandas-integration med AI-modeller
- **Statistisk analys**: Automatisk datasammanfattning
- **Kontextskapande**: Dynamisk kontextgenerering baserat på frågor
- **Stöd för flera format**: JSON, CSV och tabulär data

### 3. RAG-implementeringsmönster

- **Vektorsökning**: TF-IDF och cosinuslikhet
- **Dokumenthämtning**: Relevansbedömning och rankning
...
- **Utvecklingsarbetsflöde**: Lokalt → Staging → Produktion
- **Kostnadsoptimering**: Lokal utveckling, molnproduktion
...
#### Snabbstart för vektorlagring (FAISS)
...
- [Avancerat Multi-Agent System](./samples/09/README.md) - Mönster för agentkoordinering
