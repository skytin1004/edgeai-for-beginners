...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-17T09:33:35+00:00",
...
> **Caminho de Aprendizado Prático para Construção de Aplicações de IA na Edge Prontas para Produção**
>
> Domine o deployment de IA local com o Microsoft Foundry Local, desde a primeira conclusão de chat até a orquestração de múltiplos agentes em 6 sessões progressivas.

---

## 🎯 Introdução

Bem-vindo ao **Workshop EdgeAI para Iniciantes** - seu guia prático e prático para construir aplicações inteligentes que funcionam inteiramente em hardware local. Este workshop transforma conceitos teóricos de IA na Edge em habilidades do mundo real por meio de exercícios progressivamente desafiadores usando o Microsoft Foundry Local e Modelos de Linguagem Pequenos (SLMs).
...
3. **Controle de Custos** - Remover custos por token de API e escalar sem despesas de nuvem
...
- Padrões de deployment para dispositivos na edge

**Este Workshop Entrega Essas Habilidades**

Em 6 sessões focadas (~3 horas no total), você avançará de "Hello World" para o deployment de sistemas multi-agentes prontos para produção - tudo funcionando localmente na sua máquina.
...
1. **Deploy e Gerenciamento de Serviços de IA Locais**
   - Instalar e configurar o Microsoft Foundry Local
   - Selecionar modelos apropriados para deployment na edge
...
3. **Criar Sistemas RAG (Geração Aumentada por Recuperação)**
   ...
   - Basear respostas de LLM em conhecimento específico de domínio
...
6. **Deploy de Soluções Prontas para Produção**
   ...
   - Planejar caminhos de migração de edge para híbrido (edge + nuvem)
...
| **5** | Orquestrador multi-agente | Design de agentes, gerenciamento de memória, coordenação |
...
- Aplicativos de saúde lidando com PHI/PII localmente
...
**Certifique-se de que o Azure AI Foundry Local está rodando com uma porta fixa**
...
2. ✅ Sessão 2: Básicos de RAG (pular avaliação inicialmente)
3. ✅ Sessão 3: Benchmarking simples (apenas 2 modelos)
4. ⏭️ Pular Sessões 4-6 por enquanto
...
5. 🚀 Extensão: Construir lógica de roteamento personalizada e deploys híbridos
...
Cada arquivo de sessão inclui: resumo, objetivos de aprendizado, fluxo de demonstração de 30 minutos, projeto inicial, checklist de validação, solução de problemas e referências ao SDK Foundry Local Python oficial.
...
| `EMBED_MODEL` | Modelo de embedding do Sentence-transformers | `sentence-transformers/all-MiniLM-L6-v2` |
| `RAG_QUESTION` | Substituir consulta de teste para pipeline RAG | `Why use RAG with local inference?` |
| `AGENT_QUESTION` | Substituir consulta de pipeline de agentes | `Explain why edge AI matters for compliance.` |
...
* Helper `chat_once()` com opção de nova tentativa + impressão de uso
...
| Memória Vetorial | Recall semântico de longo prazo | 2,5,6 | Integrar índice de embedding FAISS/Chroma |
| Exportação de Rastreamento | Auditoria & avaliação | 2,5,6 | Adicionar linhas JSON por etapa |
...
Use `rag_eval_ragas.py` para calcular relevância da resposta, fidelidade e precisão de contexto em um pequeno conjunto de dados sintético:

```powershell
python samples/session02/rag_eval_ragas.py
```

Estenda fornecendo um JSONL maior de perguntas, contextos e verdades base, convertendo para um `Dataset` do Hugging Face.
...
| Sistema | `foundry system info` | Snapshot de hardware & capacidades de aceleração |
...
| `foundry model list --running` | Usar `foundry model list` simples + atividade recente / logs |
| `foundry model list --cached` | `foundry cache list` |
| `foundry model stats <model>` | Usar script de benchmark Python + ferramentas do sistema operacional (Gerenciador de Tarefas / `nvidia-smi`) |
...
À medida que novos comandos de telemetria CLI se estabilizam, podem ser incorporados com edições mínimas aos arquivos markdown das sessões.
...
| Habilitar aceleração GPU | `foundry config set compute.onnx.enable_gpu true` | `# CLI action; SDK assumes config already applied` | Configuração é efeito colateral externo |
...
| `benchmark_report.json` | Array de métricas bruto (para comparação / rastreamento de tendências) |

Configurar `BENCH_STREAM=1` no ambiente para incluir latência do primeiro token, se suportado.
