...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T18:26:52+00:00",
...
Use o Foundry Local para prototipar rapidamente aplicações com agentes: prompts de sistema, grounding e padrões de orquestração. Quando o suporte a agentes estiver disponível, você pode padronizar chamadas de função compatíveis com OpenAI ou usar Azure AI Agents no lado da nuvem em designs híbridos.
...
- Orquestrar fluxos de trabalho com múltiplos agentes (local e híbrido)
...
- Basear respostas em dados locais ou corporativos
...
## Parte 3: Orquestração com Múltiplos Agentes (Padrão)
...
Passo 3) Validar no Foundry Local  
```powershell
...
Passo 1) Registro leve de solicitações (opcional)

Nota: O seguinte helper não está incluído por padrão. Crie `infra/obs.py` se quiser registro em JSON local para experimentos.  
```python
# infra/obs.py
import time, json, os
...
Integrar registro nos agentes (opcional):  
```python
...
Passo 5) Trilhas de auditoria local e exportação  
- Armazene logs em JSON em `./agent_logs`  
- Comprimir e rotacionar logs periodicamente  
- Exportar resumos para revisões (contagens, latência média, taxas de erro)  
...
- Como fazer no Open WebUI: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/how-to/how-to-chat-application-with-open-web-ui
...
- Adicione conectores corporativos (Microsoft Graph, Search, bancos de dados)
