...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-26T18:27:55+00:00",
  "source_file": "Module08/README.md",
  "language_code": "bn"
}
-->
# Module 08: Microsoft Foundry Local - সম্পূর্ণ ডেভেলপার টুলকিটের ব্যবহারিক অভিজ্ঞতা

## সংক্ষিপ্ত বিবরণ

[Microsoft Foundry Local](https://learn.microsoft.com/azure/ai-foundry/foundry-local/) হলো এজ এআই ডেভেলপমেন্টের পরবর্তী প্রজন্ম, যা ডেভেলপারদের শক্তিশালী টুল সরবরাহ করে এআই অ্যাপ্লিকেশন স্থানীয়ভাবে তৈরি, ডিপ্লয় এবং স্কেল করার জন্য, এবং একইসাথে Azure AI Foundry-এর সাথে নির্বিঘ্ন সংযোগ বজায় রাখে। এই মডিউলটি Foundry Local-এর ইনস্টলেশন থেকে শুরু করে উন্নত এজেন্ট ডেভেলপমেন্ট পর্যন্ত বিস্তৃত কভারেজ প্রদান করে।
...
- **বিভিন্ন মডেল ডিপ্লয় করবেন**: phi, qwen, deepseek এবং GPT মডেল স্থানীয়ভাবে CLI কমান্ডের মাধ্যমে চালাবেন
- **প্রোডাকশন সমাধান তৈরি করবেন**: উন্নত প্রম্পট ইঞ্জিনিয়ারিং এবং ডেটা ইন্টিগ্রেশনের মাধ্যমে এআই অ্যাপ্লিকেশন তৈরি করবেন
- **ওপেন-সোর্স ইকোসিস্টেম ব্যবহার করবেন**: Hugging Face মডেল এবং কমিউনিটি কন্ট্রিবিউশন ইন্টিগ্রেট করবেন
- **এআই এজেন্ট তৈরি করবেন**: গ্রাউন্ডিং এবং অর্কেস্ট্রেশন ক্ষমতা সহ বুদ্ধিমান এজেন্ট তৈরি করবেন
- **এন্টারপ্রাইজ প্যাটার্ন বাস্তবায়ন করবেন**: প্রোডাকশন ডিপ্লয়মেন্টের জন্য মডুলার, স্কেলযোগ্য এআই সমাধান তৈরি করবেন
...
### [2: Azure AI Foundry দিয়ে এআই সমাধান তৈরি করা](./02.AzureAIFoundryIntegration.md)
...
### [5: দ্রুত এআই-চালিত এজেন্ট তৈরি করা](./05.AIPoweredAgents.md)
...
**ফোকাস**: জটিল এজেন্ট সমন্বয়, বিশেষায়িত কাজের প্রতিনিধিত্ব এবং সহযোগী এআই ওয়ার্কফ্লো
...
- **অপারেটিং সিস্টেম**: Windows 11 (22H2 বা পরবর্তী সংস্করণ)
...
|---------|------------|---------|------|------------|
|  ১ | সেটআপ এবং বেসিক | ০১, ০২, ০৩ | ২-৩ ঘণ্টা | প্রাথমিক |
|  ২ | এআই সমাধান | ০৪ | ২-৩ ঘণ্টা | মধ্যম |
|  ৩ | ওপেন সোর্স | ০৫ | ২-৩ ঘণ্টা | মধ্যম |
|  ৪ | উন্নত মডেল | ০৬ | ৩-৪ ঘণ্টা | উন্নত |
|  ৫ | এআই এজেন্ট | ০৫, ০৯ | ৩-৪ ঘণ্টা | উন্নত |
|  ৬ | এন্টারপ্রাইজ টুলস | ০৬, ১০ | ৩-৪ ঘণ্টা | বিশেষজ্ঞ |
|  ৭ | সরাসরি API ইন্টিগ্রেশন | ০৭ | ২-৩ ঘণ্টা | মধ্যম |
|  ৮ | Windows 11 চ্যাট অ্যাপ | ০৮ | ৩-৪ ঘণ্টা | উন্নত |
|  ৯ | উন্নত মাল্টি-এজেন্ট | ০৯ | ৪-৫ ঘণ্টা | বিশেষজ্ঞ |
| ১০ | টুলস ফ্রেমওয়ার্ক | ১০ | ৪-৫ ঘণ্টা | বিশেষজ্ঞ |
...
- **অ্যাপ্লিকেশন তৈরি**: উন্নত বৈশিষ্ট্য এবং অপ্টিমাইজেশন সহ প্রোডাকশন-রেডি এআই অ্যাপ্লিকেশন তৈরি
- **এজেন্ট ডেভেলপমেন্ট**: গ্রাউন্ডিং, রিজনিং এবং টুল ইন্টিগ্রেশন সহ উন্নত এআই এজেন্ট বাস্তবায়ন

### কৌশলগত বোঝাপড়া
- **আর্কিটেকচার সিদ্ধান্ত**: স্থানীয় বনাম ক্লাউড ডিপ্লয়মেন্টের মধ্যে তথ্যপূর্ণ সিদ্ধান্ত নেওয়া
...
- **গোপনীয়তা এবং নিরাপত্তা**: স্থানীয় ইনফারেন্স সহ গোপনীয়তা-সংরক্ষণকারী এআই সমাধান বাস্তবায়ন

### উদ্ভাবনী সক্ষমতা
- **দ্রুত প্রোটোটাইপিং**: সমস্ত ১০টি নমুনা প্যাটার্ন জুড়ে এআই অ্যাপ্লিকেশন ধারণা দ্রুত তৈরি এবং পরীক্ষা করা
- **কমিউনিটি ইন্টিগ্রেশন**: ওপেন-সোর্স মডেল ব্যবহার এবং ইকোসিস্টেমে অবদান রাখা
- **উন্নত প্যাটার্ন**: RAG, এজেন্ট এবং টুল ইন্টিগ্রেশন সহ আধুনিক এআই প্যাটার্ন বাস্তবায়ন
- **ফ্রেমওয়ার্ক দক্ষতা**: LangChain, Semantic Kernel, Chainlit এবং Electron-এর সাথে বিশেষজ্ঞ-স্তরের ইন্টিগ্রেশন
- **প্রোডাকশন ডিপ্লয়মেন্ট**: স্থানীয় প্রোটোটাইপ থেকে এন্টারপ্রাইজ সিস্টেমে স্কেলযোগ্য এআই সমাধান ডিপ্লয়
- **ভবিষ্যৎ-প্রস্তুত ডেভেলপমেন্ট**: উদীয়মান এআই প্রযুক্তি এবং প্যাটার্নের জন্য প্রস্তুত অ্যাপ্লিকেশন তৈরি
...
4. **নমুনাগুলির মাধ্যমে অগ্রসর হন**: ব্যাপক দক্ষতার জন্য নমুনা ০১-১০ সম্পন্ন করুন

## সফলতার মেট্রিক

সমস্ত ১০টি ব্যাপক নমুনার মাধ্যমে আপনার অগ্রগতি ট্র্যাক করুন:
...
- [ ] একটি কার্যকর RAG চ্যাট অ্যাপ্লিকেশন তৈরি করুন (নমুনা ০৪)
...
### দক্ষতার সূচক
- [ ] সমস্ত ১০টি নমুনা সফলভাবে চালান, কোনো ত্রুটি ছাড়াই
- [ ] নির্দিষ্ট ব্যবহার ক্ষেত্রে অন্তত ৩টি নমুন
এই মডিউলটি এজ এআই উন্নয়নের সর্বাধুনিক দিক উপস্থাপন করে, যেখানে মাইক্রোসফটের এন্টারপ্রাইজ-গ্রেড টুলসের সাথে ওপেন-সোর্স ইকোসিস্টেমের নমনীয়তা এবং উদ্ভাবন একত্রিত হয়েছে। Foundry Local-এর ১০টি বিস্তৃত নমুনা সম্পূর্ণভাবে আয়ত্ত করার মাধ্যমে আপনি এআই অ্যাপ্লিকেশন উন্নয়নের শীর্ষে অবস্থান করতে পারবেন।

**সম্পূর্ণ শেখার পথ:**
- **প্রাথমিক স্তর** (নমুনা ০১-০৩): API ইন্টিগ্রেশন এবং মডেল ব্যবস্থাপনা
- **অ্যাপ্লিকেশন** (নমুনা ০৪-০৬): RAG, এজেন্ট, এবং বুদ্ধিমান রাউটিং
- **উন্নত স্তর** (নমুনা ০৭-১০): প্রোডাকশন ফ্রেমওয়ার্ক এবং এন্টারপ্রাইজ ইন্টিগ্রেশন

Azure OpenAI ইন্টিগ্রেশনের জন্য (সেশন ২), প্রয়োজনীয় পরিবেশ ভেরিয়েবল এবং API ভার্সন সেটিংসের বিস্তারিত তথ্য প্রতিটি নমুনার README ফাইলে দেখুন।
