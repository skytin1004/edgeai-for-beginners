...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T15:01:55+00:00",
...
Microsoft Foundry Local হলো এজ AI ডেভেলপমেন্টের পরবর্তী প্রজন্ম, যা ডেভেলপারদের শক্তিশালী টুল সরবরাহ করে AI অ্যাপ্লিকেশন স্থানীয়ভাবে তৈরি, ডিপ্লয় এবং স্কেল করার জন্য, এবং একই সাথে Azure AI Foundry-এর সাথে নির্বিঘ্ন ইন্টিগ্রেশন বজায় রাখে। এই মডিউলটি Foundry Local-এর ইনস্টলেশন থেকে উন্নত এজেন্ট ডেভেলপমেন্ট পর্যন্ত বিস্তৃত কভারেজ প্রদান করে।
...
- **বিভিন্ন মডেল ডিপ্লয় করবেন**: phi, qwen, deepseek এবং GPT মডেল CLI কমান্ডের মাধ্যমে স্থানীয়ভাবে চালাবেন
...
**সময়কাল**: ২-৩ ঘণ্টা | **স্তর**: বিগিনার

---

### [2: Azure AI Foundry দিয়ে AI সলিউশন তৈরি করা](./02.AzureAIFoundryIntegration.md)
**ফোকাস**: উন্নত প্রম্পট ইঞ্জিনিয়ারিং, ডেটা ইন্টিগ্রেশন এবং ক্লাউড কানেক্টিভিটি
...
**সময়কাল**: ২-৩ ঘণ্টা | **স্তর**: ইন্টারমিডিয়েট
...
**মূল বিষয়**: HuggingFace ইন্টিগ্রেশন • Bring-your-own-model • Model Mondays ইনসাইট • কমিউনিটি কন্ট্রিবিউশন • মডেল নির্বাচন
...
**সময়কাল**: ৩-৪ ঘণ্টা | **স্তর**: অ্যাডভান্সড
...
**মূল বিষয়**: মডেল টুল হিসেবে • ডিভাইসে ডিপ্লয়মেন্ট • SDK/API ইন্টিগ্রেশন • এন্টারপ্রাইজ আর্কিটেকচার • স্কেলিং কৌশল

**নমুনা**: [Models-as-Tools Router](./samples/06/README.md) • [Foundry Tools Framework](./samples/10/README.md)

**সময়কাল**: ৩-৪ ঘণ্টা | **স্তর**: এক্সপার্ট
...
**মূল বিষয়**: HTTP ক্লায়েন্ট বাস্তবায়ন • কাস্টম অথেন্টিকেশন • মডেল হেলথ মনিটরিং • স্ট্রিমিং রেসপন্স • প্রোডাকশন ত্রুটি পরিচালনা
...
**ফোকাস**: উন্নত এজেন্ট সমন্বয়, বিশেষায়িত কাজের প্রতিনিধিত্ব এবং সহযোগী AI ওয়ার্কফ্লো

**মূল বিষয়**: বুদ্ধিমান এজেন্ট সমন্বয় • ফাংশন কলিং প্যাটার্ন • ক্রস-এজেন্ট যোগাযোগ • ওয়ার্কফ্লো অর্কেস্ট্রেশন • কোয়ালিটি অ্যাসুরেন্স মেকানিজম

**নমুনা**: [Advanced Multi-Agent System](./samples/09/README.md)

**সময়কাল**: ৪-৫ ঘণ্টা | **স্তর**: এক্সপার্ট
...
|---------|------------|---------|------|------------|
|  ১ | সেটআপ এবং বেসিক | ০১, ০২, ০৩ | ২-৩ ঘণ্টা | বিগিনার |
|  ২ | AI সলিউশন | ০৪ | ২-৩ ঘণ্টা | ইন্টারমিডিয়েট |
|  ৩ | ওপেন সোর্স | ০৫ | ২-৩ ঘণ্টা | ইন্টারমিডিয়েট |
|  ৪ | উন্নত মডেল | ০৬ | ৩-৪ ঘণ্টা | অ্যাডভান্সড |
|  ৫ | AI এজেন্ট | ০৫, ০৯ | ৩-৪ ঘণ্টা | অ্যাডভান্সড |
|  ৬ | এন্টারপ্রাইজ টুলস | ০৬, ১০ | ৩-৪ ঘণ্টা | এক্সপার্ট |
|  ৭ | সরাসরি API ইন্টিগ্রেশন | ০৭ | ২-৩ ঘণ্টা | ইন্টারমিডিয়েট |
|  ৮ | Windows 11 চ্যাট অ্যাপ | ০৮ | ৩-৪ ঘণ্টা | অ্যাডভান্সড |
|  ৯ | উন্নত মাল্টি-এজেন্ট | ০৯ | ৪-৫ ঘণ্টা | এক্সপার্ট |
| ১০ | টুলস ফ্রেমওয়ার্ক | ১০ | ৪-৫ ঘণ্টা | এক্সপার্ট |
...
- [Foundry Local Discussions](https://github.com/microsoft/Foundry-Local/discussions) - কমিউনিটি Q&A এবং ফিচার রিকোয়েস্ট
...
- **এজেন্ট ডেভেলপমেন্ট**: গ্রাউন্ডিং, রিজনিং এবং টুল ইন্টিগ্রেশনের সাথে উন্নত AI এজেন্ট বাস্তবায়ন
...
### উদ্ভাবনী সক্ষমতা
- **দ্রুত প্রোটোটাইপিং**: সমস্ত ১০টি নমুনা প্যাটার্ন জুড়ে AI অ্যাপ্লিকেশন ধারণা দ্রুত তৈরি এবং পরীক্ষা
...
3. **নমুনা ০১ চালান**: সেটআপ যাচাই করার জন্য বেসিক REST API ইন্টিগ্রেশন দিয়ে শুরু করুন
4. **নমুনাগুলি সম্পন্ন করুন**: ০১-১০ নমুনা সম্পন্ন করে সম্পূর্ণ দক্ষতা অর্জন করুন

## সফলতার মেট্রিক

সব ১০টি বিস্তৃত নমুনা সম্পন্ন করার মাধ্যমে আপনার অগ্রগতি ট্র্যাক করুন:
...
- [ ] একটি বিস্তৃত টুলস ফ্রেমওয়ার্ক তৈরি করুন (নমুনা ১০)
...
Azure OpenAI ইন্টিগ্রেশনের জন্য (সেশন ২), প্রয়োজনীয় পরিবেশ ভেরিয়েবল এবং API ভার্সন সেটিংসের জন্য প্রতিটি নমুনার README ফাইল দেখুন।
