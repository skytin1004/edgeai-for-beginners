...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-25T02:31:55+00:00",
  "source_file": "Module08/04.CuttingEdgeModels.md",
  "language_code": "uk"
}
-->
# Сесія 4: Створення чат-додатків для продакшн з Chainlit
...
- Створювати готові до продакшн чат-додатки за допомогою Chainlit
...
## Частина 2: Локальний vs хмарний матриця рішень

### Характеристики продуктивності

| Аспект | Локально (Foundry) | Хмара (Azure OpenAI) |
|--------|--------------------|----------------------|
...
| **Вартість** | 💰 Безкоштовно після обладнання | 💸 Оплата за токен |
| **Офлайн** | ✅ Працює без інтернету | ❌ Потребує інтернет |
| **Розмір моделі** | ⚠️ Обмежено обладнанням | ✅ Доступ до найбільших моделей |
| **Масштабування** | ⚠️ Залежить від обладнання | ✅ Необмежене масштабування |

### Гібридні стратегії

**Локально спочатку з резервним варіантом:**
```python
...
Зразок 04 демонструє готові до продакшн патерни:
...
| `BASE_URL` | Точка доступу Foundry Local | Автоматично визначається | `http://localhost:51211` |
...
Зразок 04 включає комплексний Jupyter Notebook (`chainlit_app.ipynb`), який надає:
...
WebGPU дозволяє запускати AI-моделі безпосередньо в браузері для максимального рівня конфіденційності та досвіду без встановлення. Цей зразок демонструє ONNX Runtime Web із виконанням через WebGPU.
...
3. **Конфігурація моделі:**
   ...
   - Базова URL-адреса: `http://host.docker.internal:51211/v1`
...
**Запобігання конфлікту порту 51211:**
```cmd
...
- **[Зразок 04: Chainlit Application](samples/04/README.md)**: Повний додаток із документацією
