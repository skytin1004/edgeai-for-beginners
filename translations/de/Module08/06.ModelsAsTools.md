...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T11:43:17+00:00",
...
> **🔄 Aktualisiert für modernes SDK**: Dieses Modul wurde an die neuesten Muster des Microsoft Foundry-Local-Repositorys angepasst und entspricht der intelligenten Routing-Implementierung in `samples/06/`. Die Beispiele verwenden jetzt das moderne `foundry-local-sdk` und fortschrittliche Modell-Auswahlstrategien.

**🏗️ Architektur-Highlights:**
- **Intelligentes Modell-Routing**: Auswahl basierend auf Schlüsselwörtern zwischen allgemeinen, logischen, Code- und kreativen Modellen
- **Moderne SDK-Integration**: Verwendet `FoundryLocalManager` mit automatischer Dienst-Erkennung
- **Umgebungskonfiguration**: Flexible Modellzuweisung über Umgebungsvariablen
- **Gesundheitsüberwachung**: Validierung von Diensten und Überprüfung der Modellverfügbarkeit
...
## Teil 1: Intelligenter Modell-Router (Moderne Implementierung)
...
Schritt 2) Initialisieren des Clients mit modernem SDK und Dienst-Erkennung  
```python
...
## Teil 2: Moderne SDK-Integration (Schritt-für-Schritt)
...
Schritt 3) Moderne SDK-Integration  
```python
# modern_sdk_demo.py
from foundry_local import FoundryLocalManager
from openai import OpenAI
...
Schritt 1) Erstellen einer Domänen-Prompt-Vorlage  
```python
# domain/templates.py
...
> **📋 Hinweis**: Die lokale Implementierung in `samples/06/model_router.ipynb` enthält umfassende Beispiele für Produktionsbereitstellungsmuster.
...
- [ ] Intelligenten Modell-Router mit schlüsselwortbasierter Auswahl implementieren (`samples/06/router.py`)
- [ ] Mehrere spezialisierte Modelle konfigurieren (allgemein, logisch, Code, kreativ)
- [ ] Interaktives Jupyter-Notebook testen (`samples/06/model_router.ipynb`)
- [ ] Umgebungsgestützte Modellkonfiguration einrichten
- [ ] Dienst-Gesundheitsüberwachung und Fehlerhandling implementieren
- [ ] Produktions-Router mit umfassendem Logging bereitstellen

## Lokale Musterintegration

Die vollständige Implementierung ausführen:  
```cmd
cd Module08
.\.venv\Scripts\activate
...
Foundry Local ermöglicht robuste KI direkt auf dem Gerät, bei der Modelle zu intelligenten, spezialisierten Werkzeugen werden. Mit automatischer Modellauswahl, umfassender Überwachung und produktionsbereiten Mustern können Teams anspruchsvolle KI-Anwendungen entwickeln, die sich an unterschiedliche Aufgaben anpassen und gleichzeitig Datenschutz und Leistung gewährleisten. Das hier vorgestellte Muster des intelligenten Routers bietet eine Grundlage für den Aufbau komplexer KI-Systeme, die von lokaler Entwicklung bis hin zur Produktionsbereitstellung skalierbar sind.
