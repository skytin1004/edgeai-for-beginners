...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T12:55:40+00:00",
...
Trata los modelos de IA como herramientas modulares y personalizables que se ejecutan directamente en el dispositivo con Foundry Local. Esta sesión pone énfasis en flujos de trabajo prácticos para inferencias con preservación de privacidad y baja latencia, y en cómo integrar estas herramientas mediante SDKs, APIs o CLI. También aprenderás a escalar a Azure AI Foundry cuando sea necesario.
...
- Compilación de modelos de Hugging Face: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/how-to/how-to-compile-hugging-face-models
...
## Parte 1: Abstracciones de Herramientas (Paso a paso)

Objetivo: Representar modelos como herramientas con contratos claros y un enrutador simple.

Paso 1) Definir la interfaz y el registro de herramientas  
```python
# tools/registry.py
...
Paso 2) Implementar dos herramientas respaldadas por Foundry Local  
```python
# tools/impl.py
import requests, os
...
## Parte 2: Integración con SDK y API (Paso a paso)

Objetivo: Usar el SDK de Python de OpenAI contra el endpoint de Foundry Local.
...
Paso 3) Llamar a la API de chat  
```python
# sdk_demo.py
...
Objetivo: Adaptar las salidas para un dominio utilizando plantillas de prompts y esquemas JSON.
...
## Parte 4: Modo Offline y Postura de Seguridad (Paso a paso)
...
Paso 3) Bandera de solo local y registro de logs  
```python
...
## Parte 5: Escalado a Azure AI Foundry (Paso a paso)

Objetivo: Reflejar modelos locales con endpoints de Azure para capacidad adicional.

Paso 1) Decidir estrategia de enrutamiento  
- Priorizar local para privacidad/latencia, usar Azure como respaldo en caso de errores o prompts grandes  

Paso 2) Implementar un stub de enrutador simple  
```python
# hybrid/router.py
import os, requests
...
- [ ] Registrar al menos dos herramientas y enrutar solicitudes  
- [ ] Llamar a Foundry Local mediante el SDK de OpenAI y REST sin procesar  
- [ ] Forzar salidas en formato JSON para una plantilla de dominio  
- [ ] Sanitizar y registrar llamadas localmente  
- [ ] Implementar un enrutador híbrido simple con respaldo en Azure  

## Conclusión

Foundry Local permite una IA robusta en el dispositivo donde los modelos se convierten en herramientas componibles. Con interfaces claras, gobernanza y escalado híbrido, los equipos pueden desarrollar aplicaciones de IA en tiempo real, seguras, que respeten la privacidad del usuario y estén listas para el entorno empresarial.
