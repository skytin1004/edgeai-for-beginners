...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-02T11:27:54+00:00",
...
歡迎來到 Windows Edge AI 開發指南！這是一份全面的指南，幫助您利用 Microsoft 的 Windows AI Foundry 平台，構建智能應用程序，充分發揮設備端 AI 的潛力。本指南專為希望將尖端的 Edge AI 功能整合到應用程序中的 Windows 開發者設計，同時充分利用 Windows 硬件加速的優勢。
...
**通用硬件支持**  
Windows ML 提供自動硬件優化，適用於整個 Windows 生態系統，確保您的 AI 應用程序無論底層芯片架構如何都能達到最佳性能。

**集成 AI 運行時**  
內建的 Windows ML 推理引擎消除了複雜的設置需求，使開發者能專注於應用邏輯，而非基礎設施問題。

**Copilot+ PC 優化**  
專為配備專用神經處理單元 (NPU) 的下一代 Windows 設備設計的 API，提供卓越的性能與功耗比。

**開發者生態系統**  
豐富的工具，包括 Visual Studio 集成、全面的文檔和示例應用程序，加速開發周期。

## 學習目標

完成本 Windows Edge AI 開發指南後，您將掌握在 Windows 平台上構建生產級 AI 應用程序的核心技能。
...
- 實施設備端 AI 應用的安全最佳實踐
- 為不同的 Windows 硬件配置優化應用程序

**API 集成專業知識**
- 精通 Windows AI API，用於文本、視覺和多模態應用
...
- 將 AI 功能整合到 Win32、UWP 和漸進式 Web 應用程序中
...
**市場定位**
- 理解 Windows 原生 AI 應用的競爭優勢
- 識別設備端 AI 提供卓越用戶體驗的使用場景
- 為 AI 增強的 Windows 應用程序制定市場策略
- 將應用程序定位於充分利用 Windows 生態系統的優勢
...
| 示例 | 框架 | 重點領域 | 主要功能 |
|------|------|----------|----------|
| [cs-winui](https://github.com/microsoft/WindowsAppSDK-Samples/tree/main/Samples/WindowsAIFoundry/cs-winui) | C# WinUI 3 | Windows AI API 集成 | 完整的 WinUI 應用，展示 Windows AI API、ARM64 優化、打包部署 |
...
| 示例 | 類型 | 重點領域 | 主要功能 |
|------|------|----------|----------|
| [CppConsoleDesktop](https://github.com/microsoft/WindowsAppSDK-Samples/tree/main/Samples/WindowsML/cpp) | 控制台應用 | 基本 Windows ML | EP 發現、命令行選項、模型編譯 |
| [CppConsoleDesktop.FrameworkDependent](https://github.com/microsoft/WindowsAppSDK-Samples/tree/main/Samples/WindowsML/cpp) | 控制台應用 | 框架部署 | 共享運行時、更小的部署足跡 |
...
| [CppConsoleDll](https://github.com/microsoft/WindowsAppSDK-Samples/tree/main/Samples/WindowsML/cpp) | DLL | 庫使用 | WindowsML 在共享庫中的應用，內存管理 |
...
| 示例 | 類型 | 重點領域 | 主要功能 |
|------|------|----------|----------|
| [CSharpConsoleDesktop](https://github.com/microsoft/WindowsAppSDK-Samples/tree/main/Samples/WindowsML/cs) | 控制台應用 | 基本 C# 集成 | 共享助手使用、命令行界面 |
...
2. **註冊執行提供者** - 發現並註冊可用的硬件加速器（CPU、GPU、NPU）
...
Foundry Local 為開發者提供快速訪問 Windows Silicon 上即用型開源語言模型的能力，支持瀏覽、測試、交互和在本地應用中部署模型。
...
| [electron/foundry-chat](https://github.com/microsoft/Foundry-Local/tree/main/samples/electron/foundry-chat) | JavaScript / Electron | 桌面聊天應用 | 跨平台聊天、本地/雲模型切換、OpenAI SDK 集成、實時流式 |
...
**桌面應用**
- **electron/foundry-chat**：具有本地/雲模型切換的生產級聊天應用程序
...
Windows ML 是 Windows 的核心 AI 平台與整合推理運行時，讓開發者能高效部署自定義模型於廣泛的 Windows 硬體生態系統中。  
...
**模型評估**  
- 使用 Foundry Local 測試開源模型是否適合使用案例  
...
Windows Edge AI 開發代表了強大 AI 功能與穩健、安全且可擴展的 Windows 平台的融合。通過掌握 Windows AI Foundry 生態系統，開發者可以創建智能應用程式，提供卓越的使用者體驗，同時保持最高的隱私、安全與效能標準。  

Windows AI API、Foundry Local 和 Windows ML 的結合，為構建下一代智能 Windows 應用程式提供了無與倫比的基礎。隨著 AI 的不斷發展，Windows 平台確保您的應用程式能隨著新興技術擴展，同時在多樣化的 Windows 硬體生態系統中保持相容性與效能。  

無論您是在構建消費者應用程式、企業解決方案，還是專業行業工具，Windows Edge AI 開發都能幫助您創建智能、響應迅速且深度整合的體驗，充分發揮現代 Windows 設備的潛力。  
...
- [ONNX Runtime 推理範例](https://github.com/microsoft/onnxruntime
...
**免責聲明**：  
本文件已使用 AI 翻譯服務 [Co-op Translator](https://github.com/Azure/co-op-translator) 進行翻譯。雖然我們努力確保翻譯的準確性，但請注意，自動翻譯可能包含錯誤或不準確之處。原始文件的母語版本應被視為權威來源。對於關鍵信息，建議使用專業人工翻譯。我們對因使用此翻譯而產生的任何誤解或錯誤解釋不承擔責任。