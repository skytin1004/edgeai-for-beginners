...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T16:54:27+00:00",
...
Microsoft Foundry Local 代表了邊緣 AI 開發的下一代技術，提供開發者強大的工具來本地化建構、部署及擴展 AI 應用，同時保持與 Azure AI Foundry 的無縫整合。本模組全面涵蓋 Foundry Local 的安裝到進階代理開發。
...
- **精通 Foundry Local 設置**：安裝、配置及優化 Foundry Local 以進行 Windows 11 開發
- **部署多樣化模型**：使用 CLI 命令本地運行 phi、qwen、deepseek 和 GPT-OSS-20B 模型
- **建構生產解決方案**：利用進階提示工程及數據整合創建 AI 應用
- **利用開源生態系統**：整合 Hugging Face 模型及社群驅動的擴展
- **比較 AI 架構**：理解 LLMs 與 SLMs 的取捨及部署策略
- **開發 AI 代理**：使用 Foundry Local 的架構及基礎能力建構智能代理
- **實現模型工具化**：創建模組化、可定制的 AI 解決方案以應對企業應用

## 課程結構

### [1: 開始使用 Foundry Local](./01.FoundryLocalSetup.md)
**重點**：安裝、CLI 設置、模型快取及硬體加速

**您將學到：**
- 完成 Foundry Local 在 Windows 11 上的安裝
- CLI 配置及命令結構
- 模型快取策略以達到最佳性能
- 硬體加速設置及優化
...
**重點**：進階提示工程、數據整合及可操作任務

**您將學到：**
- 進階提示工程技術
- 數據整合模式及最佳實踐
- 使用 Foundry Local 建構可操作的 AI 任務
...
### [4: 探索尖端模型 - LLMs、SLMs 及裝置推理](./04.CuttingEdgeModels.md)
**重點**：模型比較、使用 Phi 和 ONNX Runtime 的 EdgeAI、進階演示

**您將學到：**
- 全面比較 LLMs 與 SLMs 的使用案例
- 本地與雲端推理的取捨及決策框架
- 使用 Phi 和 ONNX Runtime 的 EdgeAI 實現
...
### [5: 使用 Foundry Local 快速建構 AI 驅動代理](./05.AIPoweredAgents.md)
...
**時長**：3-4 小時  
**先決條件**：完成所有前述課程，有企業開發經驗者更佳

## 先決條件

### 系統需求
- **作業系統**：Windows 11（22H2 或更高版本）
- **記憶體**：16GB RAM（建議 32GB 以運行更大型模型）
- **儲存空間**：50GB 可用空間以進行模型快取
- **硬體**：建議使用具 NPU 的裝置（Copilot+ PC），GPU 可選
- **網路**：高速網路以進行初始模型下載
...
- [Azure AI Foundry Local 文件](https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/)
- [Model Mondays 系列](https://aka.ms/model-mondays)

### 社群資源
- [Foundry Local 社群討論](https://github.com/microsoft/Foundry-Local/discussions)
- [Azure AI Foundry 範例](https://github.com/Azure-Samples/ai-foundry)
...
- **開發代理**：實現具備基礎、推理及工具整合的高級 AI 代理

### 策略理解
- **架構決策**：在本地與雲端部署間做出明智選擇
- **性能優化**：在不同硬體配置中優化推理性能
- **企業擴展**：設計從本地原型到企業部署的應用
- **隱私與安全**：實現隱私保護的 AI 解決方案，使用本地推理

### 創新能力
- **快速原型**：快速建構及測試 AI 應用概念
- **社群整合**：利用開源模型並為生態系統做出貢獻
- **進階模式**：實現尖端 AI 模式，包括 RAG、代理及工具整合
- **面向未來的開發**：建構適應新興 AI 技術及模式的應用

## 開始使用

1. **準備您的環境**：確保使用 Windows 11 並符合建議的硬體規格
2. **安裝先決條件**：設置開發工具及依賴項
3. **從第一節開始**：從 Foundry Local 安裝及基本設置開始
4. **按順序進行**：依次完成課程以達到最佳學習效果
...
本模組代表了邊緣 AI 開發的尖端技術，結合了 Microsoft 的企業級工具與開源生態系統的靈活性及創新性。通過掌握 Foundry Local，您將站在 AI 應用開發的最前沿。

關於 Azure OpenAI（第二節課程），請參閱範例 README 以了解所需的環境變數及 API 版本設置。

## 範例概述

- `samples/01`：快速 REST 聊天至 Foundry Local（`chat_quickstart.py`）。
- `samples/02`：OpenAI SDK 整合（`sdk_quickstart.py`）。
- `samples/03`：模型探索 + 快速基準測試（`list_and_bench.cmd`）。
