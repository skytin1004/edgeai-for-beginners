...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-17T09:14:34+00:00",
...
> **建立可投入生產的 Edge AI 應用程式的實作學習路徑**
>
> 透過 Microsoft Foundry Local，從首次聊天完成到多代理協作，掌握本地 AI 部署的技能，分為六個漸進式課程。

---

## 🎯 簡介

歡迎參加 **EdgeAI 初學者工作坊**，這是一個實作型的指南，教您如何建立完全在本地硬體上運行的智能應用程式。本工作坊將理論性的 Edge AI 概念轉化為實際技能，透過使用 Microsoft Foundry Local 和小型語言模型（SLMs）進行逐步挑戰的練習。
...
| **2** | 使用 RAG 建立 AI 解決方案 | 提示工程、嵌入、評估 | 30 分鐘 |
...
| 2 | [`session02_rag_pipeline.ipynb`](./notebooks/session02_rag_pipeline.ipynb) | 建立 RAG 系統 | ⭐⭐ 中級 |
...
如果您正在參加濃縮的六課工作坊格式，請使用這些專用指南（每個指南與上述更廣泛的模組文檔相輔相成）：

| 工作坊課程 | 指南 | 核心重點 |
|-----------|------|---------|
| 1 | [Session01-GettingStartedFoundryLocal](./Session01-GettingStartedFoundryLocal.md) | 安裝、驗證、運行 phi & GPT-OSS-20B、加速 |
| 2 | [Session02-BuildAISolutionsRAG](./Session02-BuildAISolutionsRAG.md) | 提示工程、RAG 模式、CSV 和文檔基礎化、遷移 |
...
如果 Foundry Local 服務在不同的（Windows）機器或虛擬機上運行，並從 macOS 導出端點：
...
| `BENCH_MODELS` | 基準測試模型別名的逗號列表 | `qwen2.5-0.5b,gemma-2-2b,mistral-7b` |
...
* 帶可選重試與使用量打印的 `chat_once()` 輔助功能
* 簡單的 token 使用量報告（通過設置 `SHOW_USAGE=1` 啟用）

這減少了重複並突出了高效的本地模型編排最佳實踐。
...
通過提供更大的 JSONL 問題、上下文和真實值數據集進行擴展，然後轉換為 Hugging Face `Dataset`。
...
隨著新的 CLI 遙測命令在上游穩定，它們可以以最小的編輯集成到課程 markdown 文件中。

### 自動化 Lint 守護

一個自動化的 linter 防止在 markdown 文件的代碼塊中重新引入已棄用的 CLI 模式：
...
| 下載（緩存）模型 | `foundry model download qwen2.5-0.5b` | `FoundryLocalManager("qwen2.5-0.5b")  # triggers download/load` | 管理器在別名映射到多個版本時選擇最佳變體 |
| 列出目錄 | `foundry model list` | `# use manager for each alias or maintain known list` | CLI 聚合；SDK 目前每個別名實例化 |
...
| 預熱模型 | `foundry model run <alias>` 然後首次提示 | `chat_once(alias, messages=[...])`（輔助功能） | 輔助功能處理初始冷延遲預熱 |
...
在環境中設置 `BENCH_STREAM=1` 以包括首 token 延遲（如果支持）。
