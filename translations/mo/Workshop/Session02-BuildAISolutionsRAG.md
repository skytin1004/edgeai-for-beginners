...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-09T08:03:58+00:00",
...
探索如何使用 Foundry Local 和 Azure AI Foundry 建構可行的生成式 AI 工作流程。學習進階提示工程、整合結構化數據，以及使用可重現的管道來編排任務。雖然重點在於文件與數據問答的檢索增強生成（RAG），但這些模式可廣泛應用於生成式 AI 解決方案設計。
...
- **實現 RAG 模式**：使用向量搜索建構基於文件的問答系統
- **整合結構化數據**：在 AI 工作流程中處理 CSV、JSON 和表格數據
- **建構生產級 RAG**：使用 Chainlit 創建可擴展的 RAG 應用
- **連接本地與雲端**：了解從 Foundry Local 遷移到 Azure AI Foundry 的路徑
...
如果您的環境尚未提供 Foundry Local 的 macOS 二進制檔案，請在 Windows VM 或容器中運行服務並設置：
```bash
...
### 3. 初始專案：改進 02-grounding-data（5 分鐘）
...
2. **建構生產級 RAG**：使用 Chainlit 實現（範例 04）
3. **進階向量搜索**：與 Chroma 或 Pinecone 整合
...
| 類別 | 增強 | 理由 | 指導方向 |
|------|------|------|----------|
| 檢索 | 使用向量存儲（FAISS / Chroma）替換 TF-IDF | 提升語義召回與可擴展性 | 將文件分塊（500–800 字元），嵌入，持久化索引 |
| 混合索引 | 雙重語義 + 關鍵字過濾 | 改善數字 / 程式碼查詢的精確度 | 先按關鍵字過濾，再按餘弦相似度排序 |
| 嵌入 | 評估多個嵌入模型 | 優化相關性與速度 | A/B 測試：MiniLM vs E5-small vs 本地托管編碼器 |
| 快取 | 快取嵌入與檢索結果 | 降低重複查詢延遲 | 使用簡單的磁碟 pickle / sqlite，基於哈希鍵 |
| 評估 | 擴展 ragas 數據集 | 統計上有意義的質量 | 策劃 50–100 問答 + 上下文；按主題分層 |
| 指標 | 跟蹤檢索與生成時間 | 性能分析 | 捕捉每次調用的 `retrieval_ms`、`gen_ms`、`tokens` |
| 防護措施 | 添加幻覺回退 | 更安全的答案 | 如果真實性 < 閾值 → 答案："上下文不足。" |
| 回退 | 本地 → Azure 模型級聯 | 混合質量提升 | 在低信心時通過相同的 OpenAI API 路由到雲端 |
| 確定性 | 穩定比較運行 | 可重複的評估集 | 固定種子，`temperature=0`，禁用採樣隨機性 |
...
1. 組裝一個 JSONL，字段包括：`question`、`answer`、`contexts`、`ground_truths`（列表）
...
對於磁碟持久化，使用 `faiss.write_index(index, "kb.index")`。
...
| `samples/session02/rag_eval_ragas.py` / `notebooks/session02_rag_eval_ragas.ipynb` | 質量分析員建立檢索真實性基準指標 | 在小型合成數據集上計算 ragas 指標 | `DOCS`、`QUESTIONS`、`GROUND_TRUTH` 陣列 | 本地推理有什麼優勢？ |
| `prompt_engineering.py`（進階） | 領域專家為多個垂直領域設計基礎提示 | 比較領域系統提示與令牌影響 | 內嵌的 `contexts` 字典 | Foundry Local 如何處理模型快取？ |
| `csv_qa_system.py` | 銷售運營探索導出數據的交互式分析 | 摘要與查詢小型銷售數據片段 | 生成的 `sample_sales_data.csv`（10 行） | 哪個產品的平均銷售額最高？ |
| `document_rag.py` | 產品團隊探索內部維基的文件 RAG | 檢索並引用相關文件 | `create_sample_knowledge_base()` 列表 | 邊緣 AI 有什麼好處？ |
| `migration_guide.py` | 架構師準備雲端遷移計劃 | 演示本地→Azure API 的一致性 | 靜態測試提示 | 用 2–3 句話解釋邊緣 AI 的好處。 |
...
### 場景敘述
支持工程團隊希望快速原型化以回答內部常見問題，而不將客戶數據暴露到外部。第二節的成果從最小的臨時 RAG（無持久化）→ 結構化 CSV 問答 → 文件檢索與引用 → 客觀質量評估（ragas）→ 準備好 Azure 測試環境的遷移策略。
...
**免責聲明**：  
本文檔已使用AI翻譯服務 [Co-op Translator](https://github.com/Azure/co-op-translator) 進行翻譯。我們致力於提供準確的翻譯，但請注意，自動翻譯可能包含錯誤或不準確之處。應以原始語言的文件作為權威來源。對於關鍵信息，建議尋求專業人工翻譯。我們對因使用此翻譯而引起的任何誤解或錯誤解讀概不負責。