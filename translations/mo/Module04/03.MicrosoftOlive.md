...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T16:52:15+00:00",
...
Microsoft Olive 是一款功能強大且易於使用的硬體感知模型優化工具包，簡化了針對不同硬體平台部署機器學習模型的優化過程。不論是針對 CPU、GPU 或專用 AI 加速器，Olive 都能幫助您在保持模型準確性的同時實現最佳性能。

## 什麼是 Microsoft Olive？

Olive 是一款易於使用的硬體感知模型優化工具，整合了業界領先的技術，包括模型壓縮、優化和編譯。它與 ONNX Runtime 配合使用，提供端到端的推理優化解決方案。

### 主要功能

- **硬體感知優化**：自動選擇最適合目標硬體的優化技術
- **40+ 內建優化元件**：涵蓋模型壓縮、量化、圖形優化等
- **簡易 CLI 介面**：使用簡單指令完成常見的優化任務
- **多框架支援**：支援 PyTorch、Hugging Face 模型及 ONNX
- **流行模型支援**：Olive 可自動優化流行的模型架構，例如 Llama、Phi、Qwen、Gemma 等
...
優化過程包括：從本地快取獲取模型，捕獲 ONNX 圖形並將權重存儲到 ONNX 數據文件中，優化 ONNX 圖形，並使用 RTN 方法將模型量化為 int4。

### 指令參數解析

- `--model_name_or_path`：Hugging Face 模型識別碼或本地路徑
...
### 社群範例
- **Jupyter Notebooks**：可在 Olive GitHub 儲存庫中找到 — https://github.com/microsoft/Olive/tree/main/examples
...
- **Azure Machine Learning**：基於雲的優化工作流程 — https://learn.microsoft.com/azure/machine-learning/

## ➡️ 下一步

- [第四章：OpenVINO 工具包優化套件](./04.openvino.md)
