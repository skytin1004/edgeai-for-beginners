...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T14:22:31+00:00",
...
| 第七天 | 模組 6-7：AI 代理與開發工具 | 4 小時 |
| 第八天 | 模組 8：Foundry Local 工具包（現代實現） | 1 小時 |
...
| 第 13-14 天 | 模組 7：EdgeAI 實現範例 | 2 小時 |
...
這是一門精簡的 20 小時課程，旨在以高效的方式傳授 EdgeAI 的基本知識，非常適合忙碌的專業人士和學生快速掌握這一新興領域的實用技能。
...
2. **簡單文本生成**：使用小型模型進行基本文本生成實現（1 小時）
...
3. **簡單整合**：創建一個使用已部署模型的最小應用程序（1 小時）
...
- 理解格式轉換策略（GGUF、ONNX）
- 在六個框架中實現優化（Llama.cpp、Olive、OpenVINO、MLX、工作流程綜合）
- 為 Intel、Apple 和跨平台硬件的生產邊緣環境部署優化模型
...
3. 硬體感知優化如何在 Microsoft Olive 中提升部署效率？
...
- 安裝和配置 Foundry Local，並集成現代 SDK
- 使用協調器模式實施高級多代理系統
- 構建智能模型路由器，進行自動基於任務的選擇
- 部署生產就緒的 AI 解決方案，並進行全面監控
- 與 Azure AI Foundry 集成以進行混合部署場景
...
4. 生產就緒的 AI 代理系統的主要組成部分是什麼？
5. 如何為 Foundry Local 服務實施全面的健康監控？
6. 比較現代化方法與傳統實施模式的優勢。

### 實作練習

1. **現代 SDK 設置**: 配置 FoundryLocalManager，並進行自動服務發現（30 分鐘）
2. **多代理系統**: 運行帶有專家代理的高級協調器（30 分鐘）
...
### 中級專案（每個需時 3-4 小時）
4. **AI 工具工作流程**：使用 VS Code AI Toolkit 從頭到尾優化並部署模型
5. **Windows AI Foundry 應用程式**：使用 Phi Silica API 和 NPU 優化創建一個 Windows 應用程式
6. **跨平台部署**：將同一優化模型部署到 Windows（OpenVINO）和行動裝置（.NET MAUI）
7. **函數調用代理**：建立一個具有函數調用能力的 AI 代理，用於邊緣場景

### 高級整合專案（每個需時 4-5 小時）
8. **OpenVINO 優化管道**：使用 NNCF 和 GenAI 工具包實現完整的模型優化
9. **SLMOps 管道**：實現從訓練到邊緣部署的完整模型生命周期
10. **多模型邊緣系統**：在邊緣硬體上部署多個專用模型協同工作
11. **MCP 整合系統**：使用 Model Context Protocol 建立一個代理系統進行工具互動
...
EdgeAI 代表人工智慧應用的前沿技術，將強大的能力直接帶到設備上，同時解決隱私、延遲和連接性等重要問題。本課程共 20 小時，提供您開始使用 EdgeAI 技術所需的基本知識和實用技能。

課程內容精簡且專注於最重要的概念，讓您能快速獲得有價值的專業知識，而不需投入過多時間。請記住，實作練習，即使是簡單的例子，也是鞏固所學的關鍵。
