...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-25T00:06:17+00:00",
...
- Có đủ dung lượng lưu trữ để tải xuống mô hình (mô hình có thể dao động từ 1GB đến hơn 20GB)
...
4. **Ghi chép kết quả**: Tạo một biểu đồ so sánh đơn giản
...
**Hiệu suất chậm:**
- Đảm bảo mô hình đã được tải đầy đủ (kiểm tra đầu ra chi tiết)
- Đóng các ứng dụng nền không cần thiết
- Cân nhắc sử dụng lưu trữ nhanh hơn (SSD)

### Thực hành tốt nhất

1. **Bắt đầu nhỏ**: Bắt đầu với `phi-4-mini` để xác thực thiết lập
2. **Chỉ chạy một mô hình tại một thời điểm**: Dừng các mô hình trước đó trước khi khởi động mô hình mới
3. **Theo dõi tài nguyên**: Theo dõi việc sử dụng bộ nhớ
4. **Kiểm tra nhất quán**: Sử dụng cùng một lời nhắc để so sánh công bằng
5. **Ghi chép kết quả**: Lưu lại ghi chú về hiệu suất mô hình cho các trường hợp sử dụng của bạn
...
Bạn đã có kỹ năng thực hành để khám phá, thử nghiệm và chọn các mô hình phù hợp cho ứng dụng AI của mình bằng cách sử dụng cách tiếp cận CLI đơn giản của Foundry Local.
...
**Mô hình trung bình:**
- `qwen2.5-7b-instruct`: Suy luận xuất sắc và ngữ cảnh dài hơn
- `deepseek-r1-distill-qwen-7b`: Tối ưu hóa cho việc tạo mã

**Mô hình lớn hơn:**
- `llama-3.2`: Mô hình mã nguồn mở mới nhất của Meta
- `qwen2.5-14b-instruct`: Suy luận cấp doanh nghiệp
...
Khi một mô hình đang chạy, hãy kiểm tra nó với các lời nhắc nhất quán:
...
### Thử nghiệm thay thế với PowerShell
...
- Theo dõi việc sử dụng ổ đĩa, đặc biệt trên các SSD nhỏ hơn
- Cân nhắc giữa kích thước mô hình và khả năng

### Theo dõi hiệu suất mô hình

Khi các mô hình đang chạy, theo dõi tài nguyên hệ thống:

**Trình quản lý tác vụ Windows:**
- Theo dõi việc sử dụng bộ nhớ (mô hình được giữ trong RAM)
- Theo dõi việc sử dụng CPU trong quá trình suy luận
...
- Nâng cấp lên: `phi-4` (khả năng suy luận tốt hơn)
- Nâng cao: `qwen2.5-7b-instruct` (ngữ cảnh dài hơn)

**Đối với tạo mã:**
- Khuyến nghị: `deepseek-r1-distill-qwen-7b`
- Lựa chọn thay thế: `qwen2.5-7b-instruct` (cũng tốt cho mã)

**Đối với suy luận phức tạp:**
- Tốt nhất: `qwen2.5-7b-instruct` hoặc `qwen2.5-14b-instruct`
...
Đối với mỗi mô hình, kiểm tra với các lời nhắc nhất quán và đánh giá thủ công:
