...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-25T00:03:18+00:00",
  "source_file": "STUDY_GUIDE.md",
  "language_code": "vi"
}
-->
# EdgeAI cho Người Mới Bắt Đầu: Lộ Trình Học và Lịch Học
...
| Module | Ngày hoàn thành | Thời gian đã dành | Những điều học được |
|--------|-----------------|-------------------|---------------------|
...
- **Thực hành thực tế**: Hoàn thành các bài tập gợi ý để củng cố các khái niệm lý thuyết
- **Tài nguyên bổ sung**: Khám phá thêm tài liệu cho các chủ đề bạn quan tâm nhất
...
## Module 1: Kiến thức cơ bản và sự chuyển đổi của EdgeAI

### Mục Tiêu Học Tập Chính

- Hiểu sự khác biệt giữa AI dựa trên đám mây và AI dựa trên edge
- Nắm vững các kỹ thuật tối ưu hóa cốt lõi cho môi trường hạn chế tài nguyên
...
- **Các khái niệm ưu tiên**: 
  - Các mô hình điện toán Edge vs. Cloud
...
1. So sánh và đối chiếu AI dựa trên đám mây với các triển khai AI dựa trên edge.
2. Giải thích ba kỹ thuật chính để tối ưu hóa mô hình cho triển khai edge.
3. Những lợi ích chính của việc chạy mô hình AI tại edge là gì?
4. Mô tả quy trình lượng hóa một mô hình và cách nó ảnh hưởng đến hiệu suất.
...
- So sánh khả năng của các mô hình theo các quy mô tham số khác nhau
- Đánh giá mô hình dựa trên hiệu quả, khả năng, và yêu cầu triển khai
...
- **Các khái niệm ưu tiên**: 
  ...
  - Các khả năng chuyên biệt

#### Phần 2: Họ Qwen
- **Các khái niệm ưu tiên**: 
  - Đóng góp mã nguồn mở
  - Các tùy chọn triển khai có thể mở rộng
...
3. Những lợi ích độc đáo của mô hình Mu cho tích hợp Windows là gì?
4. Mô tả cách Phi-Silica tận dụng phần cứng NPU để tối ưu hóa hiệu suất.
5. Đối với một ứng dụng di động có kết nối hạn chế, họ mô hình nào sẽ phù hợp nhất và tại sao?
...
- Lựa chọn mô hình phù hợp dựa trên các ràng buộc triển khai
- Nắm vững các kỹ thuật tối ưu hóa cho các kịch bản triển khai khác nhau
...
1. Những yếu tố nào cần được cân nhắc khi lựa chọn giữa triển khai cục bộ và triển khai đám mây?
2. So sánh Ollama và Microsoft Foundry Local như các tùy chọn triển khai.
3. Giải thích lợi ích của việc sử dụng container hóa cho triển khai SLM.
4. Những chỉ số hiệu suất chính cần theo dõi cho một SLM triển khai tại edge là gì?
...
2. **Kiểm tra hiệu suất**: Chạy một đánh giá nhanh trên mô hình đã triển khai (30 phút)
...
- Nắm vững các kỹ thuật lượng hóa nâng cao từ độ chính xác 1-bit đến 8-bit
- Hiểu các chiến lược chuyển đổi định dạng (GGUF, ONNX)
- Triển khai tối ưu hóa trên sáu framework (Llama.cpp, Olive, OpenVINO, MLX, workflow synthesis)
...
- **Các khái niệm ưu tiên**: 
  ...
  - Cân nhắc giữa hiệu suất và độ chính xác
...
6. Quy trình tổng hợp giúp chọn framework tối ưu như thế nào?
...
- Thành thạo kỹ thuật chưng cất và tinh chỉnh cho triển khai tại edge
...
- **Các khái niệm ưu tiên**: 
  ...
  - Đào tạo đa adapter và tối ưu hóa siêu tham số
...
4. Mô tả một quy trình triển khai sản xuất hoàn chỉnh cho ứng dụng AI tại edge.

### Bài tập thực hành

1. **Chưng cất cơ bản**: Tạo một mô hình nhỏ hơn từ một mô hình lớn hơn (1 giờ)
2. **Thí nghiệm tinh chỉnh**: Tinh chỉnh một mô hình cho một lĩnh vực cụ thể (1 giờ)
3. **Quy trình triển khai**: Thiết lập một quy trình CI/CD cơ bản cho triển khai mô hình (1 giờ)

## Module 6: Hệ thống SLM Agentic - AI Agents và Function Calling

### Mục tiêu học tập chính

- Xây dựng các agent AI thông minh cho môi trường edge sử dụng mô hình ngôn ngữ nhỏ
- Triển khai khả năng gọi hàm với quy trình hệ thống hóa
- Thành thạo tích hợp Model Context Protocol (MCP) để tương tác công cụ chuẩn hóa
- Tạo hệ thống agentic phức tạp với sự can thiệp tối thiểu của con người

### Các lĩnh vực trọng tâm nghiên cứu

#### Phần 1: AI Agents và nền tảng SLM
- **Các khái niệm ưu tiên**: 
  - Khung phân loại agent (phản xạ, dựa trên mô hình, dựa trên mục tiêu, agent học tập)
  - Phân tích đánh đổi giữa SLM và LLM
  - Mẫu thiết kế agent dành riêng cho edge
  - Tối ưu hóa tài nguyên cho agent

#### Phần 2: Gọi hàm trong mô hình ngôn ngữ nhỏ
- **Các khái niệm ưu tiên**: 
  - Triển khai quy trình hệ thống hóa (phát hiện ý định, đầu ra JSON, thực thi bên ngoài)
  - Triển khai theo nền tảng (Phi-4-mini, các mô hình Qwen được chọn, Microsoft Foundry Local)
  - Ví dụ nâng cao (hợp tác đa agent, lựa chọn công cụ động)
  - Các yếu tố sản xuất (giới hạn tốc độ, ghi nhật ký kiểm toán, biện pháp bảo mật)
...
1. Những yếu tố kiến trúc chính cho các agent AI tại edge là gì?
2. Gọi hàm cải thiện khả năng của agent như thế nào?
3. Giải thích vai trò của Model Context Protocol trong giao tiếp của agent.

### Bài tập thực hành

1. **Agent đơn giản**: Xây dựng một agent AI cơ bản với khả năng gọi hàm (1 giờ)
2. **Tích hợp MCP**: Triển khai MCP trong một ứng dụng agent (30 phút)
...
- Xây dựng ứng dụng EdgeAI sẵn sàng sản xuất với các tối ưu hóa dành riêng cho nền tảng
...
- **Các khái niệm ưu tiên**: 
  ...
  - Quy trình thử nghiệm, tối ưu hóa và phát triển agent tại địa phương
...
3. Những lợi ích của Windows AI Foundry đối với phát triển edge là gì?
...
6. So sánh lợi ích của triển khai tại địa phương và trên cloud đối với các ứng dụng nhạy cảm về quyền riêng tư.

### Bài tập thực hành

1. **Cài đặt AI Toolkit**: Cấu hình AI Toolkit và tối ưu hóa một mô hình (1 giờ)
2. **Windows AI Foundry**: Xây dựng một ứng dụng Windows AI đơn giản sử dụng API Phi Silica (1 giờ)
...
## Module 8: Microsoft Foundry Local – Bộ công cụ nhà phát triển hoàn chỉnh (Hiện đại hóa)

### Mục tiêu học tập chính

- Cài đặt và cấu hình Foundry Local với tích hợp SDK hiện đại
- Triển khai hệ thống đa agent nâng cao với các mẫu phối hợp
- Xây dựng bộ định tuyến mô hình thông minh với lựa chọn tự động dựa trên nhiệm vụ
- Triển khai các giải pháp AI sẵn sàng sản xuất với giám sát toàn diện
- Tích hợp với Azure AI Foundry cho các kịch bản triển khai lai
...
- **Các khái niệm ưu tiên**: 
  ...
  - Tự động phát hiện dịch vụ và giám sát sức khỏe
  - Mẫu cấu hình dựa trên môi trường
  - Các yếu tố triển khai sản xuất

#### Phần 2: Hệ thống đa agent nâng cao
- **Các khái niệm ưu tiên**: 
  - Mẫu phối hợp với các agent chuyên biệt
  - Chuyên môn hóa agent truy xuất, lý luận và thực thi
  - Cơ chế vòng phản hồi để tinh chỉnh
  - Giám sát hiệu suất và theo dõi thống kê

#### Phần 3: Định tuyến mô hình thông minh
- **Các khái niệm ưu tiên**: 
  - Thuật toán lựa chọn mô hình dựa trên từ khóa
  - Hỗ trợ nhiều mô hình (chung, lý luận, mã, sáng tạo)
  - Cấu hình biến môi trường để linh hoạt
  - Kiểm tra sức khỏe dịch vụ và xử lý lỗi

#### Phần 4: Triển khai sẵn sàng sản xuất
- **Các khái niệm ưu tiên**: 
  - Xử lý lỗi toàn diện và cơ chế dự phòng
  - Giám sát yêu cầu và theo dõi hiệu suất
  - Ví dụ tương tác với Jupyter notebook và các benchmark
  - Mẫu tích hợp với các ứng dụng hiện có

### Câu hỏi tự đánh giá

1. Cách tiếp cận FoundryLocalManager hiện đại khác gì so với các cuộc gọi REST thủ công?
2. Giải thích mẫu phối hợp và cách nó điều phối các agent chuyên biệt.
3. Bộ định tuyến thông minh chọn mô hình phù hợp dựa trên nội dung truy vấn như thế nào?
4. Những thành phần chính của hệ thống agent AI sẵn sàng sản xuất là gì?
5. Làm thế nào để triển khai giám sát sức khỏe toàn diện cho các dịch vụ Foundry Local?
...
2. **Hệ thống đa agent**: Chạy mẫu phối hợp nâng cao với các agent chuyên biệt (30 phút)
...
6. **Tích hợp lai**: Cấu hình các kịch bản dự phòng Azure AI Foundry (30 phút)

## Hướng dẫn phân bổ thời gian

Để giúp bạn tận dụng tối đa thời gian 20 giờ của khóa học, đây là gợi ý phân bổ thời gian:
...
Những tài nguyên được chọn lọc này cung cấp giá trị lớn nhất cho thời gian học tập hạn chế của bạn:
...
- [Ollama Quick Start](https://github.com/ollama/ollama#get-started) - Cách nhanh nhất để triển khai SLM tại địa phương
- [Microsoft Phi Model Card](https://huggingface.co/microsoft/phi-2) - Tham khảo cho một mô hình tối ưu hóa edge hàng đầu
...
- [Gradio](https://www.gradio.app/docs/interface) - Phát triển giao diện người dùng nhanh chóng cho các demo AI
- [Microsoft Olive](https://github.com/microsoft/Olive) - Tối ưu hóa mô hình đơn giản
- [Llama.cpp](https://github.com/ggml-ai/llama.cpp) - Suy luận CPU hiệu quả
- [OpenVINO NNCF](https://github.com/openvinotoolkit/nncf) - Framework nén mạng nơ-ron
...
Hãy thử hoàn thành một trong những dự án dưới đây để thực hành các khái niệm về EdgeAI (mỗi dự án được thiết kế để mất khoảng 2-4 giờ):

### Dự án dành cho người mới bắt đầu (2-3 giờ mỗi dự án)
1. **Trợ lý văn bản Edge**: Tạo một công cụ hoàn thành văn bản đơn giản hoạt động offline bằng cách sử dụng một mô hình ngôn ngữ nhỏ.
2. **Bảng điều khiển so sánh mô hình**: Xây dựng một công cụ trực quan hóa cơ bản để so sánh các chỉ số hiệu suất giữa các SLM khác nhau.
...
4. **Quy trình công cụ AI**: Sử dụng VS Code AI Toolkit để tối ưu hóa và triển khai một mô hình từ đầu đến cuối.
5. **Ứng dụng Windows AI Foundry**: Tạo một ứng dụng Windows sử dụng API Phi Silica và tối ưu hóa NPU.
6. **Triển khai đa nền tảng**: Triển khai cùng một mô hình đã được tối ưu hóa trên Windows (OpenVINO) và di động (.NET MAUI).
7. **Agent gọi hàm**: Xây dựng một agent AI với khả năng gọi hàm cho các kịch bản edge.

### Dự án tích hợp nâng cao (4-5 giờ mỗi dự án)
8. **Pipeline tối ưu hóa OpenVINO**: Triển khai toàn bộ quy trình tối ưu hóa mô hình bằng NNCF và GenAI toolkit.
9. **Pipeline SLMOps**: Triển khai toàn bộ vòng đời mô hình từ huấn luyện đến triển khai edge.
10. **Hệ thống Edge đa mô hình**: Triển khai nhiều mô hình chuyên biệt hoạt động cùng nhau trên phần cứng edge.
11. **Hệ thống tích hợp MCP**: Xây dựng một hệ thống agentic sử dụng Model Context Protocol để tương tác với công cụ.
...
- Microsoft Learn (Foundry Local)
  ...
  - Tích hợp với inference SDKs: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/how-to/how-to-integrate-with-inference-sdks
...
- Azure AI Foundry
  ...
  - Agents (tổng quan): https://learn.microsoft.com/en-us/azure/ai-services/agents/overview
...
- Framework triển khai và mô hình
  - Llama.cpp: https://github.com/ggml-ai/llama.cpp
  - Hugging Face Transformers: https://huggingface.co/docs/transformers/index
  - vLLM (docs): https://docs.vllm.ai/
  - Ollama (quick start): https://github.com/ollama/ollama#get-started
- Công cụ dành cho nhà phát triển (Windows và VS Code)
  - AI Toolkit cho VS Code: https://learn.microsoft.com/en-us/azure/ai-toolkit/overview
...
- Thảo luận trên GitHub trong [EdgeAI for Beginners repository](https://github.com/microsoft/edgeai-for-beginners/discussions)
- [Microsoft Tech Community](https://techcommunity.microsoft.com/)
- [Stack Overflow](https://stackoverflow.com/questions/tagged/edge-ai)

## Kết luận

EdgeAI đại diện cho một bước tiến mới trong việc triển khai trí tuệ nhân tạo, mang lại khả năng mạnh mẽ trực tiếp trên các thiết bị đồng thời giải quyết các vấn đề quan trọng về quyền riêng tư, độ trễ và kết nối. Khóa học 20 giờ này cung cấp cho bạn kiến thức cơ bản và kỹ năng thực hành cần thiết để bắt đầu làm việc với các công nghệ EdgeAI ngay lập tức.

Khóa học được thiết kế ngắn gọn và tập trung vào các khái niệm quan trọng nhất, giúp bạn nhanh chóng đạt được chuyên môn mà không cần cam kết thời gian quá lớn. Hãy nhớ rằng thực hành trực tiếp, ngay cả với các ví dụ đơn giản, là chìa khóa để củng cố những gì bạn đã học.
