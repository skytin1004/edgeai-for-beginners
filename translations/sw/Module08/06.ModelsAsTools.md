...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-23T01:07:02+00:00",
...
Tumia miundo ya AI kama zana zinazoweza kubadilishwa na modular zinazofanya kazi moja kwa moja kwenye kifaa kwa kutumia Foundry Local. Kipindi hiki kinazingatia mchakato wa vitendo wa utabiri wa faragha, wa haraka, na jinsi ya kuunganisha zana hizi kupitia SDKs, APIs, au CLI. Pia utajifunza jinsi ya kupanua hadi Azure AI Foundry inapohitajika.

Marejeleo:
- Nyaraka za Foundry Local: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/
- Kuunganisha na inference SDKs: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/how-to/how-to-integrate-with-inference-sdks
- Kuandaa miundo ya Hugging Face: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/how-to/how-to-compile-hugging-face-models

## Malengo ya Kujifunza
- Kubuni mifumo ya "model-as-a-tool" kwenye kifaa
- Kuunganisha kupitia OpenAI-compatible REST API au SDKs
- Kubadilisha miundo kwa matumizi maalum ya kikoa
- Kupanga upanuzi wa mseto hadi Azure AI Foundry

## Sehemu ya 1: Abstraksia za Zana (Hatua kwa hatua)

Lengo: Kuwakilisha miundo kama zana zenye mikataba wazi na router rahisi.

Hatua ya 1) Fafanua interface ya zana na rejista  
```python
# tools/registry.py
...
Hatua ya 2) Tekeleza zana mbili zinazotumia Foundry Local  
```python
# tools/impl.py
import requests, os
...
## Sehemu ya 2: SDK na API Integration (Hatua kwa hatua)

Lengo: Kutumia OpenAI Python SDK dhidi ya endpoint ya Foundry Local.
...
Lengo: Kubadilisha matokeo kwa kikoa kwa kutumia templates za maelezo na JSON schema.

Hatua ya 1) Unda template ya maelezo ya kikoa  
```python
# domain/templates.py
...
Lengo: Kuhakikisha faragha na uimara wakati wa kuendesha miundo kama zana kwa ndani.

Hatua ya 1) Pre-warm na thibitisha endpoint ya ndani  
```cmd
foundry model run phi-4-mini
curl http://localhost:8000/v1/models
```
  
Hatua ya 2) Safisha pembejeo  
```python
...
Hatua ya 3) Bendera ya ndani tu na logging  
```python
...
## Sehemu ya 5: Upanuzi hadi Azure AI Foundry (Hatua kwa hatua)

Lengo: Kuakisi miundo ya ndani na endpoints za Azure kwa uwezo wa ziada.

Hatua ya 1) Amua mkakati wa routing  
- Kwanza ndani kwa faragha/haraka, Azure fallback kwa makosa au maelezo makubwa  

Hatua ya 2) Tekeleza router stub rahisi  
```python
# hybrid/router.py
import os, requests
...
## Orodha ya Vitendo
- [ ] Sajili angalau zana mbili na route maombi  
- [ ] Piga Foundry Local kupitia OpenAI SDK na REST mbichi  
- [ ] Lazimisha matokeo ya JSON kwa template ya kikoa  
- [ ] Safisha na logging maombi kwa ndani  
- [ ] Tekeleza router rahisi ya mseto na Azure fallback  

## Hitimisho

Foundry Local inawezesha AI thabiti kwenye kifaa ambapo miundo inakuwa zana zinazoweza kuunganishwa. Kwa interfaces wazi, usimamizi, na upanuzi wa mseto, timu zinaweza kuunda programu za AI za wakati halisi, salama zinazoheshimu faragha ya mtumiaji huku zikiwa tayari kwa matumizi ya biashara.
