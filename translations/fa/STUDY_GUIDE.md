...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T13:06:21+00:00",
...
7. **نمونه‌های پیاده‌سازی EdgeAI** - ابزارهای هوش مصنوعی، توسعه ویندوز و پیاده‌سازی‌های خاص پلتفرم
8. **Microsoft Foundry Local – مجموعه کامل ابزارهای توسعه‌دهنده** - توسعه محلی با یکپارچه‌سازی هیبریدی Azure (ماژول 08)

## نحوه استفاده از این راهنمای مطالعه

- **یادگیری تدریجی**: ماژول‌ها را به ترتیب دنبال کنید تا تجربه یادگیری منسجم‌تری داشته باشید.
...
| روز 6 | ماژول 8: ابزار Foundry Local | 3 ساعت |
...
| هفته 3 | ماژول 3: استقرار SLM (3 ساعت) + ماژول 8: ابزار Foundry Local (2-3 ساعت) | 5-6 ساعت |
...
- تکنیک‌های اصلی بهینه‌سازی برای محیط‌های محدود منابع را بیاموزید.
...
3. **کوانتیزاسیون پایه**: یک کوانتیزاسیون ساده روی یک مدل کوچک انجام دهید (1 ساعت)

## ماژول 2: مبانی مدل‌های کوچک زبانی (SLM)
...
1. **مقایسه مدل‌ها**: یک بنچمارک سریع از دو مدل SLM مختلف انجام دهید (1 ساعت)
2. **تولید متن ساده**: پیاده‌سازی پایه تولید متن با یک مدل کوچک (1 ساعت)
3. **بهینه‌سازی سریع**: یک تکنیک بهینه‌سازی برای بهبود سرعت استنتاج اعمال کنید (1 ساعت)
...
4. معیارهای کلیدی عملکرد برای یک SLM مستقر در لبه چیست؟
5. یک جریان کاری کامل استقرار از انتخاب مدل تا پیاده‌سازی تولید را توضیح دهید.

### تمرین‌های عملی

1. **استقرار محلی پایه**: یک SLM ساده با استفاده از Ollama مستقر کنید (1 ساعت)
...
- بهینه‌سازی در شش چارچوب (Llama.cpp، Olive، OpenVINO، MLX، سنتز جریان کاری) را پیاده‌سازی کنید.
...
- **مفاهیم اولویت‌دار**: 
  ...
  - تکنیک‌های شتاب‌دهنده سخت‌افزاری
...
- **مفاهیم اولویت‌دار**:  
  ...
  - درخت تصمیم‌گیری انتخاب چارچوب  
  - اعتبارسنجی آمادگی تولید  
  - استراتژی‌های آینده‌نگر  

### سوالات ارزیابی شخصی  

1. استراتژی‌های کمیت‌سازی را در سطوح دقت مختلف (از ۱ بیت تا ۸ بیت) مقایسه کنید.  
2. مزایای فرمت GGUF برای استقرار در لبه را توضیح دهید.  
3. چگونه بهینه‌سازی سخت‌افزار محور در Microsoft Olive کارایی استقرار را بهبود می‌بخشد؟  
...
1. **کمیت‌سازی مدل**: سطوح مختلف کمیت‌سازی را روی یک مدل اعمال کنید و نتایج را مقایسه کنید (۱ ساعت)  
...
4. **معیارهای عملکرد**: تأثیر بهینه‌سازی بر سرعت استنتاج و استفاده از حافظه را اندازه‌گیری کنید (۱ ساعت)  
...
- **مفاهیم اولویت‌دار**:  
  ...
  - معماری اولویت‌دار هزینه و حفظ حریم خصوصی  
...
- **مفاهیم اولویت‌دار**:  
  ...
  - آموزش چند آداپتر و بهینه‌سازی هایپرپارامتر  

#### بخش ۴: استقرار تولید  
- **مفاهیم اولویت‌دار**:  
  - تبدیل و کمیت‌سازی مدل برای تولید  
...
4. یک جریان کاری کامل استقرار تولید برای برنامه‌های Edge AI را توضیح دهید.  
...
3. **جریان کاری استقرار**: یک جریان کاری CI/CD پایه برای استقرار مدل تنظیم کنید (۱ ساعت)  

## ماژول ۶: سیستم‌های عامل SLM - عوامل هوش مصنوعی و فراخوانی توابع  

### اهداف کلیدی یادگیری  

- عوامل هوش مصنوعی هوشمند برای محیط‌های لبه با استفاده از مدل‌های زبانی کوچک بسازید  
- قابلیت‌های فراخوانی توابع را با جریان‌های کاری سیستماتیک اجرا کنید  
- ادغام پروتکل زمینه مدل (MCP) را برای تعامل استاندارد ابزارها بیاموزید  
- سیستم‌های عامل پیچیده با حداقل دخالت انسانی ایجاد کنید  
...
1. **عامل ساده**: یک عامل هوش مصنوعی پایه با قابلیت فراخوانی توابع بسازید (۱ ساعت)  
...
- ابزار AI Toolkit برای Visual Studio Code را برای جریان‌های کاری جامع توسعه EdgeAI بیاموزید  
- تخصص در پلتفرم Windows AI Foundry و استراتژی‌های بهینه‌سازی NPU کسب کنید  
- EdgeAI را در چندین پلتفرم سخت‌افزاری و سناریوهای استقرار اجرا کنید  
- برنامه‌های EdgeAI آماده تولید با بهینه‌سازی‌های خاص پلتفرم بسازید  

### حوزه‌های تمرکز مطالعه  

#### بخش ۱: ابزار AI Toolkit برای Visual Studio Code  
- **مفاهیم اولویت‌دار**:  
...
- **مفاهیم اولویت‌دار**:  
  ...
  - برنامه‌های Foundry Local با اجرای RAG متمرکز بر حفظ حریم خصوصی  

### سوالات ارزیابی شخصی  

1. ابزار AI Toolkit چگونه جریان کاری توسعه EdgeAI را ساده می‌کند؟  
2. استراتژی‌های استقرار را در پلتفرم‌های سخت‌افزاری مختلف مقایسه کنید.  
3. مزایای Windows AI Foundry برای توسعه لبه چیست؟  
4. نقش بهینه‌سازی NPU در برنامه‌های مدرن Edge AI چیست؟  
...
1. **تنظیم ابزار AI Toolkit**: ابزار AI Toolkit را پیکربندی کنید و یک مدل را بهینه کنید (۱ ساعت)  
2. **Windows AI Foundry**: یک برنامه ساده Windows AI با استفاده از API Phi Silica بسازید (۱ ساعت)  
...
## ماژول ۸: Microsoft Foundry Local – مجموعه ابزار کامل توسعه‌دهنده  

### اهداف کلیدی یادگیری  

- Foundry Local را روی Windows نصب و پیکربندی کنید  
- مدل‌ها را به صورت محلی از طریق CLI Foundry اجرا، کشف و مدیریت کنید  
- با مشتریان REST و SDK سازگار با OpenAI ادغام کنید  
- نمونه‌های عملی بسازید: چت Chainlit، عوامل و مسیریاب مدل  
- الگوهای ترکیبی با Azure AI Foundry را درک کنید  

### حوزه‌های تمرکز مطالعه  

- نصب و اصول CLI (مدل، سرویس، کش)  
- ادغام SDK (مشتریان سازگار با OpenAI و Azure OpenAI)  
- اعتبارسنجی سریع Open WebUI  
- الگوهای عوامل و فراخوانی توابع  
- مدل‌ها به عنوان ابزار (طراحی مسیریاب و رجیستری)  

### سوالات ارزیابی شخصی  

1. چگونه نقطه پایانی محلی را کشف کرده و مدل‌های موجود را لیست می‌کنید؟  
2. تفاوت‌های بین استفاده از REST Foundry Local و Azure OpenAI چیست؟  
3. چگونه یک مسیریاب ساده برای انتخاب مدل‌ها به عنوان ابزار طراحی می‌کنید؟  
4. کدام دسته‌های CLI برای توسعه روزانه بیشتر مرتبط هستند؟  
5. چگونه آمادگی Foundry Local را قبل از اجرای برنامه‌ها اعتبارسنجی می‌کنید؟  

### تمرین‌های عملی  

1. Foundry Local را نصب/ارتقا دهید و `phi-4-mini` را به صورت محلی اجرا کنید (۳۰ دقیقه)  
2. `/v1/models` را فراخوانی کنید و یک چت ساده از طریق REST اجرا کنید (۳۰ دقیقه)  
3. نمونه برنامه Chainlit را راه‌اندازی کنید و به صورت محلی چت کنید (۳۰ دقیقه)  
4. هماهنگ‌کننده چند عامل را اجرا کنید و خروجی‌ها را بررسی کنید (۳۰ دقیقه)  
5. مسیریاب مدل‌ها به عنوان ابزار را با تنظیمات مبتنی بر محیط آزمایش کنید (۳۰ دقیقه)  

## راهنمای تخصیص زمان  

برای کمک به شما در استفاده بهینه از زمان ۲۰ ساعته دوره، در اینجا یک تقسیم‌بندی پیشنهادی ارائه شده است:  
...
| ارزیابی شخصی | ۲ ساعت | آزمایش درک خود از طریق سوالات و تأمل |  
...
- [Microsoft Phi Model Card](https://huggingface.co/microsoft/phi-2) - مرجع برای یک مدل پیشرو بهینه‌شده برای لبه  
...
- [OpenVINO GenAI](https://github.com/openvinotoolkit/openvino.genai) - ابزار استقرار مدل‌های زبانی بزرگ  

## قالب پیگیری پیشرفت  

از این قالب ساده برای پیگیری پیشرفت یادگیری خود در طول دوره ۲۰ ساعته استفاده کنید:  

| ماژول | تاریخ تکمیل | زمان صرف شده | نکات کلیدی |  
|-------|-------------|--------------|------------|  
| ماژول ۱: مبانی EdgeAI | | | |  
| ماژول ۲: مبانی SLM | | | |  
| ماژول ۳: استقرار SLM | | | |  
...
| ماژول ۸: ابزار Foundry Local | | | |  
| تمرین‌های عملی | | | |  
| پروژه کوچک | | | |  

## ایده‌های پروژه کوچک  

یکی از این پروژه‌ها را برای تمرین مفاهیم EdgeAI کامل کنید (هر کدام طراحی شده‌اند تا ۲-۴ ساعت زمان ببرند):  

### پروژه‌های مبتدی (۲-۳ ساعت هر کدام)  
1. **دستیار متنی لبه**: یک ابزار تکمیل متن ساده آفلاین با استفاده از یک مدل زبانی کوچک ایجاد کنید  
2. **داشبورد مقایسه مدل**: یک تجسم پایه از معیارهای عملکرد در مدل‌های مختلف SLM بسازید  
3. **آزمایش بهینه‌سازی**: تأثیر سطوح مختلف کمیت‌سازی را روی یک مدل پایه اندازه‌گیری کنید  

### پروژه‌های متوسط (۳-۴ ساعت هر کدام)  
4. **جریان کاری ابزار AI Toolkit**: از ابزار AI Toolkit در VS Code برای بهینه‌سازی و استقرار یک مدل از ابتدا تا انتها استفاده کنید  
5. **برنامه Windows AI Foundry**: یک برنامه Windows با استفاده از API Phi Silica و بهینه‌سازی NPU ایجاد کنید  
6. **استقرار چند پلتفرمی**: یک مدل بهینه‌شده را در Windows (OpenVINO) و موبایل (.NET MAUI) مستقر کنید  
7. **عامل فراخوانی توابع**: یک عامل هوش مصنوعی با قابلیت فراخوانی توابع برای سناریوهای لبه بسازید  

### پروژه‌های ادغام پیشرفته (۴-۵ ساعت هر کدام)  
8. **خط لوله بهینه‌سازی OpenVINO**: پیاده‌سازی بهینه‌سازی کامل مدل با استفاده از NNCF و ابزار GenAI  
9. **خط لوله SLMOps**: پیاده‌سازی چرخه کامل عمر مدل از آموزش تا استقرار در لبه  
10. **سیستم چندمدلی لبه**: استقرار چندین مدل تخصصی که به صورت مشترک روی سخت‌افزار لبه کار می‌کنند  
11. **سیستم یکپارچه MCP**: ساخت یک سیستم عامل‌محور با استفاده از پروتکل Model Context برای تعامل ابزارها  
...
- ابزارهای توسعه‌دهنده (ویندوز و VS Code)  
  - ابزارهای AI برای VS Code: https://learn.microsoft.com/en-us/azure/ai-toolkit/overview  
...
EdgeAI نمایانگر پیشرفته‌ترین مرزهای پیاده‌سازی هوش مصنوعی است که قابلیت‌های قدرتمند را مستقیماً به دستگاه‌ها می‌آورد و در عین حال نگرانی‌های مهمی مانند حریم خصوصی، تأخیر و اتصال را برطرف می‌کند. این دوره ۲۰ ساعته دانش ضروری و مهارت‌های عملی را برای شروع کار با فناوری‌های EdgeAI به شما ارائه می‌دهد.  

این دوره به طور عمدی مختصر و متمرکز بر مهم‌ترین مفاهیم طراحی شده است تا شما بتوانید به سرعت تخصص ارزشمندی کسب کنید بدون اینکه زمان زیادی صرف کنید. به یاد داشته باشید که تمرین عملی، حتی با مثال‌های ساده، کلید تقویت آنچه آموخته‌اید است.  
