...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T12:19:26+00:00",
...
مدل‌های هوش مصنوعی را به‌عنوان ابزارهای ماژولار و قابل تنظیم در نظر بگیرید که به‌صورت مستقیم روی دستگاه با Foundry Local اجرا می‌شوند. این جلسه بر روی جریان‌های کاری عملی برای استنتاج با حفظ حریم خصوصی و تأخیر کم تأکید دارد و نحوه ادغام این ابزارها از طریق SDKها، APIها یا CLI را آموزش می‌دهد. همچنین یاد خواهید گرفت که در صورت نیاز به Azure AI Foundry مقیاس‌بندی کنید.

> **🔄 به‌روزرسانی شده برای SDK مدرن**: این ماژول با الگوهای مخزن Foundry-Local مایکروسافت هماهنگ شده و با پیاده‌سازی مسیریابی هوشمند در `samples/06/` مطابقت دارد. مثال‌ها اکنون از `foundry-local-sdk` مدرن و استراتژی‌های پیشرفته انتخاب مدل استفاده می‌کنند.
...
- **پایش سلامت**: اعتبارسنجی سرویس و بررسی در دسترس بودن مدل
...
هدف: استفاده از SDK Foundry Local با SDK پایتون OpenAI برای ادغام بی‌دردسر.
...
هدف: تنظیم خروجی‌ها برای یک دامنه با استفاده از قالب‌های پرسش و طرح JSON.

مرحله 1) ایجاد قالب پرسش دامنه  
```python
# domain/templates.py
...
- **الگوهای پیشرفته**: بررسی فراخوانی توابع و ارکستراسیون چند عاملی در ماژول 5

## جمع‌بندی

Foundry Local امکان اجرای قدرتمند هوش مصنوعی روی دستگاه را فراهم می‌کند، جایی که مدل‌ها به ابزارهای هوشمند و تخصصی تبدیل می‌شوند. با انتخاب خودکار مدل، پایش جامع و الگوهای آماده تولید، تیم‌ها می‌توانند برنامه‌های هوش مصنوعی پیچیده‌ای را ارائه دهند که به انواع وظایف مختلف سازگار می‌شوند و در عین حال حریم خصوصی و عملکرد را حفظ می‌کنند. الگوی مسیریاب هوشمند ارائه شده در اینجا پایه‌ای برای ساخت سیستم‌های پیچیده هوش مصنوعی فراهم می‌کند که می‌توانند از توسعه محلی به استقرار تولید مقیاس‌بندی شوند.
