...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-25T02:05:19+00:00",
  "source_file": "Module08/02.AzureAIFoundryIntegration.md",
  "language_code": "hr"
}
-->
# Sesija 2: Integracija OpenAI SDK-a s Azure AI Foundry

## Pregled

Nadovezujući se na temelje Foundry Local-a, ova sesija fokusira se na napredne obrasce integracije OpenAI SDK-a koji podržavaju i Microsoft Foundry Local i Azure OpenAI. Naučit ćete kako izraditi fleksibilne AI aplikacije koje mogu raditi lokalno radi privatnosti i razvoja, dok istovremeno omogućuju skalabilnost u oblaku putem Azure OpenAI-a kada je to potrebno.

## Ciljevi učenja

Na kraju ove sesije, moći ćete:
- Ovladati naprednom integracijom OpenAI SDK-a s Foundry Local i Azure OpenAI
- Implementirati streaming odgovore za poboljšano korisničko iskustvo
- Kreirati robusne obrasce za klijentske tvornice s podrškom za više pružatelja usluga
- Izgraditi sustave za upravljanje razgovorima uz očuvanje konteksta
- Uspostaviti mjerenje performansi i praćenje zdravlja sustava
- Implementirati aplikacije spremne za produkciju uz pravilno rukovanje greškama

## Preduvjeti

- Završena Sesija 1: Uvod u Foundry Local
- Aktivna instalacija Foundry Local-a s pokrenutim modelima
- Python 3.8 ili noviji s mogućnošću virtualnog okruženja
- Instaliran OpenAI Python SDK (`pip install openai foundry-local-sdk`)
- Azure račun s OpenAI uslugom (opcionalno, za scenarije u oblaku)
- Osnovno razumijevanje Python async/await obrazaca

## Dio 1: Obrazac za klijentsku tvornicu OpenAI SDK-a

### Razumijevanje arhitekture s više pružatelja usluga

Izrada aplikacija koje rade s Foundry Local i Azure OpenAI zahtijeva fleksibilan obrazac za kreiranje klijenata:
...
## Dio 2: Streaming odgovori i interakcija u stvarnom vremenu

### Implementacija streaming chat odgovora

Streaming omogućuje bolje korisničko iskustvo prikazivanjem odgovora dok se generiraju:
...
### Upravljanje razgovorima s više izmjena
...
## Dio 3: Benchmarking performansi i analiza

### Mjerenje vremena odgovora

Mjerite i uspoređujte performanse u različitim konfiguracijama:
...
### Testiranje temperature i parametara
...
## Dio 4: Praćenje zdravlja sustava i dijagnostika

### Sveobuhvatan sustav provjere zdravlja
...
### Validator konfiguracije okruženja
...
## Dio 5: Varijable okruženja i upravljanje konfiguracijom

### Referenca varijabli okruženja

Kompletna referenca za konfiguraciju oba pružatelja usluga:
...
**Postavljanje u Windows Command Promptu:**
...
## Dio 6: Praktične vježbe

### Vježba 1: Integracija SDK-a s više pružatelja usluga

Izradite kompletnu aplikaciju koja se neprimjetno prebacuje između pružatelja usluga:
...
### Vježba 2: Napredno upravljanje razgovorima
...
### Vježba 3: Praćenje zdravlja sustava spremno za produkciju
...
## Dio 7: Sažetak i sljedeći koraci

### Postignuća sesije

U ovoj sesiji ste savladali:
- ✅ **Integraciju OpenAI SDK-a**: Napredni obrasci za Foundry Local i Azure OpenAI
- ✅ **Streaming odgovore**: Chat odgovori u stvarnom vremenu za poboljšano korisničko iskustvo
- ✅ **Podršku za više pružatelja usluga**: Neprimjetno prebacivanje između lokalnih i cloud AI usluga
- ✅ **Upravljanje razgovorima**: Razgovori s očuvanjem konteksta
- ✅ **Praćenje performansi**: Benchmarking i provjera zdravlja za produkcijske implementacije
- ✅ **Obrasce za produkciju**: Robusno rukovanje greškama i upravljanje konfiguracijom

### Ključni arhitekturni obrasci

**Obrazac za klijentsku tvornicu:**
```
...
1. **🔄 Uvijek implementirajte rezervne opcije**: Azure → Foundry Local → Rukovanje greškama
2. **🌊 Koristite streaming za duge odgovore**: Bolja percepcija performansi
3. **🛡️ Implementirajte sveobuhvatno rukovanje greškama**: Korisnički prihvatljive poruke o greškama
4. **📈 Pratite performanse**: Mjerite vrijeme odgovora i stope uspješnosti
5. **⚙️ Konfiguracija temeljena na okruženju**: Jednostavno prebacivanje između dev/staging/prod
6. **🔒 Sigurno upravljanje vjerodajnicama**: Nikada ne hardkodirajte API ključeve

### Smjernice za odabir pružatelja usluga

| Scenarij | Preporučeni pružatelj usluga | Razlog |
|----------|-----------------------------|--------|
| **Razvoj** | Foundry Local | Brza iteracija, bez troškova API-ja |
| **Privatnost** | Foundry Local | Podaci ne napuštaju uređaj |
| **Produkcija velikog obujma** | Azure OpenAI | Bolja skalabilnost, SLA za poduzeća |
| **Najnoviji modeli** | Azure OpenAI | Pristup najnovijim modelima |
| **Offline zahtjevi** | Foundry Local | Nema ovisnosti o internetu |
| **Osjetljivost na troškove** | Foundry Local | Bez troškova po tokenu |

### Brza referenca varijabli okruženja
...
### Priprema za Sesiju 3: Open-Source modeli

1. **Istražite katalog modela**: Pregledajte dostupne modele u Foundry Local
2. **Razumijevanje formata modela**: Naučite o ONNX-u, kvantizaciji i optimizaciji
3. **Razmotrite prilagođene modele**: Razmislite o zahtjevima za modele specifične za domenu
...
- **[OpenAI Python SDK Dokumentacija](https://github.com/openai/openai-python)**: Službena referenca za SDK
- **[Azure OpenAI Dokumentacija](https://learn.microsoft.com/azure/ai-services/openai/)**: Vodič za Azure OpenAI uslugu
- **[Foundry Local SDK Referenca](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**: Dokumentacija za lokalnu inferenciju
- **[Vodič za streaming odgovore](https://learn.microsoft.com/azure/ai-foundry/foundry-local/how-to/integrate-with-inference-sdks)**: Napredni obrasci za streaming
- **[Primjer 01: Brzi chat putem OpenAI SDK-a](samples/01/README.md)**: Osnovni obrasci integracije
- **[Primjer 02: Napredna integracija SDK-a](samples/02/README.md)**: Praktični primjeri iz ove sesije
- **[Primjer 04: Chainlit aplikacija](samples/04/README.md)**: Razvoj web sučelja
- **[Primjer 05: Sustavi s više agenata](samples/05/README.md)**: Napredni obrasci orkestracije

Sada ste opremljeni za izradu sofisticiranih AI aplikacija koje neprimjetno integriraju lokalne i cloud AI mogućnosti, pružajući fleksibilnost u odabiru pravog pružatelja usluga za svaki specifični slučaj upotrebe uz održavanje dosljednih obrazaca razvoja.
