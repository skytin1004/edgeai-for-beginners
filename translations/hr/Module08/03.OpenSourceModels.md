...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-01T01:39:44+00:00",
...
- Razumjeti obrasce predmemorije modela i lokalnog pohranjivanja
...
- Osnovno razumijevanje vrsta modela i slučajeva upotrebe

## Pregled

Ova sesija istražuje kako integrirati modele otvorenog koda u Foundry Local, uključujući odabir modela zajednice, integraciju sadržaja Hugging Face i usvajanje strategija "donosi vlastiti model" (BYOM). Također ćete otkriti seriju Model Mondays za kontinuirano učenje i otkrivanje modela.

## Ciljevi učenja

- Otkrivanje i procjena modela otvorenog koda za lokalnu inferenciju
- Kompilacija i pokretanje odabranih Hugging Face modela unutar Foundry Local
...
**Srednji modeli:**
- `qwen2.5-7b`: Izvrsno zaključivanje i duži kontekst
- `deepseek-r1-7b`: Optimiziran za generiranje koda

**Veći modeli:**
- `llama-3.2`: Najnoviji otvoreni model od Mete
- `qwen2.5-14b`: Razina za poslovno zaključivanje

## Dio 2: Brzo testiranje i usporedba modela

### Pristup uzoraka 03: Jednostavan popis i usporedba

Na temelju našeg uzorka 03, evo minimalnog tijeka rada:
...
Kad model radi, testirajte ga s dosljednim upitima:
...
### Alternativa testiranju u PowerShellu
...
### Odabir modela prema slučaju upotrebe

**Za općeniti razgovor i pitanja:**
- Započnite s: `phi-4-mini` (brz, učinkovit)
- Nadogradite na: `phi-4` (bolje zaključivanje)
- Napredno: `qwen2.5-7b` (duži kontekst)

**Za generiranje koda:**
- Preporučeno: `deepseek-r1-7b`
- Alternativa: `qwen2.5-7b` (također dobar za kod)

**Za složeno zaključivanje:**
- Najbolje: `qwen2.5-7b` ili `qwen2.5-14b`
- Opcija za budžet: `phi-4`
...
- Podrška za NPU (Windows 11 Copilot+ računala) za ubrzanje
...
Izradite vlastiti skript za procjenu modela na temelju Uzorka 03:
...
1. **Pokrenite skript Uzorka 03**: `samples\03\list_and_bench.cmd`
...
4. **Dokumentirajte nalaze**: Izradite jednostavan grafikon usporedbe
...
- Razmotrite bržu pohranu (SSD)

### Najbolje prakse

1. **Započnite s malim modelima**: Počnite s `phi-4-mini` za provjeru postavki
...
4. **Testirajte dosljedno**: Koristite iste upite za poštene usporedbe
5. **Dokumentirajte rezultate**: Vodite bilješke o performansama modela za svoje slučajeve upotrebe
...
- **Preduvjeti**: Ugodno prebacivanje modela i osnovno testiranje performansi
...
- **[Uzorak 03: Otkrivanje modela](samples/03/README.md)**: Praktični primjer skripta

### Ključni zaključci

✅ **Otkrivanje modela**: Koristite `foundry model list` za istraživanje dostupnih modela  
✅ **Brzo testiranje**: Uzorak `list_and_bench.cmd` za brzu procjenu  
✅ **Praćenje performansi**: Osnovno praćenje korištenja resursa i vremena odgovora  
✅ **Odabir modela**: Praktične smjernice za odabir modela prema slučaju upotrebe  
