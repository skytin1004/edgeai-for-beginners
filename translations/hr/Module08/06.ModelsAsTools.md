...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-23T01:09:22+00:00",
...
Tretirajte AI modele kao modularne, prilagodljive alate koji se pokreću izravno na uređaju uz Foundry Local. Ova sesija naglašava praktične radne procese za očuvanje privatnosti, inferenciju s niskom latencijom i kako integrirati ove alate putem SDK-ova, API-ja ili CLI-ja. Također ćete naučiti kako skalirati na Azure AI Foundry kada je to potrebno.
...
- Integrirati putem OpenAI-kompatibilnog REST API-ja ili SDK-ova
- Prilagoditi modele specifičnim domenama
- Planirati hibridno skaliranje na Azure AI Foundry

## Dio 1: Apstrakcije alata (Korak po korak)

Cilj: Predstaviti modele kao alate s jasnim ugovorima i jednostavnim usmjerivačem.

Korak 1) Definirajte sučelje alata i registar  
```python
# tools/registry.py
...
Korak 2) Implementirajte dva alata podržana Foundry Localom  
```python
# tools/impl.py
import requests, os
...
Korak 3) Usmjerivač prema zadatku  
```python
# tools/router.py
...
## Dio 2: SDK i API integracija (Korak po korak)

Cilj: Koristiti OpenAI Python SDK na Foundry Local endpointu.
...
## Dio 4: Offline i sigurnosni pristup (Korak po korak)

Cilj: Osigurati privatnost i otpornost prilikom lokalnog pokretanja modela kao alata.

Korak 1) Predzagrijte i validirajte lokalni endpoint  
```cmd
...
Korak 3) Lokalna zastavica i zapisivanje  
```python
...
## Dio 5: Skaliranje na Azure AI Foundry (Korak po korak)

Cilj: Ogledati lokalne modele s Azure endpointima za dodatni kapacitet.

Korak 1) Odlučite o strategiji usmjeravanja  
- Lokalno prvo za privatnost/latenciju, Azure kao rezervna opcija u slučaju grešaka ili velikih upita  

Korak 2) Implementirajte jednostavni usmjerivač  
```python
# hybrid/router.py
import os, requests
...
## Praktični popis zadataka
- [ ] Registrirajte barem dva alata i usmjerite zahtjeve  
- [ ] Pozovite Foundry Local putem OpenAI SDK-a i sirovog REST-a  
- [ ] Osigurajte JSON izlaze za predložak domene  
- [ ] Sanitizirajte i zapisujte pozive lokalno  
- [ ] Implementirajte jednostavni hibridni usmjerivač s Azure rezervnom opcijom  

## Zaključak

Foundry Local omogućuje robusnu AI funkcionalnost na uređaju gdje modeli postaju sastavljivi alati. Uz jasna sučelja, upravljanje i hibridno skaliranje, timovi mogu isporučiti aplikacije za real-time, sigurnu AI funkcionalnost koje poštuju privatnost korisnika, a istovremeno su spremne za poslovnu primjenu.
