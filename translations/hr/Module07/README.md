...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-23T00:09:17+00:00",
...
- Cijena od samo 249 dolara, čineći platformu najpristupačnijom za programere, studente i entuzijaste

### Primjene
Jetson Orin Nano izvrsno pokreće moderne generativne AI modele, uključujući vizijske transformatore, velike jezične modele i modele koji kombiniraju viziju i jezik. Posebno je dizajniran za GenAI slučajeve upotrebe, omogućujući pokretanje nekoliko LLM-ova na uređaju veličine dlana. Popularne primjene uključuju AI-pokretane robote, pametne dronove, inteligentne kamere i autonomne rubne uređaje.

**Saznajte više**: [NVIDIA Jetson Orin Nano SuperComputer: Sljedeća velika stvar u EdgeAI](https://medium.com/data-science-in-your-pocket/nvidias-jetson-orin-nano-supercomputer-the-next-big-thing-in-edgeai-e9eff687ae62)
...
- Optimizacije specifične za platformu poput CoreML za iOS i NNAPI za Android putem ONNX Runtime
...
- Pokretanje SLM-ova/LLM-ova i multimodalnih modela na uređaju i u oblaku putem ONNX Runtime
- Optimizirano za implementaciju na razini poduzeća
- Podrška za kontinuirano ažuriranje i upravljanje modelima

### Slučajevi upotrebe
Azure EdgeAI implementacija izvrsna je u scenarijima koji zahtijevaju AI implementaciju na razini poduzeća s mogućnostima upravljanja u oblaku. To uključuje inteligentnu obradu dokumenata, analitiku u stvarnom vremenu i hibridne AI radne tokove koji koriste resurse oblaka i ruba.

**Saznajte više**: [Azure EdgeAI SLM Motor](https://github.com/microsoft/onnxruntime-genai/tree/main/examples/slm_engine)

## 4. EdgeAI s Windows ML

Windows ML predstavlja Microsoftov najmoderniji runtime optimiziran za učinkovitu inferenciju modela na uređaju i pojednostavljenu implementaciju, služeći kao temelj Windows AI Foundryja. Ova platforma omogućuje programerima izradu AI-pokretanih Windows aplikacija koje koriste puni spektar PC hardvera.
...
- Omogućuje programerima da koriste vlastite modele i učinkovito ih implementiraju na ekosustav silikonskih partnera, uključujući AMD, Intel, NVIDIA i Qualcomm, pokrivajući CPU, GPU, NPU
- Zahvaljujući infrastrukturnim API-jevima, programeri više ne moraju izrađivati više verzija aplikacije za različite vrste hardvera
...
**Saznajte više**: 
- [Pregled Windows ML](https://learn.microsoft.com/en-us/windows/ai/new-windows-ml/overview)
- [Vodič za razvoj Windows EdgeAI](../windowdeveloper.md) - Sveobuhvatan vodič za razvoj Windows Edge AI

## 5. EdgeAI s Foundry Local aplikacijama

Foundry Local omogućuje programerima izradu aplikacija za Retrieval Augmented Generation (RAG) koristeći lokalne resurse u .NET-u, kombinirajući lokalne jezične modele sa semantičkim mogućnostima pretraživanja. Ovaj pristup pruža AI rješenja usmjerena na privatnost koja rade isključivo na lokalnoj infrastrukturi.
...
Microsoft Foundry Local pruža OpenAI-kompatibilni REST server pokretan ONNX Runtimeom za pokretanje modela lokalno na Windowsu. Ispod je brzi, provjereni sažetak; pogledajte službenu dokumentaciju za sve detalje.
...
Instalacija ili nadogradnja na Windowsu (cmd.exe):
```cmd
...
- Donošenje vlastitog modela (kompilacija): https://learn.microsoft.com/azure/ai-foundry/foundry-local/how-to/how-to-compile-hugging-face-models

## Resursi za razvoj Windows EdgeAI

Za programere koji ciljaju Windows platformu, kreirali smo sveobuhvatan vodič koji pokriva cijeli Windows EdgeAI ekosustav. Ovaj resurs pruža detaljne informacije o Windows AI Foundryju, uključujući API-je, alate i najbolje prakse za razvoj EdgeAI na Windowsu.

### Windows AI Foundry Platforma
Windows AI Foundry platforma pruža sveobuhvatan set alata i API-ja posebno dizajniranih za razvoj Edge AI na Windows uređajima. To uključuje specijaliziranu podršku za NPU-akcelerirani hardver, integraciju Windows ML-a i tehnike optimizacije specifične za platformu.

**Sveobuhvatan vodič**: [Vodič za razvoj Windows EdgeAI](../windowdeveloper.md)
...
- Integraciju i optimizaciju Windows ML runtimea
...
### AI Toolkit za razvoj Edge AI
Za programere koji koriste Visual Studio Code, AI Toolkit ekstenzija pruža sveobuhvatno razvojno okruženje posebno dizajnirano za izradu, testiranje i implementaciju Edge AI aplikacija. Ovaj alat pojednostavljuje cijeli tijek rada razvoja Edge AI unutar VS Code-a.

**Vodič za razvoj**: [AI Toolkit za razvoj Edge AI](../aitoolkit.md)

Vodič za AI Toolkit pokriva:
- Otkrivanje i odabir modela za implementaciju na rubu
- Lokalni testni i optimizacijski tijekovi rada
- ONNX i Ollama integracija za rubne modele
- Tehnike konverzije i kvantizacije modela
- Razvoj agenata za rubne scenarije
- Procjena performansi i praćenje
- Priprema za implementaciju i najbolje prakse

## Zaključak

Ovih pet EdgeAI implementacija demonstriraju zrelost i raznolikost rubnih AI rješenja dostupnih danas. Od hardverski ubrzanih rubnih uređaja poput Jetson Orin Nano do softverskih okvira poput ONNX Runtime GenAI i Windows ML-a, programeri imaju neviđene opcije za implementaciju inteligentnih aplikacija na rubu.
...
Svaka platforma nudi jedinstvene prednosti: Jetson Orin Nano za hardverski ubrzano rubno računalstvo, ONNX Runtime GenAI za razvoj mobilnih aplikacija na više platformi, Azure EdgeAI za integraciju oblaka i ruba na razini poduzeća, Windows ML za nativne Windows aplikacije i Foundry Local za implementacije RAG usmjerene na privatnost. Zajedno, one predstavljaju sveobuhvatan ekosustav za razvoj EdgeAI.
