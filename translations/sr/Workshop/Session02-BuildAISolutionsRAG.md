...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-08T14:08:51+00:00",
...
Истражите како да изградите применљиве GenAI токове рада користећи Foundry Local и Azure AI Foundry. Научите напредно креирање упита, интегришите структуриране податке и оркестрирајте задатке са репродуктивним цевоводима. Иако је фокус на Retrieval-Augmented Generation (RAG) за Q&A докумената и података, обрасци се могу применити на шири дизајн GenAI решења.

## Циљеви учења

До краја ове сесије, моћи ћете:

- **Овладати креирањем упита**: Дизајнирајте ефикасне системске упите и стратегије за утемељење
- **Применити RAG обрасце**: Изградите Q&A системе засноване на документима са претрагом вектора
- **Интегрисати структуриране податке**: Радите са CSV, JSON и табеларним подацима у AI токовима рада
- **Изградити продукцијски RAG**: Креирајте скалабилне RAG апликације са Chainlit
...
- Основно разумевање векторских база података и уграђивања
- Искусто у програмирању на Python-у
- Познавање концепата обраде докумената

### Брзи почетак за окружење на више платформи (Windows и macOS)
...
Ако Foundry Local macOS бинарни фајлови још нису доступни у вашем окружењу, покрените сервис на Windows VM-у или контејнеру и поставите:
```bash
...
### 1. Системски упити и стратегије за утемељење (10 минута)

#### Корак 1.1: Напредно креирање упита
...
#### Корак 1.2: Тестирајте стратегије за утемељење
...
### 2. Интеграција табеларних података са упитима (CSV Q&A) (10 минута)
...
### 4. Приказ пута миграције CLI-а ка Azure-у (5 минута)
...
### 1. Напредно креирање упита

- **Системски упити**: Експертске персоне специфичне за домен
- **Стратегије за утемељење**: Технике интеграције контекста
...
- **Претрага вектора**: TF-IDF и косинусна сличност
- **Проналажење докумената**: Оцењивање релевантности и рангирање
- **Комбинација контекста**: Синтеза више докумената
- **Генерисање одговора**: Креирање одговора заснованих на утемељењу

### 4. Стратегије миграције ка облаку

- **Уједињени API-ји**: Једна база кода за локално и облачно окружење
- **Апстракција окружења**: Деплојмент заснован на конфигурацији
- **Радни ток развоја**: Локално → Стагинг → Продукција
...
3. **Напредна претрага вектора**: Интегришите са Chroma или Pinecone
4. **Миграција ка облаку**: Деплојте на Azure AI Foundry
5. **Оцените квалитет RAG-а**: Покрените `python Workshop/samples/session02/rag_eval_ragas.py` за мерење релевантности одговора, веродостојности и прецизности контекста користећи ragas

### Опциона унапређења

| Категорија | Унапређење | Разлог | Упутство |
|------------|------------|--------|----------|
| Проналажење | Замените TF-IDF са векторском базом (FAISS / Chroma) | Боља семантичка прецизност и скалабилност | Сегментирајте документе (500–800 карактера), уградите, сачувајте индекс |
| Хибридни индекс | Двојно семантичко + филтрирање кључних речи | Побољшава прецизност за нумеричке / код упите | Филтрирајте по кључној речи, затим рангирајте по косинусној сличности |
| Уграђивање | Оцените више модела за уграђивање | Оптимизујте релевантност у односу на брзину | A/B: MiniLM vs E5-small vs локално хостовани енкодер |
| Кеширање | Кеширајте резултате уграђивања и проналажења | Смањите латенцију поновљених упита | Једноставно на диску pickle / sqlite са хеш кључем |
| Евалуација | Проширите ragas скуп података | Статистички значајан квалитет | Креирајте 50–100 Q/A + контекста; стратификујте по теми |
| Метрике | Пратите време проналажења и генерисања | Профилисање перформанси | Снимите `retrieval_ms`, `gen_ms`, `tokens` по позиву |
| Заштитне мере | Додајте fallback за халуцинације | Безбеднији одговори | Ако је веродостојност < праг → одговор: "Недовољан контекст." |
| Резервни план | Каскадно локално → Azure модел | Хибридно побољшање квалитета | На ниској сигурности преусмерите на облак преко истог OpenAI API-ја |
| Детерминизам | Стабилна поређења | Поновљиви скупови евалуације | Фиксирајте seed, `temperature=0`, онемогућите случајност узорковања |
| Праћење | Сачувајте историју евалуације | Детекција регресије | Додајте JSON линије са временском ознаком + метрикским променама |

#### Пример: Додавање времена проналажења
...
- [Водич за креирање упита](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/advanced-prompt-engineering)
...
| Скрипт / Бележница радионице | Сценарио | Циљ | Основни скуп података / Извор | Пример питања |
|-----------------------------|----------|-----|------------------------------|---------------|
| `samples/session02/rag_pipeline.py` / `notebooks/session02_rag_pipeline.ipynb` | Интерна база знања за подршку која одговара на питања о приватности и перформансама | Минимални RAG у меморији са уграђивањем | `DOCS` листа у скрипту (5 кратких пасуса) | Зашто користити RAG са локалним инференцијама? |
| `samples/session02/rag_eval_ragas.py` / `notebooks/session02_rag_eval_ragas.ipynb` | Аналитичар квалитета који успоставља основне метрике веродостојности проналажења | Израчунајте ragas метрике на малом синтетичком скупу података | `DOCS`, `QUESTIONS`, `GROUND_TRUTH` низови | Која је предност локалних инференција? |
| `prompt_engineering.py` (напредно) | SME за домен који креира утемељене упите за више вертикала | Упоредите системске упите за домен и утицај токена | Уграђени `contexts` речник | Како Foundry Local управља кеширањем модела? |
| `csv_qa_system.py` | Операције продаје које истражују интерактивну аналитику преко извоза | Сумирајте и упитајте мали део продаје | Генерисани `sample_sales_data.csv` (10 редова) | Који производ има највећи просечан износ продаје? |
| `document_rag.py` | Тим за производе који истражује RAG докумената за интерну вики | Проналажење + цитирање релевантних докумената | `create_sample_knowledge_base()` листа | Које су предности Edge AI-а? |
| `migration_guide.py` | Архитекта који припрема план миграције ка облаку | Демонстрирајте паритет локалног → Azure API-ја | Статички тест упити | Објасните предности Edge AI-а у 2–3 реченице. |
...
Истинити парови за евалуацију ragas:
```python
...
Група за подршку инжењеринга жели брз прототип за одговарање на интерна питања без излагања података о клијентима екстерно. Артефакти из Сесије 2 напредују од минималног ефермерног RAG-а (без перзистенције) → структурирани CSV Q&A → проналажење докумената са цитирањем → објективна евалуација квалитета (ragas) → стратегија миграције спремна за Azure стагинг.

### Путеви за проширење

Користите табелу Опционих унапређења за еволуцију: замените TF‑IDF са FAISS/Chroma, проширите корпус за евалуацију (50–100 Q/A), додајте fallback ескалацију на већи модел када је веродостојност < праг.
