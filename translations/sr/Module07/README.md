...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-26T19:44:32+00:00",
...
Ово решење показује како интегрисати генеративну AI и моделе великог језика (LLMs) у мобилне апликације које раде на више платформи користећи .NET MAUI (Multi-platform App UI) и ONNX Runtime GenAI. Овај приступ омогућава .NET програмерима да креирају софистициране AI мобилне апликације које раде нативно на Android и iOS уређајима.
...
Microsoft-ово Azure EdgeAI решење се фокусира на ефикасно распоређивање малих језичких модела (SLMs) у cloud-edge хибридним окружењима. Овај приступ премошћује јаз између cloud AI услуга и edge захтева за распоређивање.
...
Windows ML апстрахује хардвер и провајдере за извршење, тако да можете да се фокусирате на писање кода. Поред тога, Windows ML се аутоматски ажурира како би подржао најновије NPU, GPU и CPU компоненте чим буду доступне. Платформа пружа јединствени оквир за AI развој на разноврсном Windows хардверском екосистему.
...
Foundry Local омогућава програмерима за Windows и Mac да креирају Retrieval Augmented Generation (RAG) апликације користећи локалне ресурсе у .NET-у, комбинујући локалне језичке моделе са семантичким претраживањем. Овај приступ пружа решења фокусирана на приватност која раде искључиво на локалној инфраструктури.
...
RAG, или Retrieval Augmented Generation, је само сложен начин да се каже "потражи нешто и укључи то у упит". Ова локална имплементација обезбеђује приватност података уз пружање интелигентних одговора заснованих на прилагођеним базама знања. Приступ је посебно вредан за сценарије у предузећима који захтевају суверенитет података и могућности рада ван мреже.
...
Microsoft Foundry Local пружа OpenAI‑компатибилан REST сервер који покреће ONNX Runtime за покретање модела локално на Windows-у. Испод је брз, проверен резиме; погледајте званичну документацију за све детаље.
...
Инсталирање или ажурирање на Windows-у (cmd.exe):
```cmd
...
Windows AI Foundry платформа пружа свеобухватан сет алата и API-ја посебно дизајнираних за Edge AI развој на Windows уређајима. Ово укључује специјализовану подршку за NPU-акцелераторе, интеграцију Windows ML-а и технике оптимизације специфичне за платформу.
...
За програмере који користе Visual Studio Code, AI Toolkit екстензија пружа свеобухватно окружење за развој посебно дизајнирано за креирање, тестирање и распоређивање Edge AI апликација. Овај алат поједностављује цео Edge AI развојни процес унутар VS Code-а.
