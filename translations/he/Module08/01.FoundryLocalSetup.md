...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-25T00:01:14+00:00",
...
Microsoft Foundry Local מביאה את יכולות Azure AI Foundry ישירות לסביבת הפיתוח של Windows 11, ומאפשרת פיתוח AI עם שמירה על פרטיות, זמן תגובה מהיר וכלים ברמה ארגונית. מפגש זה מכסה התקנה מלאה, הגדרות ופריסה מעשית של מודלים פופולריים כמו phi, qwen, deepseek ו-GPT-OSS-20B.

## מטרות למידה

בסיום המפגש, תוכלו:
- להתקין ולהגדיר את Foundry Local על Windows 11
- לשלוט בפקודות CLI ואפשרויות הגדרה
- להבין אסטרטגיות שמירת מודלים (caching) לביצועים מיטביים
- להפעיל בהצלחה את המודלים phi, qwen, deepseek ו-GPT-OSS-20B
...
- **חומרה**: מכשיר עם NPU או GPU מועדף (מחשב Copilot+ או GPU של NVIDIA)  
...
Foundry Local מיישמת שמירת מודלים חכמה לשיפור ביצועים וניהול אחסון:
...
## חלק 5: יצירת האפליקציה הראשונה שלכם

### אפליקציית צ'אט מודרנית (OpenAI SDK + Foundry Local)

צרו אפליקציית צ'אט מוכנה לייצור באמצעות OpenAI SDK עם אינטגרציה ל-Foundry Local, לפי הדפוסים מהדוגמה Sample 01.
...
## חלק 6: פתרון בעיות ושיטות עבודה מומלצות
...
|-------|-------|------------|-------|
| `MODEL` | כינוי או שם המודל | `phi-4-mini` | לא |
...
- **השתמשו ב-OpenAI SDK**: העדיפו את OpenAI SDK על פני בקשות HTTP גולמיות לשיפור תחזוקה  
- **FoundryLocalManager**: השתמשו ב-SDK הרשמי לניהול שירותים כאשר זמין  
- **טיפול בשגיאות**: יישמו אסטרטגיות גיבוי מתאימות לאפליקציות בייצור  
- **עדכנו באופן קבוע**: שמרו על Foundry Local מעודכן כדי לגשת למודלים ותיקונים חדשים  
- **התחילו בקטן**: התחילו עם מודלים קטנים (Phi mini, Qwen 7B) והתקדמו  
...
- **סקירת פקודות CLI**: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/reference/reference-cli  
...
- **דוגמה 01: צ'אט מהיר עם OpenAI SDK**: samples/01/README.md  
