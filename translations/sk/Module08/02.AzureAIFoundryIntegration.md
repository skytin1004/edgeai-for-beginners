...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-25T01:30:04+00:00",
  "source_file": "Module08/02.AzureAIFoundryIntegration.md",
  "language_code": "sk"
}
-->
# Session 2: Integrácia OpenAI SDK s Azure AI Foundry

## Prehľad

Na základe vašich znalostí z Foundry Local sa táto relácia zameriava na pokročilé integračné vzory OpenAI SDK, ktoré bezproblémovo podporujú Microsoft Foundry Local aj Azure OpenAI. Naučíte sa vytvárať flexibilné AI aplikácie, ktoré môžu bežať lokálne pre ochranu súkromia a vývoj, pričom poskytujú škálovateľnosť v cloude prostredníctvom Azure OpenAI, keď je to potrebné.

## Ciele učenia

Na konci tejto relácie budete:
- Ovládať pokročilú integráciu OpenAI SDK s Foundry Local aj Azure OpenAI
- Implementovať streamovanie odpovedí pre lepší používateľský zážitok
- Vytvárať robustné vzory klientských tovární pre podporu viacerých poskytovateľov
- Budovať systémy správy konverzácií s uchovaním kontextu
- Zavádzať benchmarking výkonu a monitorovanie zdravia
- Nasadzovať aplikácie pripravené na produkciu s riadnym spracovaním chýb

## Predpoklady

- Dokončená relácia 1: Začíname s Foundry Local
- Aktívna inštalácia Foundry Local s bežiacimi modelmi
- Python 3.8 alebo novší s podporou virtuálneho prostredia
- Nainštalovaný OpenAI Python SDK (`pip install openai foundry-local-sdk`)
- Azure účet s OpenAI službou (voliteľné, pre cloudové scenáre)
- Základné pochopenie vzorov Python async/await

## Časť 1: Vzor klientskej továrne OpenAI SDK

### Pochopenie architektúry viacerých poskytovateľov

Budovanie aplikácií, ktoré fungujú s Foundry Local aj Azure OpenAI, vyžaduje flexibilný vzor na vytváranie klientov:
...
## Časť 2: Streamovanie odpovedí a interakcia v reálnom čase

### Implementácia streamovania chatových odpovedí

Streamovanie poskytuje lepší používateľský zážitok tým, že zobrazuje odpovede počas ich generovania:
...
### Správa konverzácií s viacerými kolami
...
## Časť 3: Benchmarking výkonu a analýza

### Meranie času odpovede

Merajte a porovnávajte výkon v rôznych konfiguráciách:
...
### Testovanie teploty a parametrov
...
## Časť 4: Monitorovanie zdravia služby a diagnostika

### Komplexný systém kontroly zdravia
...
### Validátor konfigurácie prostredia
...
## Časť 5: Premenné prostredia a správa konfigurácie

### Referencia premenných prostredia

Kompletná referencia pre konfiguráciu oboch poskytovateľov:
...
**Nastavenie vo Windows Command Prompt:**
...
## Časť 6: Praktické cvičenia

### Cvičenie 1: Integrácia SDK s viacerými poskytovateľmi

Vytvorte kompletnú aplikáciu, ktorá bezproblémovo prepína medzi poskytovateľmi:
...
### Cvičenie 2: Pokročilá správa konverzácií
...
### Cvičenie 3: Monitorovanie zdravia pripravené na produkciu
...
## Časť 7: Zhrnutie a ďalšie kroky

### Úspechy relácie

V tejto relácii ste zvládli:
- ✅ **Integráciu OpenAI SDK**: Pokročilé vzory pre Foundry Local aj Azure OpenAI
- ✅ **Streamovanie odpovedí**: Chatové odpovede v reálnom čase pre lepší používateľský zážitok
- ✅ **Podpora viacerých poskytovateľov**: Bezproblémové prepínanie medzi lokálnymi a cloudovými AI službami
- ✅ **Správa konverzácií**: Kontextovo citlivé konverzácie s viacerými kolami
- ✅ **Monitorovanie výkonu**: Benchmarking a kontrola zdravia pre produkčné nasadenia
- ✅ **Produkčné vzory**: Robustné spracovanie chýb a správa konfigurácie

### Kľúčové architektonické vzory

**Vzor klientskej továrne:**
```
...
1. **🔄 Vždy implementujte záložné riešenia**: Azure → Foundry Local → Spracovanie chýb
2. **🌊 Používajte streamovanie pre dlhé odpovede**: Lepší vnímaný výkon
3. **🛡️ Implementujte komplexné spracovanie chýb**: Používateľsky prívetivé chybové hlásenia
4. **📈 Monitorujte výkon**: Sledujte časy odpovedí a úspešnosť
5. **⚙️ Konfigurácia na základe prostredia**: Jednoduché prepínanie medzi vývojom/testovaním/produkciou
6. **🔒 Bezpečná správa poverení**: Nikdy neukladajte API kľúče priamo v kóde

### Usmernenia pre výber poskytovateľa

| Scenár | Odporúčaný poskytovateľ | Dôvody |
|--------|-------------------------|--------|
| **Vývoj** | Foundry Local | Rýchla iterácia, žiadne náklady na API |
| **Citlivé na súkromie** | Foundry Local | Dáta nikdy neopúšťajú zariadenie |
| **Produkcia s vysokým objemom** | Azure OpenAI | Lepšia škálovateľnosť, podniková SLA |
| **Najnovšie modely** | Azure OpenAI | Prístup k najnovším modelom |
| **Požiadavky na offline režim** | Foundry Local | Žiadna závislosť na internete |
| **Citlivé na náklady** | Foundry Local | Žiadne poplatky za tokeny |

### Rýchla referencia premenných prostredia
...
### Príprava na reláciu 3: Open-Source modely

1. **Preskúmajte katalóg modelov**: Prezrite si dostupné modely vo Foundry Local
2. **Pochopte formáty modelov**: Naučte sa o ONNX, kvantizácii a optimalizácii
3. **Zvážte vlastné modely**: Premýšľajte o požiadavkách na modely špecifické pre danú oblasť
...
- **[OpenAI Python SDK Dokumentácia](https://github.com/openai/openai-python)**: Oficiálna referencia SDK
- **[Azure OpenAI Dokumentácia](https://learn.microsoft.com/azure/ai-services/openai/)**: Sprievodca službou Azure OpenAI
- **[Foundry Local SDK Referencia](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**: Dokumentácia lokálneho inferenčného systému
- **[Sprievodca streamovaním odpovedí](https://learn.microsoft.com/azure/ai-foundry/foundry-local/how-to/integrate-with-inference-sdks)**: Pokročilé vzory streamovania
- **[Ukážka 01: Rýchly chat cez OpenAI SDK](samples/01/README.md)**: Základné integračné vzory
- **[Ukážka 02: Pokročilá integrácia SDK](samples/02/README.md)**: Praktické príklady z tejto relácie
- **[Ukážka 04: Chainlit aplikácia](samples/04/README.md)**: Vývoj webového UI
- **[Ukážka 05: Systémy s viacerými agentmi](samples/05/README.md)**: Pokročilé vzory orchestrácie

Teraz ste pripravení vytvárať sofistikované AI aplikácie, ktoré bezproblémovo integrujú lokálne a cloudové AI schopnosti, poskytujúc flexibilitu pri výbere správneho poskytovateľa pre každý konkrétny prípad použitia, pričom zachovávajú konzistentné vzory vývoja.
