...
| Deň 7-8 | Modul 4: Optimalizácia modelov | 4 hodiny |
| Deň 9-10 | Modul 5: SLMOps | 3 hodiny |
| Deň 11-12 | Modul 6: Agentické systémy SLM | 2 hodiny |
...
Kurz je rozdelený do siedmich komplexných modulov:
...
- Porozumieť rozdielom medzi AI založenou na cloude a AI na okraji
- Ovládnuť základné techniky optimalizácie pre prostredia s obmedzenými zdrojmi
...
#### Sekcia 2: Reálne prípadové štúdie
- **Prioritné koncepty**: 
...
2. **Preskúmanie modelu**: Stiahnite a preskúmajte predtrénovaný malý jazykový model (1 hodina)
3. **Základná kvantizácia**: Vyskúšajte jednoduchú kvantizáciu na malom modeli (1 hodina)

## Modul 2: Základy malých jazykových modelov (SLM)
...
- **Prioritné koncepty**: 
  ...
  - Architektúra pokročilého uvažovania
...
- **Prioritné koncepty**: 
  - Architektúra orientovaná na zariadenia
  - Systémová integrácia s Windows
  - Operácia zachovávajúca súkromie

#### Sekcia 6: Phi-Silica
- **Prioritné koncepty**: 
  - Architektúra optimalizovaná pre NPU
  - Výkonnostné metriky
  - Integrácia pre vývojárov

### Otázky na sebahodnotenie

1. Porovnajte architektonické prístupy rodín modelov Phi a Qwen.
2. Vysvetlite, ako sa technológia kvantizácie BitNET líši od tradičnej kvantizácie.
3. Aké sú jedinečné výhody modelu Mu pre integráciu s Windows?
4. Opíšte, ako Phi-Silica využíva hardvér NPU na optimalizáciu výkonu.
5. Pre mobilnú aplikáciu s obmedzenou konektivitou, ktorá rodina modelov by bola najvhodnejšia a prečo?

### Praktické cvičenia

1. **Porovnanie modelov**: Rýchly benchmark dvoch rôznych SLM modelov (1 hodina)
2. **Jednoduchá generácia textu**: Základná implementácia generácie textu s malým modelom (1 hodina)
3. **Rýchla optimalizácia**: Použite jednu techniku optimalizácie na zlepšenie rýchlosti inferencie (1 hodina)

## Modul 3: Nasadenie malých jazykových modelov (SLM)

### Kľúčové ciele učenia

- Vybrať vhodné modely na základe obmedzení nasadenia
- Ovládnuť techniky optimalizácie pre rôzne scenáre nasadenia
- Implementovať SLM v lokálnych aj cloudových prostrediach
- Navrhnúť konfigurácie pripravené na produkciu pre aplikácie EdgeAI
...
3. Vysvetlite výhody kontajnerizácie pre nasadenie SLM.
4. Aké sú kľúčové výkonnostné metriky na monitorovanie pre SLM nasadené na okraji?
...
2. **Kontrola výkonu**: Spustite rýchly benchmark na vašom nasadenom modeli (30 minút)
3. **Jednoduchá integrácia**: Vytvorte minimálnu aplikáciu, ktorá používa váš nasadený model (1 hodina)

## Modul 4: Konverzia formátov modelov a kvantizácia
...
- **Prioritné koncepty**: 
  ...
  - Pracovné postupy destilácie v Azure ML
...
- Vytvoriť inteligentných AI agentov pre prostredia na okraji pomocou malých jazykových modelov
- Implementovať schopnosti volania funkcií so systematickými pracovnými postupmi
...
- **Prioritné koncepty**: 
  - Rámec klasifikácie agentov (reflexní, založení na modeloch, založení na cieľoch, učební agenti)
  - Analýza kompromisov medzi SLM a LLM
  - Dizajnové vzory agentov špecifické pre okraj
...
1. Aké sú kľúčové architektonické úvahy pre agentov Edge AI?
2. Ako volanie funkcií zlepšuje schopnosti agentov?
3. Vysvetlite úlohu Model Context Protocol v komunikácii agentov.
...
- **Prioritné koncepty**: 
  - Komplexné prostredie vývoja Edge AI vo VS Code
  - Katalóg modelov a ich objavovanie pre nasadenie na okraji
  - Lokálne testovanie, optimalizácia a pracovné postupy vývoja agentov
  - Monitorovanie výkonu a hodnotenie pre scenáre na okraji

#### Sekcia 2: Príručka vývoja Windows EdgeAI
- **Prioritné koncepty**: 
...
#### Sekcia 3: Implementácie špecifické pre platformu
- **Prioritné koncepty**: 
  - Nasadenie NVIDIA Jetson Orin Nano (67 TOPS AI výkon)
...
1. Ako AI Toolkit zjednodušuje pracovný postup vývoja EdgeAI?
2. Porovnajte stratégie nasadenia na rôznych hardvérových platformách.
...
1. **Nastavenie AI Toolkit**: Konfigurujte AI Toolkit a optimalizujte model (1 hodina)
2. **Windows AI Foundry**: Vytvorte jednoduchú aplikáciu Windows AI pomocou Phi Silica API (1 hodina)
...
Aby ste čo najlepšie využili 20-hodinový časový rámec kurzu, tu je navrhované rozdelenie času:

| Aktivita | Časové rozdelenie | Popis |
|----------|----------------|-------------|
| Čítanie základných materiálov | 9 hodín | Zameranie na základné koncepty v každom module |
| Praktické cvičenia | 6 hodín | Praktická implementácia kľúčových techník |
| Sebahodnotenie | 2 hodiny | Testovanie porozumenia prostredníctvom otázok a reflexie |
| Mini-projekt | 3 hodiny | Aplikácia znalostí na malú praktickú implementáciu |
...
- Microsoft Learn (Foundry Local)
  ...
  - Integrácia s inference SDKs: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/how-to/how-to-integrate-with-inference-sdks
  ...
  - Kompilácia Hugging Face modelov: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/how-to/how-to-compile-hugging-face-models
- Azure AI Foundry
...
- Nástroje pre vývojárov (Windows a VS Code)
  - AI Toolkit pre VS Code: https://learn.microsoft.com/en-us/azure/ai-toolkit/overview
