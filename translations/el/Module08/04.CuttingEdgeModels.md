...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T22:29:19+00:00",
...
Αυτή η συνεδρία επικεντρώνεται στη δημιουργία εφαρμογών συνομιλίας έτοιμων για παραγωγή χρησιμοποιώντας το Chainlit και το Microsoft Foundry Local. Θα μάθετε να δημιουργείτε σύγχρονες διεπαφές ιστού για συνομιλίες AI, να υλοποιείτε αποκρίσεις σε ροή και να αναπτύσσετε ισχυρές εφαρμογές συνομιλίας με σωστή διαχείριση σφαλμάτων και σχεδιασμό εμπειρίας χρήστη.

**Τι θα δημιουργήσετε:**
- **Εφαρμογή Συνομιλίας Chainlit**: Σύγχρονη διεπαφή ιστού με αποκρίσεις σε ροή
- **WebGPU Demo**: Επεξεργασία στον περιηγητή για εφαρμογές με προτεραιότητα στην ιδιωτικότητα  
- **Ενσωμάτωση Open WebUI**: Επαγγελματική διεπαφή συνομιλίας με το Foundry Local
- **Παραγωγικά Μοτίβα**: Διαχείριση σφαλμάτων, παρακολούθηση και στρατηγικές ανάπτυξης

## Στόχοι Μάθησης

- Δημιουργία εφαρμογών συνομιλίας έτοιμων για παραγωγή με το Chainlit
- Υλοποίηση αποκρίσεων σε ροή για βελτιωμένη εμπειρία χρήστη
...
- **Μοντέλο**: Τουλάχιστον ένα μοντέλο φορτωμένο (`foundry model run phi-4-mini`)
...
| **Κόστος** | 💰 Δωρεάν μετά το υλικό | 💸 Πληρωμή ανά token |
...
Η εφαρμογή Δείγμα 04 παρουσιάζει μοτίβα έτοιμα για παραγωγή:
...
- **📚 Εκπαιδευτικό Υλικό**: Βήμα-βήμα οδηγίες μάθησης
...
**Χρήση VS Code:**
1. Ανοίξτε το VS Code στον φάκελο Module08
...
#### Άσκηση 1: Δοκιμή Ρύθμισης Πελάτη
...
#### Άσκηση 2: Προσομοίωση Απόκρισης σε Ροή
...
## Μέρος 5: Επίδειξη Επεξεργασίας στον Περιηγητή με WebGPU

### Επισκόπηση

Το WebGPU επιτρέπει την εκτέλεση μοντέλων AI απευθείας στον περιηγητή για μέγιστη ιδιωτικότητα και εμπειρίες χωρίς εγκατάσταση. Αυτό το δείγμα παρουσιάζει το ONNX Runtime Web με εκτέλεση WebGPU.
...
Δημιουργήστε φάκελο: `samples/04/webgpu-demo/`
...
**Σημείωση:** Το `host.docker.internal` επιτρέπει στα containers του Docker να έχουν πρόσβαση στον host υπολογιστή στα Windows.
...
3. **Διαμόρφωση Μοντέλου:**
   ...
   - Βασικό URL: `http://host.docker.internal:51211/v1`
   - Κλειδί API: `foundry-local-key` (οποιαδήποτε τιμή λειτουργεί)
4. **Δοκιμή Σύνδεσης:** Τα μοντέλα θα πρέπει να εμφανίζονται στο dropdown
...
2. **Μοντέλα Δεν Εμφανίζονται:**
   ...
   - Επανεκκινήστε το container του Open WebUI

## Μέρος 7: Σκέψεις για Ανάπτυξη Παραγωγής
...
Η εφαρμογή Δείγμα 04 παρουσιάζει βέλτιστες πρακτικές για τη δημιουργία ισχυρών διεπαφών συνομιλίας που αξιοποιούν τοπικά μοντέλα AI μέσω του Microsoft Foundry Local, προσφέροντας εξαιρετικές εμπειρίες χρήστη.
...
- **[Οδηγός Ενσωμάτωσης Open WebUI](https://github.com/microsoft/foundry-local/blob/main/docs/tutorials/chat-application-with-open-web-ui.md)**: Επίσημο tutorial
