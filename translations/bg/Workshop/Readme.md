...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-17T10:09:08+00:00",
...
Организации по целия свят преминават от AI, зависим от облака, към edge computing по три основни причини:

1. **Поверителност и съответствие** - Обработвайте чувствителни данни локално без предаване към облака (HIPAA, GDPR, финансови регулации)
2. **Производителност** - Елиминирайте мрежовата латентност (50-500ms локално срещу 500-2000ms облачно)
...
- Инженеринг на подканите за по-малки модели
...
2. **Създаване на приложения, захранвани от AI**
   - Имплементиране на OpenAI-съвместими чат завършвания локално
   - Дизайн на ефективни подканяния за Малки езикови модели
   - Работа с поточни отговори за по-добро потребителско изживяване
...
|-------|----------|-----------------------|
| **1** | Чат приложение с поточно предаване | Настройка на услуги, основни завършвания, UX за поточно предаване |
| **2** | RAG система с оценка | Вграждания, семантично търсене, качествени метрики |
| **3** | Комплект за бенчмаркинг на много модели | Измерване на производителност, сравнение на модели |
| **4** | Сравнителен анализ на SLM срещу LLM | Анализ на компромиси, стратегии за оптимизация |
...
- Приложения за здравеопазване, обработващи PHI/PII локално
...
- Хибридни edge-облак внедрявания
- Оптимизирана AI инфраструктура
...
| **2** | Създаване на AI решения с RAG | Инженеринг на подканяния, вграждания, оценка | 30 мин |
...
**✅ Успех!** Трябва да видите поточен отговор за Edge AI.
...
|-------|--------|----------|---------------------|
| 1 | [`chat_bootstrap.py`](../../../Workshop/samples/session01/chat_bootstrap.py) | Основен и поточен чат | ~30s |
| 2 | [`rag_pipeline.py`](../../../Workshop/samples/session02/rag_pipeline.py) | RAG с вграждания | ~45s |
| 2 | [`rag_eval_ragas.py`](../../../Workshop/samples/session02/rag_eval_ragas.py) | Оценка на качеството на RAG | ~60s |
| 3 | [`benchmark_oss_models.py`](../../../Workshop/samples/session03/benchmark_oss_models.py) | Бенчмаркинг на много модели | ~2-3m |
...
| 6 | [`models_pipeline.py`](../../../Workshop/samples/session06/models_pipeline.py) | Многоетапна pipeline | ~60s |
...
|-------|----------|----------|----------|
| 1 | [`session01_chat_bootstrap.ipynb`](./notebooks/session01_chat_bootstrap.ipynb) | Основи на чат и поточно предаване | ⭐ Начинаещ |
| 2 | [`session02_rag_pipeline.ipynb`](./notebooks/session02_rag_pipeline.ipynb) | Създаване на RAG система | ⭐⭐ Средно |
| 2 | [`session02_rag_eval_ragas.ipynb`](./notebooks/session02_rag_eval_ragas.ipynb) | Оценка на качеството на RAG | ⭐⭐ Средно |
| 3 | [`session03_benchmark_oss_models.ipynb`](./notebooks/session03_benchmark_oss_models.ipynb) | Бенчмаркинг на модели | ⭐⭐ Средно |
| 4 | [`session04_model_compare.ipynb`](./notebooks/session04_model_compare.ipynb) | Сравнение на модели | ⭐⭐ Средно |
...
| 6 | [`session06_models_pipeline.ipynb`](./notebooks/session06_models_pipeline.ipynb) | Оркестрация на pipeline | ⭐⭐⭐ Напреднал |
...
| [FOUNDRY_SDK_QUICKREF.md](./FOUNDRY_SDK_QUICKREF.md) | Модели и примери за SDK | Пишете код |
...
2. ✅ Сесия 2: Пълна RAG pipeline с оценка
3. ✅ Сесия 3: Пълен комплект за бенчмаркинг
...
Ако следвате кондензирания формат на 6-сесийна работилница, използвайте тези специализирани ръководства (всяко съответства и допълва по-широките модулни документи по-горе):

| Сесия на работилницата | Ръководство | Основен фокус |
|-------------------------|------------|---------------|
| 1 | [Session01-GettingStartedFoundryLocal](./Session01-GettingStartedFoundryLocal.md) | Инсталиране, валидиране, работа с phi & GPT-OSS-20B, ускорение |
|
...
Ако Foundry Local услугата се изпълнява на различна (Windows) машина или виртуална машина от macOS, експортирайте крайна точка:
...
| 5 | `samples/session05/agents_orchestrator.py` | Двуагентен изследователски → редакторски процес |
| 6 | `samples/session06/models_router.py` | Демонстрация на рутинг, базиран на намерение |
|   | `samples/session06/models_pipeline.py` | Многостъпков план/изпълнение/усъвършенстване на веригата |

### Променливи на средата (общи за всички примери)

| Променлива | Цел | Пример |
|------------|-----|--------|
| `FOUNDRY_LOCAL_ALIAS` | Основен псевдоним за единичен модел за базови примери | `phi-4-mini` |
...
Ако дадена променлива не е зададена, скриптовете използват разумни стойности по подразбиране. За демонстрации с единичен модел обикновено е необходим само `FOUNDRY_LOCAL_ALIAS`.
...
Това намалява дублирането и подчертава най-добрите практики за ефективна локална оркестрация на модели.
...
| Многомоделни агенти | Специализация на хетерогенни роли | 5 | `AGENT_MODEL_PRIMARY`, `AGENT_MODEL_EDITOR` |
| Адаптивен рутинг | Намерение + евристики за разходи | 6 | Разширете рутера с логика за ескалация |
...
| Тестове за проверка | Бърза предварителна проверка за работилницата | Всички | `python Workshop/tests/smoke.py` |
...
Очаквайте стабилни броя на токените при повторение на идентични входни данни.
...
Разширете, като предоставите по-голям JSONL с въпроси, контексти и истини, след което го конвертирайте в Hugging Face `Dataset`.
...
Стабилизирането на нови CLI команди за телеметрия може да бъде включено с минимални редакции в markdown файловете на сесиите.

### Автоматична проверка на синтаксиса

Автоматичен линтер предотвратява повторното въвеждане на остарели CLI модели в оградени кодови блокове на markdown файлове:

Скрипт: `Workshop/scripts/lint_markdown_cli.py`

Остарелите модели са блокирани в оградени кодови блокове.
...
| Задача | CLI команда | Еквивалент в SDK (Python) | Бележки |
|--------|-------------|---------------------------|---------|
...
| Списък с каталога | `foundry model list` | `# use manager for each alias or maintain known list` | CLI агрегира; SDK в момента е за всяка инстанция на псевдоним |
| Списък с кеширани модели | `foundry cache list` | `manager.list_cached_models()` | След инициализация на мениджъра (всеки псевдоним) |
| Активиране на GPU ускорение | `foundry config set compute.onnx.enable_gpu true` | `# CLI action; SDK assumes config already applied` | Конфигурацията е външен страничен ефект |
| Получаване на URL на крайна точка | (имплицитно) | `manager.endpoint` | Използва се за създаване на OpenAI-съвместим клиент |
| Загряване на модел | `foundry model run <alias>` след първия подканващ текст | `chat_once(alias, messages=[...])` (utility) | Помощните функции обработват първоначалното студено стартиране |
...
| Получаване на използване на токени | (гледайте изхода) | `resp.usage.total_tokens` | Предоставено, ако бекендът връща обект за използване |

## Експорт на бенчмарк в Markdown

Използвайте скрипта `Workshop/scripts/export_benchmark_markdown.py`, за да изпълните нов бенчмарк (същата логика като `samples/session03/benchmark_oss_models.py`) и да генерирате Markdown таблица, подходяща за GitHub, плюс суров JSON.
...
| Файл | Съдържание |
|------|------------|
| `benchmark_report.md` | Markdown таблица + насоки за интерпретация |
| `benchmark_report.json` | Суров масив от метрики (за сравнение/проследяване на тенденции) |
