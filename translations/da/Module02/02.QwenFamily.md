...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-01T21:12:29+00:00",
  "source_file": "Module02/02.QwenFamily.md",
  "language_code": "da"
}
-->
# Afsnit 2: Grundlæggende om Qwen-familien

Qwen-modelfamilien repræsenterer Alibaba Clouds omfattende tilgang til store sprogmodeller og multimodal AI. Den viser, at open source-modeller kan opnå bemærkelsesværdig ydeevne, samtidig med at de er tilgængelige på tværs af forskellige implementeringsscenarier. Det er vigtigt at forstå, hvordan Qwen-familien muliggør kraftfulde AI-funktioner med fleksible implementeringsmuligheder, samtidig med at den opretholder konkurrencedygtig ydeevne på tværs af forskellige opgaver.

## Ressourcer til udviklere

### Hugging Face Model Repository
Udvalgte modeller fra Qwen-familien er tilgængelige via [Hugging Face](https://huggingface.co/models?search=qwen), hvilket giver adgang til nogle varianter af disse modeller. Du kan udforske de tilgængelige varianter, finjustere dem til dine specifikke anvendelser og implementere dem gennem forskellige frameworks.
...
Ved afslutningen af denne vejledning vil du kunne:
...
Behovet for AI af høj kvalitet og tilgængelighed er blevet stadig vigtigere på tværs af forskellige scenarier. Overvej applikationer, der kræver fleksible implementeringsmuligheder for forskellige organisatoriske behov, omkostningseffektive løsninger, hvor API-omkostninger kan blive betydelige, flersproget funktionalitet til globale applikationer eller specialiseret domæneekspertise inden for områder som kodning og matematik.
...
- **Global rækkevidde**: Stærke flersprogede og tværkulturelle funktioner
- **Specialisering**: Domænespecifikke varianter til særlige anvendelser

## Qwen-modellens filosofi

Qwen-modelfamilien repræsenterer en omfattende tilgang til AI-modeludvikling, der prioriterer open source-tilgængelighed, flersproget funktionalitet og praktisk implementering, samtidig med at den opretholder konkurrencedygtige ydeevneegenskaber. Qwen-modeller opnår dette gennem forskellige modelstørrelser, højkvalitets træningsmetoder og specialiserede varianter til forskellige domæner.
...
### Grundlæggende designprincipper for Qwen
...
Denne tilgang kombinerer højkvalitets webindhold, akademisk litteratur, kodearkiver og flersprogede ressourcer. Træningsmetoden lægger vægt på både bredde af viden og dybde af forståelse på tværs af forskellige domæner og sprog.
...
**Dual-Mode Operation**: Evne til at skifte mellem hurtig svarmodus til simple forespørgsler og dybere tænkemodus til komplekse problemer.
...
Moderne implementeringsmiljøer drager fordel af Qwen-modellernes fleksibilitet på tværs af forskellige beregningskrav:

### Små modeller (0.5B-3B)

Qwen tilbyder effektive små modeller, der er velegnede til edge-implementering, mobile applikationer og miljøer med begrænsede ressourcer, samtidig med at de opretholder imponerende funktioner.

### Mellemstore modeller (7B-32B)

Mellemstore modeller tilbyder forbedrede funktioner til professionelle applikationer og giver en fremragende balance mellem ydeevne og beregningskrav.

### Store modeller (72B+)

Modeller i fuld skala leverer banebrydende ydeevne til krævende applikationer, forskning og virksomheders implementeringer, der kræver maksimal kapacitet.
...
Qwen-modeller opnår konsekvent konkurrencedygtige resultater på benchmarks, samtidig med at de tilbyder open source-tilgængelighed, hvilket viser, at åbne modeller kan matche proprietære alternativer.
...
Qwen-modeller demonstrerer stærke flersprogede funktioner på tværs af forskellige sprog:
...
```
# Eksempel på implementering på mobil enhed med kvantisering
from transformers import AutoModelForCausalLM, AutoTokenizer
import torch

# Indlæs kvantiseret model til mobil implementering
...
- **Qwen-14B (14B parametre)**: Forbedrede funktioner med forbedret ræsonnement og viden
- **Qwen-72B (72B parametre)**: Stor model, der leverer banebrydende ydeevne
...
Qwen2-serien markerede betydelige fremskridt inden for både sprog- og multimodale funktioner:

- **Qwen2-0.5B til 72B**: Omfattende række af sprogmodeller til forskellige implementeringsbehov
- **Qwen2-57B-A14B (MoE)**: Mixture-of-experts arkitektur til effektiv parameterbrug
- **Qwen2-VL**: Avancerede vision-sprog funktioner til billedforståelse
- **Qwen2-Audio**: Lydbehandling og forståelsesfunktioner
- **Qwen2-Math**: Specialiseret matematisk ræsonnement og problemløsning

### Qwen2.5-familien: Forbedret ydeevne

Qwen2.5-serien bragte betydelige forbedringer på alle dimensioner:

- **Udvidet træning**: 18 billioner tokens af træningsdata for forbedrede funktioner
...
- **Qwen3-235B-A22B**: Flagskib med mixture-of-experts model med 235B samlede parametre
...
- **Flersproget ekspertise**: Support til 119 sprog og dialekter
- **Forbedret træning**: 36 billioner tokens af diverse, højkvalitets træningsdata
...
Internationale applikationer drager fordel af Qwen-modellernes stærke flersprogede funktioner, hvilket muliggør konsistente AI-oplevelser på tværs af forskellige sprog og kulturelle kontekster.
...
Selvom Qwen-modeller klarer sig godt på tværs af generelle domæner, kan meget specialiserede applikationer drage fordel af domænespecifik finjustering eller specialiserede modeller.
...
Selvom mange sprog understøttes, kan ydeevnen variere på tværs af forskellige sprog, med de stærkeste funktioner i engelsk og kinesisk.

## Fremtiden for Qwen-modelfamilien

Qwen-modelfamilien repræsenterer den fortsatte udvikling mod demokratiseret AI af høj kvalitet. Fremtidige udviklinger inkluderer forbedrede effektivitetsoptimeringer, udvidede multimodale funktioner, forbedrede ræsonnementsmekanismer og bedre integration på tværs af forskellige implementeringsscenarier.

Efterhånden som teknologien fortsætter med at udvikle sig, kan vi forvente, at Qwen-modeller bliver stadig mere kapable, samtidig med at de opretholder deres open source-tilgængelighed, hvilket muliggør AI-implementering på tværs af forskellige scenarier og anvendelser.

Qwen-familien demonstrerer, at fremtiden for AI-udvikling kan omfavne både banebrydende ydeevne og åben tilgængelighed, hvilket giver organisationer kraftfulde værktøjer, samtidig med at gennemsigtighed og kontrol opretholdes.

## Udviklings- og integrations-eksempler

### Hurt
- Qwen3-235B-A22B opnår konkurrencedygtige resultater i benchmark-evalueringer inden for kodning, matematik og generelle evner sammenlignet med andre topmodeller som DeepSeek-R1, o1, o3-mini, Grok-3 og Gemini-2.5-Pro.  
- Qwen3-30B-A3B overgår QwQ-32B med 10 gange flere aktiverede parametre.  
- Qwen3-4B kan matche ydeevnen for Qwen2.5-72B-Instruct.  

**Effektivitetsresultater:**  
- Qwen3-MoE basismodeller opnår lignende ydeevne som Qwen2.5 tætte basismodeller, men bruger kun 10 % af de aktive parametre.  
- Betydelige omkostningsbesparelser både i træning og inferens sammenlignet med tætte modeller.  

**Multisproglige evner:**  
...
|------------------|-----------------|----------------|-------------------------|-----------------------------------|  
| **Qwen2.5**      | 0.5B-72B        | 32K-128K       | Balanceret ydeevne, multisproglig | Generelle applikationer, produktionsudrulning |  
...
| **Qwen2.5-VL**   | Varierende      | Variabel       | Vision-sprog forståelse | Multimodale applikationer, billedanalyse |  
| **Qwen3**        | 0.6B-235B       | Variabel       | Avanceret ræsonnement, tænkemodus | Kompleks ræsonnement, forskningsapplikationer |  
| **Qwen3 MoE**    | 30B-235B total  | Variabel       | Effektiv stor-skala ydeevne | Virksomhedsapplikationer, højtydende behov |  
...
3. **Lokal udrulning**: Brug Ollama eller direkte transformers til lokal test  
...
3. **Praktiser implementering**: Udrul modeller i udviklingsmiljøer  
4. **Optimer udrulning**: Finjustér til produktionsbrug  
...
Qwen-modelfamilien repræsenterer en omfattende tilgang til at demokratisere AI-teknologi, samtidig med at den opretholder konkurrencedygtig ydeevne på tværs af forskellige applikationer. Gennem sit engagement i open source-tilgængelighed, multisproglige evner og fleksible udrulningsmuligheder giver Qwen organisationer og udviklere mulighed for at udnytte kraftfulde AI-evner uanset deres ressourcer eller specifikke behov.  
...
**Skalerbar arkitektur**: Spændet fra 0.5B til 235B parametre muliggør udrulning på tværs af hele spektret af computermiljøer, fra mobile enheder til virksomhedsklynger.  
...
**Global tilgængelighed**: Stærk multisproglig support på tværs af 119+ sprog gør Qwen velegnet til internationale applikationer og diverse brugergrupper.  

**Kontinuerlig innovation**: Udviklingen fra Qwen 1.0 til Qwen3 viser konstant forbedring af evner, effektivitet og udrulningsmuligheder.  
...
- **Bedre udrulningsværktøjer**: Forbedrede rammer og optimeringsværktøjer til diverse udrulningsscenarier  
- **Fællesskabsvækst**: Udvidet økosystem af værktøjer, applikationer og fællesskabsbidrag  

### Næste skridt  

Uanset om du bygger en chatbot, udvikler uddannelsesværktøjer, skaber kodningsassistenter eller arbejder på multisproglige applikationer, tilbyder Qwen-familien skalerbare løsninger med stærk fællesskabsstøtte og omfattende dokumentation.  

For de nyeste opdateringer, modeludgivelser og detaljeret teknisk dokumentation, besøg de officielle Qwen-repositorier på Hugging Face og udforsk de aktive fællesskabsdiskussioner og eksempler.  
...
- **Fællesskab**: [GitHub Diskussioner og Problemer](https://github.com/QwenLM/)  
