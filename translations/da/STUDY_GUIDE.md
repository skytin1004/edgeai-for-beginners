...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T19:59:12+00:00",
...
| Dag 7 | Modul 6-7: AI-agenter & Udviklingsværktøjer | 5 timer |
...
| Dag 7-8 | Modul 4: Modeloptimering | 4 timer |
| Dag 9-10 | Modul 5: SLMOps | 3 timer |
| Dag 11-12 | Modul 6: SLM Agentbaserede Systemer | 2 timer |
...
| Modul 5: SLMOps | | | |
| Modul 6: SLM Agentbaserede Systemer | | | |
...
Velkommen til EdgeAI for begyndere studieguide! Dette dokument er designet til at hjælpe dig med at navigere i kursusmaterialet effektivt og maksimere din læringsoplevelse. Det giver strukturerede læringsforløb, foreslåede studieplaner, nøglebegrebsoversigter og supplerende ressourcer til at uddybe din forståelse af EdgeAI-teknologier.

Dette er et kortfattet 20-timers kursus, der leverer essentiel viden om EdgeAI i et tidsbesparende format, hvilket gør det perfekt for travle fagfolk og studerende, der hurtigt vil opnå praktiske færdigheder inden for dette fremvoksende område.
...
4. **Model Formatkonvertering og Kvantisering** - Avanceret optimering med 6 frameworks, herunder OpenVINO
5. **SLMOps - Small Language Model Operations** - Produktionslivscyklus og implementering
6. **SLM Agentbaserede Systemer** - AI-agenter, funktionkald og Model Context Protocol
7. **EdgeAI Implementerings Eksempler** - AI Toolkit, Windows-udvikling og platformspecifikke implementeringer
8. **Microsoft Foundry Local – Komplet Udviklingsværktøj** - Lokal-først udvikling med hybrid Azure-integration (Modul 08)
...
- **Praktisk øvelse**: Fuldfør de foreslåede øvelser for at styrke teoretiske begreber
...
- **Prioriterede begreber**: 
  ...
  - Hardwareacceleration (NPUs, GPUs, CPUs)
...
1. Sammenlign og kontraster cloud-baseret AI med edge-baseret AI-implementeringer.
...
2. **Modelundersøgelse**: Download og undersøg en forudtrænet lille sproglig model (1 time)
...
- Genkende passende anvendelser for forskellige modelfamilier
...
- Mestre optimeringsteknikker for forskellige implementeringsscenarier
- Implementer SLM'er i både lokale og cloud-miljøer
- Design produktionsklare konfigurationer for EdgeAI-applikationer
...
- **Prioriterede begreber**: 
  ...
  - Modelanskaffelsesstrategier

#### Sektion 2: Lokal Miljø Implementering
- **Prioriterede begreber**: 
...
3. Forklar fordelene ved containerisering for SLM implementering.
...
## Modul 4: Model Formatkonvertering og Kvantisering

### Vigtige læringsmål

- Mestre avancerede kvantiseringsteknikker fra 1-bit til 8-bit præcision
- Forstå formatkonverteringsstrategier (GGUF, ONNX)
...
5. Beskriv, hvordan Apple MLX udnytter enhedsarkitekturen for hukommelse til optimering.  
...
## Modul 5: SLMOps - Drift af små sproglige modeller  

### Centrale læringsmål  

- Forstå principperne for livscyklusstyring i SLMOps  
- Mestre destillation og finjusteringsteknikker til edge-implementering  
- Implementere produktionsstrategier med overvågning  
- Opbygge workflows til drift og vedligeholdelse af SLM i virksomhedsklasse  
...
#### Sektion 2: Modeldestillation  
- **Prioritetsbegreber**:  
  - Teknikker til vidensoverførsel  
  - Implementering af to-trins destillationsprocesser  
  - Destillationsworkflows med Azure ML  
...
- **Prioritetsbegreber**:  
  ...
  - Konfiguration af Foundry Local til implementering  
  - Benchmarking af performance og validering af kvalitet  

### Selv-evalueringsspørgsmål  

1. Hvordan adskiller SLMOps sig fra traditionel MLOps?  
2. Forklar fordelene ved modeldestillation til edge-implementering.  
...
1. **Grundlæggende destillation**: Skab en mindre model fra en større lærer-model (1 time)  
...
## Modul 6: SLM Agentiske Systemer - AI-agenter og funktionkald  

### Centrale læringsmål  

- Byg intelligente AI-agenter til edge-miljøer ved hjælp af små sproglige modeller  
- Implementer funktionkald med systematiske workflows  
- Mestre integration af Model Context Protocol (MCP) for standardiseret værktøjsinteraktion  
...
#### Sektion 2: Funktionkald i små sproglige modeller  
- **Prioritetsbegreber**:  
...
#### Sektion 3: Integration af Model Context Protocol (MCP)  
- **Prioritetsbegreber**:  
...
- **Prioritetsbegreber**:  
  ...
  - Lokale test-, optimerings- og agentudviklingsworkflows  
...
## Modul 8: Microsoft Foundry Local – Komplet udviklerværktøj  

### Centrale læringsmål  

- Installér og konfigurer Foundry Local på Windows  
- Kør, opdag og administrer modeller lokalt via Foundry CLI  
- Integrer med OpenAI-kompatible REST- og SDK-klienter  
- Byg praktiske eksempler: Chainlit chat, agenter og modelrouter  
- Forstå hybride mønstre med Azure AI Foundry  

### Fokusområder  

- Installation og CLI-essentials (model, service, cache)  
- SDK-integration (OpenAI-kompatible klienter og Azure OpenAI)  
- Hurtig validering med Open WebUI  
- Agent- og funktionkaldsmønstre  
- Modeller som værktøjer (router- og registreringsdesign)  

### Selv-evalueringsspørgsmål  

1. Hvordan opdager du den lokale endpoint og lister tilgængelige modeller?  
2. Hvad er forskellene mellem Foundry Local REST og Azure OpenAI-brug?  
3. Hvordan ville du designe en simpel router til at vælge modeller som værktøjer?  
4. Hvilke CLI-kategorier er mest relevante for daglig udvikling?  
5. Hvordan validerer du Foundry Local-parathed før kørsel af apps?  

### Praktiske øvelser  

1. Installér/opgrader Foundry Local og kør `phi-4-mini` lokalt (30 minutter)  
2. Kald `/v1/models` og kør en simpel chat via REST (30 minutter)  
3. Start Chainlit-app-eksemplet og chat lokalt (30 minutter)  
4. Kør multi-agent koordinatoren og inspicér outputs (30 minutter)  
5. Prøv routeren med modeller som værktøjer med miljøbaserede overrides (30 minutter)  
...
- Fuldfør modulerne 1, 2 og 3 (kernebegreber inden for EdgeAI)  
- Lav mindst én praktisk øvelse pr. modul  
- Fokusér på at forstå de grundlæggende begreber frem for implementeringsdetaljer  

**Hvis du kan dedikere de fulde 20 timer:**  
- Fuldfør alle syv moduler  
...
- Fuldfør alle moduler med detaljerede øvelser  
...
- [OpenVINO GenAI](https://github.com/openvinotoolkit/openvino.genai) - Toolkit til implementering af store sproglige modeller  

## Fremskridtssporingsskabelon  

Brug denne forenklede skabelon til at spore din læringsfremskridt gennem det 20-timers kursus:  
...
Overvej at fuldføre et af disse projekter for at øve EdgeAI-konceptet (hver designet til at tage 2-4 timer):  

### Begynderprojekter (2-3 timer hver)  
1. **Edge Tekstassistent**: Skab et simpelt offline tekstfuldførelsesværktøj ved hjælp af en lille sproglig model  
2. **Dashboard til Model-sammenligning**: Byg en grundlæggende visualisering af performance-metrics på tværs af forskellige SLM'er  
...
7. **Funktionkaldsagent**: Byg en AI-agent med funktionkaldsmuligheder til edge-scenarier  

### Avancerede integrationsprojekter (4-5 timer hver)  
8. **OpenVINO Optimeringspipeline**: Implementer komplet modeloptimering ved hjælp af NNCF og GenAI-værktøjssættet  
9. **SLMOps Pipeline**: Implementer en komplet modellivscyklus fra træning til edge-udrulning  
10. **Multi-Model Edge System**: Udrul flere specialiserede modeller, der arbejder sammen på edge-hardware  
...
Deltag i diskussionen og forbind dig med andre lærende:  
...
EdgeAI repræsenterer grænsen for implementering af kunstig intelligens, hvor kraftfulde funktioner bringes direkte til enheder, samtidig med at vigtige bekymringer om privatliv, latenstid og forbindelsesmuligheder adresseres. Dette 20-timers kursus giver dig den nødvendige viden og praktiske færdigheder til straks at begynde at arbejde med EdgeAI-teknologier.  

Kurset er bevidst kortfattet og fokuseret på de vigtigste begreber, så du hurtigt kan opnå værdifuld ekspertise uden en overvældende tidsforpligtelse. Husk, at praktisk erfaring, selv med enkle eksempler, er nøglen til at styrke det, du har lært.  
