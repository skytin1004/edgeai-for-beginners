...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T20:23:38+00:00",
...
## Læringsmål
- Design mønstre for modeller-som-værktøjer på enheden
...
## Del 1: Værktøjsabstraktioner (Trin-for-trin)

Mål: Repræsenter modeller som værktøjer med klare kontrakter og en simpel router.

Trin 1) Definér værktøjsinterface og register  
```python
# tools/registry.py
...
Trin 2) Implementér to værktøjer understøttet af Foundry Local  
```python
# tools/impl.py
import requests, os
...
Trin 3) Router baseret på opgave  
```python
# tools/router.py
...
## Del 2: SDK- og API-integration (Trin-for-trin)

Mål: Brug OpenAI Python SDK mod Foundry Local-endpointet.
...
Mål: Skræddersy output til et domæne ved hjælp af promptskabeloner og JSON-schema.

Trin 1) Opret en domæne-promptskabelon  
```python
# domain/templates.py
...
Trin 1) Forvarm og valider lokalt endpoint  
```cmd
...
## Del 5: Skalering til Azure AI Foundry (Trin-for-trin)

Mål: Spejl lokale modeller med Azure-endpoints for overflow-kapacitet.

Trin 1) Beslut routing-strategi  
- Lokalt først for privatliv/latens, Azure fallback ved fejl eller store prompts  

Trin 2) Implementér en simpel routerstub  
```python
# hybrid/router.py
import os, requests
...
## Hands-On Tjekliste
- [ ] Registrér mindst to værktøjer og rout forespørgsler  
- [ ] Kald Foundry Local via OpenAI SDK og rå REST  
- [ ] Tving JSON-output for en domæneskabelon  
- [ ] Rens og log kald lokalt  
- [ ] Implementér en simpel hybrid router med Azure fallback  

## Afslutning

Foundry Local muliggør robust AI på enheden, hvor modeller bliver sammensatte værktøjer. Med klare interfaces, governance og hybrid skalering kan teams levere realtids-, sikre AI-apps, der respekterer brugerens privatliv og samtidig er klar til virksomhedsanvendelse.
