...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T23:14:32+00:00",
...
Med udgangspunkt i din Foundry Local-viden fokuserer denne session på avancerede integrationsmønstre for OpenAI SDK, der understøtter både Microsoft Foundry Local og Azure OpenAI. Du vil lære at bygge fleksible AI-applikationer, der kan køre lokalt for privatliv og udvikling, samtidig med at de tilbyder cloud-skalerbarhed via Azure OpenAI, når det er nødvendigt.

## Læringsmål

Ved afslutningen af denne session vil du:
- Mestre avanceret OpenAI SDK-integration med både Foundry Local og Azure OpenAI
- Implementere streaming-svar for forbedret brugeroplevelse
- Oprette robuste klientfabrikmønstre til understøttelse af flere leverandører
- Bygge samtalestyringssystemer med kontekstbevarelse
- Etablere performance-benchmarking og sundhedsovervågning
- Udrulle produktionsklare applikationer med korrekt fejlhåndtering

## Forudsætninger

- Fuldført Session 1: Kom godt i gang med Foundry Local
- Aktiv Foundry Local-installation med kørende modeller
- Python 3.8 eller nyere med mulighed for virtuelle miljøer
- OpenAI Python SDK installeret (`pip install openai foundry-local-sdk`)
- Azure-konto med OpenAI-tjeneste (valgfrit, til cloud-scenarier)
- Grundlæggende forståelse af Python async/await-mønstre

## Del 1: OpenAI SDK Client Factory Pattern

### Forståelse af Multi-Provider Arkitektur

At bygge applikationer, der fungerer med både Foundry Local og Azure OpenAI, kræver et fleksibelt mønster for klientoprettelse:
...
## Del 2: Streaming-svar og realtidsinteraktion

### Implementering af Streaming Chat Completions

Streaming giver en bedre brugeroplevelse ved at vise svar, mens de genereres:
...
## Del 3: Performance Benchmarking og Analyse

### Måling af svartid

Mål og sammenlign ydeevne på tværs af forskellige konfigurationer:
...
### Temperatur- og parametertestning
...
## Del 4: Sundhedsovervågning og Diagnostik

### Omfattende sundhedstjek-system
...
### Validator for miljøkonfiguration
...
## Del 5: Miljøvariabler og konfigurationsstyring

### Reference for miljøvariabler

Komplet reference til konfiguration af begge leverandører:
...
### Øvelse 1: Multi-Provider SDK Integration

Byg en komplet applikation, der problemfrit skifter mellem leverandører:
...
### Øvelse 2: Avanceret Samtalestyring
...
### Øvelse 3: Produktionsklar Sundhedsovervågning
...
## Del 7: Opsummering og Næste Skridt

### Sessionens Resultater

I denne session har du mestret:
- ✅ **OpenAI SDK Integration**: Avancerede mønstre for både Foundry Local og Azure OpenAI
- ✅ **Streaming-svar**: Realtids chat completions for forbedret brugeroplevelse
- ✅ **Multi-Provider Support**: Problemfri skift mellem lokale og cloud AI-tjenester
- ✅ **Samtalestyring**: Kontekstbevidste multi-turn samtaler
- ✅ **Performance Overvågning**: Benchmarking og sundhedstjek til produktionsudrulninger
- ✅ **Produktionsmønstre**: Robust fejlhåndtering og konfigurationsstyring

### Nøglearkitekturmønstre
...
1. **🔄 Implementer altid fallback-mekanismer**: Azure → Foundry Local → Fejlhåndtering
2. **🌊 Brug streaming til lange svar**: Bedre opfattet ydeevne
3. **🛡️ Implementer omfattende fejlhåndtering**: Brugervenlige fejlmeddelelser
4. **📈 Overvåg ydeevne**: Spor svartider og succesrater
5. **⚙️ Konfiguration baseret på miljø**: Nem skift mellem udvikling/staging/produktion
6. **🔒 Sikker håndtering af legitimationsoplysninger**: Aldrig hardcode API-nøgler

### Retningslinjer for valg af leverandør

| Scenario | Anbefalet Leverandør | Begrundelse |
|----------|---------------------|-------------|
| **Udvikling** | Foundry Local | Hurtig iteration, ingen API-omkostninger |
| **Privatlivskritisk** | Foundry Local | Data forlader aldrig enheden |
| **Højvolumen Produktion** | Azure OpenAI | Bedre skalering, enterprise SLA |
| **Nyeste Modeller** | Azure OpenAI | Adgang til de nyeste modeludgivelser |
| **Offline Krav** | Foundry Local | Ingen internetafhængighed |
| **Omkostningsfølsom** | Foundry Local | Ingen per-token gebyrer |

### Hurtig Reference for Miljøvariabler
...
### Forberedelse til Session 3: Open-Source Modeller

1. **Udforsk Modelkatalog**: Gennemgå tilgængelige modeller i Foundry Local
2. **Forstå Modelformater**: Lær om ONNX, kvantisering og optimering
3. **Overvej Custom Modeller**: Tænk på domænespecifikke modelbehov
...
- **[OpenAI Python SDK Dokumentation](https://github.com/openai/openai-python)**: Officiel SDK-reference
- **[Azure OpenAI Dokumentation](https://learn.microsoft.com/azure/ai-services/openai/)**: Azure OpenAI-tjenestevejledning
- **[Foundry Local SDK Reference](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**: Lokal inferensdokumentation
- **[Streaming Completions Guide](https://learn.microsoft.com/azure/ai-foundry/foundry-local/how-to/integrate-with-inference-sdks)**: Avancerede streamingmønstre
- **[Sample 01: Quick Chat via OpenAI SDK](samples/01/README.md)**: Grundlæggende integrationsmønstre
- **[Sample 02: Advanced SDK Integration](samples/02/README.md)**: Denne sessions praktiske eksempler
- **[Sample 04: Chainlit Application](samples/04/README.md)**: Web UI-udvikling
- **[Sample 05: Multi-Agent Systems](samples/05/README.md)**: Avancerede orkestreringsmønstre

Du er nu klar til at bygge sofistikerede AI-applikationer, der problemfrit integrerer lokale og cloud AI-funktioner, hvilket giver fleksibilitet til at vælge den rette leverandør til hver specifik brugssituation, samtidig med at du opretholder konsistente udviklingsmønstre.
