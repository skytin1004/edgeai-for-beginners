...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T18:31:23+00:00",
...
Riferimenti:
- Documentazione di Foundry Local: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/
...
## Parte 1: Astrazioni degli Strumenti (Passo dopo passo)

Obiettivo: Rappresentare i modelli come strumenti con contratti chiari e un router semplice.

Passo 1) Definire l'interfaccia dello strumento e il registro
```python
# tools/registry.py
...
Passo 2) Implementare due strumenti basati su Foundry Local
```python
# tools/impl.py
import requests, os
...
## Parte 2: Integrazione SDK e API (Passo dopo passo)

Obiettivo: Utilizzare l'SDK Python di OpenAI contro l'endpoint di Foundry Local.
...
Passo 3) Chiamare l'API di chat
```python
# sdk_demo.py
...
Obiettivo: Adattare gli output per un dominio utilizzando modelli di prompt e schema JSON.
...
## Parte 4: Modalità Offline e Sicurezza (Passo dopo passo)
...
## Parte 5: Scalabilità su Azure AI Foundry (Passo dopo passo)

Obiettivo: Rispecchiare i modelli locali con endpoint Azure per capacità di overflow.

Passo 1) Decidere la strategia di routing
- Locale-prima per privacy/latenza, fallback su Azure in caso di errori o prompt grandi

Passo 2) Implementare un semplice stub di router
```python
# hybrid/router.py
import os, requests
...
## Checklist Pratica
- [ ] Registrare almeno due strumenti e instradare le richieste
- [ ] Chiamare Foundry Local tramite SDK OpenAI e REST grezzo
- [ ] Forzare output JSON per un modello di dominio
- [ ] Sanificare e registrare le chiamate localmente
- [ ] Implementare un semplice router ibrido con fallback su Azure

## Conclusione

Foundry Local consente un'AI robusta sul dispositivo, dove i modelli diventano strumenti componibili. Con interfacce chiare, governance e scalabilità ibrida, i team possono sviluppare app AI in tempo reale, sicure e rispettose della privacy, mantenendo al contempo la prontezza aziendale.
