...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-11T14:26:27+00:00",
...
Questa guida completa ti aiuterà a prepararti per il corso EdgeAI, che si concentra sulla creazione di soluzioni AI pratiche ed efficienti per dispositivi edge. Il corso enfatizza lo sviluppo pratico utilizzando framework moderni e modelli all'avanguardia ottimizzati per il deployment su edge.
...
- **Ambiente Virtuale**: Utilizza venv o ambienti conda per l'isolamento
...
- Jupyter Notebooks per lo sviluppo interattivo e il prototipaggio
...
### Strategia per Risorse di Calcolo
Il corso è progettato per essere accessibile su diverse configurazioni hardware:

**Sviluppo Locale (Focus su CPU/NPU)**
- Lo sviluppo principale utilizzerà accelerazione CPU e NPU
- Adatto alla maggior parte di laptop e desktop moderni
- Focus su efficienza e scenari di deployment pratici

**Risorse Cloud GPU (Opzionale)**
...
- **Kaggle Notebooks**: Alternativa per il calcolo su cloud

### Considerazioni sui Dispositivi Edge
- Comprensione dei processori basati su ARM
- Conoscenza delle limitazioni hardware di dispositivi mobili e IoT
...
- **Risorsa**: [Collezione Phi-4 su Hugging Face](https://huggingface.co/collections/microsoft/phi-4-677e9380e514feb5577a40e4)
...
- **Risorsa**: [Collezione Qwen-3 su Hugging Face](https://huggingface.co/collections/Qwen/qwen3-67dd247413f0e2e4f653967f)
...
- **Risorsa**: [Collezione Gemma-3n su Hugging Face](https://huggingface.co/collections/google/gemma-3n-685065323f5984ef315c93f4)
...
- **Installazione e Uso Base**:
  ...
  # Esempio di script Python per l'ottimizzazione dei modelli
  ```python
...
### Apple MLX (Utenti macOS)
- **Repository**: [Apple MLX su GitHub](https://github.com/ml-explore/mlx)
- **Scopo**: Framework di machine learning per Apple Silicon
- **Caratteristiche Chiave**:
  - Ottimizzazione nativa per Apple Silicon
  - Operazioni efficienti in termini di memoria
  - API simile a PyTorch
  - Supporto per architettura di memoria unificata
- **Installazione e Uso Base**:
  ```bash
  # Install MLX
  pip install mlx
...
- **Scopo**: Accelerazione multipiattaforma per inferenza di modelli ONNX
- **Caratteristiche Chiave**:
  - Ottimizzazioni specifiche per hardware (CPU, GPU, NPU)
  - Ottimizzazioni grafiche per inferenza
  - Supporto per quantizzazione
...
- **Design Patterns per Edge AI**: Best practice per il deployment su edge
...
- [ ] Account per il calcolo su cloud configurati (se necessario)
...
4. Selezionare modelli appropriati in base ai vincoli e ai requisiti dei dispositivi
