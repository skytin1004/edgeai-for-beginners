...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-22T12:36:56+00:00",
...
O Microsoft Olive é uma ferramenta poderosa e fácil de usar para a otimização de modelos de machine learning, adaptada ao hardware, que simplifica o processo de preparação de modelos para diferentes plataformas de hardware. Seja para CPUs, GPUs ou aceleradores de IA especializados, o Olive ajuda a alcançar o desempenho ideal enquanto mantém a precisão do modelo.

## O que é o Microsoft Olive?

O Olive é uma ferramenta de otimização de modelos adaptada ao hardware, que combina técnicas líderes da indústria em compressão, otimização e compilação de modelos. Funciona com o ONNX Runtime como uma solução de otimização de inferência de ponta a ponta.

### Principais Funcionalidades

- **Otimização Adaptada ao Hardware**: Seleciona automaticamente as melhores técnicas de otimização para o hardware alvo.
- **Mais de 40 Componentes de Otimização Integrados**: Inclui compressão de modelos, quantização, otimização de grafos, entre outros.
- **Interface CLI Simples**: Comandos fáceis para tarefas comuns de otimização.
- **Suporte a Múltiplas Frameworks**: Compatível com PyTorch, modelos Hugging Face e ONNX.
- **Suporte a Modelos Populares**: O Olive otimiza automaticamente arquiteturas de modelos populares como Llama, Phi, Qwen, Gemma, entre outros.

### Benefícios

- **Redução do Tempo de Desenvolvimento**: Elimina a necessidade de experimentar manualmente diferentes técnicas de otimização.
...
O Olive oferece várias dependências opcionais para funcionalidades adicionais:
...
Se a instalação for bem-sucedida, deverá ver a mensagem de ajuda do CLI do Olive.

## Guia Rápido

### A Sua Primeira Otimização

Vamos otimizar um pequeno modelo de linguagem utilizando a funcionalidade de auto-otimização do Olive:
...
O processo de otimização inclui: obter o modelo do cache local, capturar o grafo ONNX e armazenar os pesos num ficheiro de dados ONNX, otimizar o grafo ONNX e quantizar o modelo para int4 utilizando o método RTN.
...
- `--use_ort_genai`: Utilizar o ONNX Runtime Generate AI para inferência.
- `--precision`: Precisão de quantização (int4, int8, fp16).
- `--log_level`: Verbosidade do registo (0=mínimo, 1=detalhado).
...
### Passo 1: Fazer Download do Modelo (Opcional)

Para minimizar o tempo de download, faça cache apenas dos ficheiros essenciais:
...
Para fluxos de trabalho de otimização mais complexos, pode utilizar ficheiros de configuração JSON:
...
O Olive também suporta o ajuste fino de modelos:
...
- Alinhe o alvo de otimização ao hardware de implantação.
- Utilize otimização para GPU se tiver hardware compatível com CUDA.
...
- Utilize dados de entrada representativos para avaliação.

### 5. Otimização Iterativa
- Comece com auto-otimização para resultados rápidos.
- Utilize ficheiros de configuração para controlo detalhado.
...
- Utilize tamanhos de lote menores durante a otimização.
...
- Certifique-se de que todos os ficheiros necessários do modelo foram descarregados.
...
- **Publicações no Blog**: Blog de Código Aberto da Microsoft — https://opensource.microsoft.com/blog/
...
- **Azure Machine Learning**: Fluxos de trabalho de otimização na cloud — https://learn.microsoft.com/azure/machine-learning/
