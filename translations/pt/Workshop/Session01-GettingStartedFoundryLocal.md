...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-08T20:44:55+00:00",
...
- **Dominar Operações na CLI**: Utilizar a CLI do Foundry Local para gestão e implementação de modelos
...
- **Construir a Sua Primeira Aplicação**: Adaptar exemplos existentes para usar o Foundry Local Python SDK
...
foundry model list
## NOTE: Atualmente não há uma flag dedicada `--running`; para ver quais estão carregados, inicie um chat ou inspecione os logs do serviço.
...
Crie um arquivo `.env` (ou defina no shell) para tornar os scripts portáveis:
# Comparar respostas (não interativo)
foundry model run gpt-oss-20b --prompt "Explique edge AI em termos simples"
...
| `FOUNDRY_LOCAL_ENDPOINT` | Substituir endpoint (caso contrário, automático a partir do gerenciador) | `http://localhost:5273/v1` |
| `FOUNDRY_LOCAL_STREAM` | Ativar demonstração de streaming | `true` |

> Se `FOUNDRY_LOCAL_ENDPOINT=auto` (ou não definido), derivamos do gerenciador do SDK.

## Fluxo da Demonstração (30 minutos)

### 1. Instalar o Foundry Local e Verificar Configuração da CLI (10 minutos)
...
Em vez de iniciar manualmente o serviço e executar os modelos, o **Foundry Local Python SDK** pode inicializar tudo automaticamente:
...
Se preferir controle explícito, ainda pode usar a CLI + cliente OpenAI conforme mostrado posteriormente.

### 2. Ativar Aceleração de GPU (5 minutos)
...
| Teste de Fumaça | Verificação rápida de fluxos principais | Execute `python Workshop/tests/smoke.py` antes de um workshop |
...
| Aquecimento na Primeira Execução | Reduzir picos de latência p95 | Execute um prompt pequeno após a criação do `FoundryLocalManager` |

Exemplo de aquecimento determinístico inicial (PowerShell):
...
Você deve ver uma saída semelhante e contagens de tokens idênticas na segunda execução, confirmando o determinismo.
...
3. **Otimize o Desempenho**: Ajuste as configurações para o seu hardware específico
4. **Construa Aplicações Personalizadas**: Utilize o Foundry Local SDK nos seus próprios projetos
...
| Código de adaptação do quickstart | Desenvolvedor migrando um script OpenAI existente para Foundry Local | Mostrar compatibilidade direta | "Dê dois benefícios da inferência local." | Apenas prompt inline |

### Narrativa do Cenário
A equipe de segurança e conformidade deve validar se os dados sensíveis do protótipo podem ser processados localmente. Eles executam o script de inicialização com vários prompts (privacidade, latência, custo) usando um modo determinístico com temperatura=0 para capturar saídas de referência para comparação futura (benchmarking na Sessão 3 e contraste entre SLM e LLM na Sessão 4).

### Conjunto Mínimo de Prompts em JSON (opcional)
```json
[
