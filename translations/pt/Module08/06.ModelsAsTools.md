...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T11:27:32+00:00",
...
> **🔄 Atualizado para o SDK Moderno**: Este módulo foi alinhado com os padrões mais recentes do repositório Microsoft Foundry-Local e corresponde à implementação de roteamento inteligente em `samples/06/`. Os exemplos agora utilizam o moderno `foundry-local-sdk` e estratégias avançadas de seleção de modelos.
...
- **Configuração de Ambiente**: Atribuição flexível de modelos através de variáveis de ambiente
- **Monitorização de Saúde**: Validação de serviços e verificação de disponibilidade de modelos
- **Pronto para Produção**: Tratamento abrangente de erros e mecanismos de fallback
...
Objetivo: Implementar a seleção inteligente de modelos com roteamento automático baseado no conteúdo da consulta.

> **📋 Nota**: Esta implementação segue os padrões utilizados em `samples/06/router.py` com seleção avançada de modelos baseada em palavras-chave.

Passo 1) Definir o roteador moderno de modelos com FoundryLocalManager  
```python
...
Passo 2) Inicializar o cliente com o SDK moderno e descoberta de serviços  
```python
...
Passo 3) Implementar o roteamento e execução inteligentes (ver `samples/06/router.py`)  
```python
...
Objetivo: Usar o Foundry Local SDK com o OpenAI Python SDK para integração perfeita.
...
Passo 2) Configurar o ambiente (opcional - ver `samples/06/README.md`)  
```cmd
...
Passo 3) Integração com o SDK moderno  
```python
# modern_sdk_demo.py
from foundry_local import FoundryLocalManager
from openai import OpenAI
...
Passo 2) Forçar saída em formato JSON  
```python
# domain/analyst.py
...
Passo 3) Configurar flag de uso local e registo  
```python
...
Objetivo: Implementar o roteador inteligente com monitorização e integração com o Azure AI Foundry.
...
- [ ] Implementar o roteador inteligente de modelos com seleção baseada em palavras-chave (`samples/06/router.py`)
...
- [ ] Configurar modelos baseados no ambiente
- [ ] Implementar monitorização de saúde do serviço e tratamento de erros
- [ ] Implementar o roteador de produção com registo abrangente
...
- **Padrões Avançados**: Explore chamadas de funções e orquestração multi-agente no Módulo 5

## Conclusão

O Foundry Local permite uma IA robusta no dispositivo, onde os modelos se tornam ferramentas inteligentes e especializadas. Com seleção automática de modelos, monitorização abrangente e padrões prontos para produção, as equipas podem lançar aplicações de IA sofisticadas que se adaptam a diferentes tipos de tarefas, mantendo a privacidade e o desempenho. O padrão de roteador inteligente demonstrado aqui fornece uma base para construir sistemas de IA complexos que podem escalar do desenvolvimento local para a implementação em produção.
