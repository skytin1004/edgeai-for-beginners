...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-24T13:24:29+00:00",
  "source_file": "Module08/03.OpenSourceModels.md",
  "language_code": "ar"
}
-->
# الجلسة الثالثة: اكتشاف وإدارة النماذج مفتوحة المصدر

## نظرة عامة

تركز هذه الجلسة على اكتشاف النماذج وإدارتها عمليًا باستخدام Foundry Local. ستتعلم كيفية عرض النماذج المتاحة، اختبار الخيارات المختلفة، وفهم الخصائص الأساسية للأداء. يتمحور النهج حول الاستكشاف العملي باستخدام واجهة الأوامر (CLI) الخاصة بـ Foundry لمساعدتك في اختيار النماذج المناسبة لحالات الاستخدام الخاصة بك.

## أهداف التعلم

- إتقان أوامر واجهة الأوامر (CLI) الخاصة بـ Foundry لاكتشاف وإدارة النماذج
- فهم أنماط التخزين المؤقت للنماذج والتخزين المحلي
- تعلم كيفية اختبار ومقارنة النماذج بسرعة
...
- تثبيت واجهة الأوامر (CLI) الخاصة بـ Foundry Local وإمكانية الوصول إليها
...
## الجزء السادس: التمرين العملي

### التمرين: اكتشاف النماذج ومقارنتها

قم بإنشاء نص لتقييم النماذج الخاص بك بناءً على العينة 03:
...
## الجزء السابع: استكشاف الأخطاء وإصلاحها وأفضل الممارسات
...
- قم بترقية ذاكرة الوصول العشوائي (RAM) إذا كنت تواجه حدودًا بشكل متكرر
...
## الجزء الثامن: الخطوات التالية والمراجع

### التحضير للجلسة الرابعة

- **تركيز الجلسة الرابعة**: أدوات وتقنيات تحسين الأداء
...
## الجزء الأول: اكتشاف النماذج باستخدام واجهة الأوامر (CLI) الخاصة بـ Foundry

### أوامر إدارة النماذج الأساسية

توفر واجهة الأوامر (CLI) الخاصة بـ Foundry أوامر مباشرة لاكتشاف وإدارة النماذج:
...
- `phi-4-mini`: سريع وفعال، رائع للدردشة العامة
- `phi-4`: إصدار أكثر قدرة مع تحسين التفكير

**النماذج المتوسطة:**
- `qwen2.5-7b-instruct`: ممتاز في التفكير وسياق أطول
- `deepseek-r1-distill-qwen-7b`: مُحسّن لتوليد الأكواد

**النماذج الأكبر:**
- `llama-3.2`: أحدث نموذج مفتوح المصدر من Meta
- `qwen2.5-14b-instruct`: تفكير على مستوى المؤسسات

## الجزء الثاني: اختبار النماذج بسرعة ومقارنتها
...
## الجزء الثالث: إدارة التخزين المؤقت والتخزين للنماذج

### فهم التخزين المؤقت للنماذج

يدير Foundry Local تنزيل النماذج والتخزين المؤقت تلقائيًا:
...
- `qwen2.5-7b-instruct`: ~4.1 جيجابايت  
- `deepseek-r1-distill-qwen-7b`: ~4.3 جيجابايت
- `llama-3.2`: ~4.9 جيجابايت
- `qwen2.5-14b-instruct`: ~8.2 جيجابايت
...
## الجزء الرابع: إرشادات عملية لاختيار النماذج

### اختيار النماذج حسب حالة الاستخدام

**للدردشة العامة والأسئلة والأجوبة:**
- ابدأ بـ: `phi-4-mini` (سريع وفعال)
- قم بالترقية إلى: `phi-4` (تفكير أفضل)
- متقدم: `qwen2.5-7b-instruct` (سياق أطول)

**لتوليد الأكواد:**
- الموصى به: `deepseek-r1-distill-qwen-7b`
- البديل: `qwen2.5-7b-instruct` (جيد أيضًا للأكواد)

**للتفكير المعقد:**
- الأفضل: `qwen2.5-7b-instruct` أو `qwen2.5-14b-instruct`
...
**الموصى به للحصول على أفضل أداء:**
...
## الجزء الخامس: قياس الأداء البسيط للنماذج
...
- التحضير للجلسة الرابعة: مقارنة LLMs مقابل SLMs، الاستدلال المحلي مقابل السحابي، والعروض العملية
